...
| `make kv-check` | ~2s | `tools/tycho-kv/` — **the only lane that runs the persistent B+ tree KV store**. Three command scripts (1,816 commands, up to 5,074 output lines) replay against the B+ tree and a naive map backend, byte-identical; the reloaded store reproduces the batch; golden locked. A tree bug (lost key, stale separator, wrong scan) reddens here and nowhere else |
| `make scheme-check` | ~1s | `tools/tycho-scheme/` — **the only lane that runs the Scheme interpreter AND its bytecode compiler**. Six programs: interpreter output vs a byte-identical golden on two runs, and each program compiled and run on tycho-vm with the VM output byte-identical to the interpreter (the compiler differential); four compile-time rejections; five interpreter error cases dying non-zero with empty stdout. The programs stay shallow because their goldens are answers; deep-recursion crash tests live in `tests/recursion/run.sh` |
...
| `[3h] scheme-check` | `make scheme-check` (~1s; it builds and runs `tycho-scheme` over the four `progs/*.scm` — a red here is a changed golden or a die-path regression, and `RECORD=1 sh tools/tycho-scheme/run.sh` re-records once you know why. Programs stay shallow; the deep-recursion fail-closed tests are `tests/recursion/run.sh`) |
| `[4] conc` | `make conc` |
| `[5] ffi` | `make ffi` |
