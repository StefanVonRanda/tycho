...
NAME_SITES = [
    ...
    "unknown procedure '%s'",
    # The hard-coded sibling of the line above (src/tychoc.c:7459). It reached
...
FALLBACK = [
    ...
    # report_unused_locals (src/tychoc.c:5864) is an fprintf loop, not a die_at, so
...
def scan(text):
    ...
    as a live diagnostic site until 2026-09-04 (src/tychoc.c:3689).
    """
    out, i, n = [], 0, len(text)
    while i < n:
        c = text[i]
...
def literals(text, start):
    ...
    the hex and binary literal rules are written at src/tychoc.c:597) puts two
