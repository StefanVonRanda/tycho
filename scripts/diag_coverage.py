...
# language -- src/tychoc.c:3108-3111 turns every call into `map_set was removed;
...
# src/tychoc.c:2625, which dominates all six. Two of the six say so in their own
...
# src/tychoc.c:3721 is the fall-through of parse_value_ctrl, and all FIVE of its
# call sites (src/tychoc.c:4017, :4138, :4152, :4161, :4183) are inside an
...
DEAD = ["reserve only supports arrays of scalars",
        ...
        # anchored on the tail: src/tychoc.c:3064's LIVE `Err() carries no value
...
# `cannot find the corelib for import` (src/tychoc.c:5351) reports on the
