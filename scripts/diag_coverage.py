...
Dedup: a rule is one FORMAT STRING. `eat()` at src/tychoc.c:2071 emits
...
Two limits this cannot see, and both make the answer OPTIMISTIC:
  ...
  * "expected %s" at src/tychoc.c:2071 is ONE rule here and 105 distinct `what`
...
# language -- src/tychoc.c:3064-3067 turns every call into `map_set was removed;
...
# `if (!is_array(arrt) && !is_map(arrt))` at src/tychoc.c:7153 dies, and
# src/tychoc.c:7163 asks the identical question with `arrt` never reassigned in
...
# parameter is PARSED (src/tychoc.c:4376, the only place a param's type is
...
# (src/tychoc.c:9188) on the same substituted type. `inout Channel(int)` and a
...
# underlying-type rule (src/tychoc.c:4899), which admits only
...
# src/tychoc.c:9435 is NOT dead -- `[$N]int` is an array, so it passes that rule
# -- and has a fixture.
...
# and CLEARED at parse_type_inner's entry (src/tychoc.c:2388), so the permission
...
# src/tychoc.c:2581, which dominates all six. Two of the six say so in their own
...
# src/tychoc.c:3620 is the fall-through of parse_value_ctrl, and all FIVE of its
# call sites (src/tychoc.c:3866, :4138, :4152, :4161, :4183) are inside an
...
# than by argument. `cannot infer the type of None` (src/tychoc.c:8507) is the
# SELF-DEFEATING GUARD shape: T_NONE is produced at exactly ONE site
# (src/tychoc.c:6118, `case E_NONE`), and the untyped-decl arm eighteen lines
# above the guard (src/tychoc.c:8490) already diverts every `s->expr->kind ==
...
# `a counting `for` needs int bounds` (src/tychoc.c:8895) has three S_FORRANGE
# producers (src/tychoc.c:4058, :4069, :4084) and no fourth: the first two are
...
# `a spawned task must be bound and waited` (src/tychoc.c:8979) needs an
...
DEAD = ["reserve only supports arrays of scalars",
        ...
        # anchored on the tail: src/tychoc.c:3020's LIVE `Err() carries no value
        ...
        # E_INDEX whose base is not composite/soa/map. src/tychoc.c:6288 NORMALISES
        ...
        # src/tychoc.c:6884 is dominated by the IS_TASK test one line above it.
        # `task_of` is called at exactly ONE site, src/tychoc.c:6062 (the E_SPAWN
...
# `cannot find the corelib for import` (src/tychoc.c:5097) reports on the
...
# (src/tychoc.c:11095) is a compiler-bug assertion, not a rule about a program.
# E_SPREAD has exactly two dispositions -- the variadic call arm UNWRAPS it
# (src/tychoc.c:7194 takes args[nfixed]->lhs, so no E_SPREAD node survives), and
# every other position dies at src/tychoc.c:6039. Four spread positions probed
