...
step "[3/13] make fixpoint  (self-host B==C + packages + standalone driver)"
make -s fixpoint

step "[4/13] make corelib  (corelib packages + examples + the site dogfood: C compiler vs tychoc0 + goldens)"
make -s corelib
make -s corelib-examples
make -s site

step "[5/13] make conc  (spawn/parallel-for/channels: ASan+TSan + tychoc0 parity)"
make -s conc

step "[6/13] make ffi  (extern fn: both compilers vs golden, ASan-clean)"
make -s ffi

if [ "$N" -gt 0 ]; then
    step "[7/13] make fuzz N=$N  (differential tychoc vs tychoc0 + ASan/UBSan)"
    python3 fuzz/run.py "$N"
    step "[8/13] make fuzz-reject N=$N  (malformed input: both compilers must fail closed)"
    ...
    step "[9/13] make fuzz-leak N=$LN  (LeakSanitizer: arena / owner-0 leaks)"
    python3 fuzz/run_leak.py "$LN"
else
    step "[7/13] fuzz lanes skipped (N=0)"
fi

step "[10/13] make tools-check  (formatter idempotence + semantic preservation + LSP smoke)"
sh scripts/tools_check.sh

step "[11/13] make typeparity  (binary-op operand types: tychoc and tychoc0 must agree on accept/reject)"
make -s typeparity

step "[12/13] make parforparity  (parallel-for body gates: tychoc and tychoc0 must agree on accept/reject)"
make -s parforparity

step "[13/13] bench-guard  (tree-alloc wall: tycho must beat C -- perf regression gate)"
sh bench/guard.sh
