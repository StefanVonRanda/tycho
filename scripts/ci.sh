...
step "[1d2/13] make friction-check-light  (the closed entries in docs/internals/FRICTION.md are claims about the tree, and they rot -- #58 said the uuid entropy caveat was on \"both\" surfaces while the package carried no warning at all. Each closed entry carries a Pinned-by: line holding a shell command and this RUNS them, so a fix that stops holding names its own entry. It was enforced only when somebody typed it: the full target existed and NOTHING called it, which is the state surface-check above was rescued from, and eight pins added on 2026-09-19 would never have run unattended (FRICTION 104). This lane is the FACT pins only -- every grep, test -f and inline python3 -c -- and deliberately not the make/sh-scripts ones, which re-run suites this gate has just finished: the full run costs +113s against a ~290s ci for coverage ci largely has. The summary names how many entries and suite pins it DEFERRED, because a subset that does not say what it omitted reads exactly like the whole. NOTE: no backticks in this string -- it is double-quoted, so a backtick here is a COMMAND SUBSTITUTION that runs during the sweep)"
make -s friction-check-light

step "[1d3/13] make corpus-check  (the corpus COUNTS the suites assert -- compiler/run.sh n_acc and leg1/leg1c, verdict_diff EXPECT, the leg15 refused and accepted totals, the SYNTAX/NAME/TYPE/SEMANTIC split and the classified reject rows. Every one is a hand-written literal, and adding a fixture moves the tree out from under all of them at once. This is the sub-second predictor of make parse-check opening counts, by its own Makefile comment -- and parse-check is in NEITHER this sweep nor the pre-push hook, so on 2026-09-19 a single fixture added to close FRICTION 94 left it RED for a day while five commits each recorded a green sweep, truthfully (FRICTION 106). 0.04s. It cannot see what parse-check sees; it sees the one thing that broke it, for nothing)"
make -s corpus-check

step "[1d4/13] make builtin-qualified  (a QUALIFIED builtin -- strings.len(...) -- must name the builtin and the cure, in BOTH compilers, for every builtin the freeze records. src/tychoc.c decided this with sig_find(), which has no entry for a generic builtin, so strings.len died as \"package strings has no symbol len\", pointing the reader at a symbol that was never going to be there while tychoc1 said the useful thing. The two disagreed for a day and nothing noticed, because the gate that scores them is make parse-check and nothing runs it (FRICTION 106, 107). The fix put a fourth builtin list in src/tychoc.c and a list nothing checks is the same defect waiting, so this reads the set out of surface.lock and requires both compilers to name every one -- a builtin added without being listed reddens here. The control requires a NON-builtin to still get has-no-symbol, without which the script would pass by answering every input the same way)"
make -s builtin-qualified

step "[1d5/13] make parity-fuzz  (three DIFFERENTIAL fuzzers that generate cases, compile them with both compilers and compare accept/reject plus the emitted C against an oracle: eq-parity over composite and newtype equality, unary-parity over the unary operators, parfor-parity over the parallel-for gate. docs/spec/appendix-e-conformance.md cites all three as LANES -- they are the spec's stated evidence for sections 5.5, 9.3, 13.3 and 22 -- and until 2026-09-19 nothing ran them, so the normative conformance table pointed at files no gate executed (FRICTION 119). They pass: 512, 30 and 25 cases. Total 6s, which is why there was never a cost argument for their absence; there was simply never a target. typeparity (65s) and run_pkg.py (143s) stay out as a budget decision, recorded rather than taken)"
make -s parity-fuzz

step "[1d6/13] make probe-selfcheck  (the probe runner's two controls: a WEDGED process must be detected as quiet and a WORKING one must be left alone. ROADMAP.md 1 -- a real program by someone who is not the author -- is served by scripts/probe_run.sh, and both of its quiescence signals were spelled for Linux only: /proc for CPU time and GNU find -printf for the last write. On macOS neither errors, both go CONSTANT, and a constant is a zero delta -- so the watchdog satisfied both halves on its first window, killed the agent after QUIET seconds and reported a clean exit. Every probe run on a Mac would have been truncated at five minutes (FRICTION 122). --selfcheck failed on the parent commit here and was attached to no verdict, which is the shape of 104 and 106 a third time. 9.2s, so the cost argument that kept it out does not survive being measured)"
make -s probe-selfcheck
...
step "[2e2/13] make glibc-check  (the libc FLOOR of a program this tree builds. The driver sets no -std, so on a C23-default host glibc redirects strtol to __isoc23_strtol@GLIBC_2.38 and every Tycho binary stops starting on Rocky 9, Ubuntu 22.04 and Debian 12 -- while building, running and matching its golden here. Third shipped artifact in this tree to carry a floor nobody checked)"
make -s glibc-check

step "[2f/13] make release-content  (what is INSIDE the release archives, not whether release.sh exited 0. make release-check compares two builds byte for byte and asserts nothing about their contents, which is why an archive shipping the BOOTSTRAP compiler, four .exe files that could not start -- -pthread with no -static, importing a libwinpthread-1.dll nothing carried, wine exit 53 -- and no runtime/ directory was invisible to every lane here. Each archive is extracted and read: layout, runtime/tycho_rt.c cmp-identical to the repo's, the version the packaged compiler reports, every imported DLL either a Windows system one or a file in the archive, every .exe actually starting under wine, the emitted C byte-identical to ./tychoc1's for a program the two compilers DISAGREE on -- the disagreement asserted first, or the identity leg is decoration -- and a core:strings program emitted, linked and RUN with corelib found beside the binary and no TYCHO_CORELIB. Its selfcheck rebuilds all three historical defects for real and requires each to redden the leg that names it) EACH LEG NOW STATES ITS OWN PREREQUISITES: until 2026-09-19 this script exited at the first missing Windows tool BEFORE it checked the native archive, so on every Mac and most Linux boxes the native tarball got zero content checks while the lane printed a tidy SKIP -- which is the shape the first macOS artifact would have shipped in (FRICTION 109). The native leg needs nothing a build host lacks and now always runs; the mingw leg skips by name. On Darwin the glibc-floor leg is replaced rather than skipped, because a Mach-O binary has no versioned glibc symbol and the old leg congratulated it for being statically linked -- macOS has no static libc. Its three legs check the architecture the archive name promises, that nothing outside /usr/lib and /System is linked (the Homebrew trap: works everywhere except off the build box), and the minimum macOS the binary reports"
...
step "[3u7b/27] sh scripts/handle_guard.sh  (a double free of a core:crypto / core:tls / core:http / core:image handle was a SIGSEGV and a use-after-free returned freed memory: those handles are a bare ptr, which is not affine, so the compiler cannot see either. Nothing else here can redden for it -- make corelib runs only CORRECT programs and a golden is byte-identical with the guard gone. Three legs per package from one probe: the legitimate open/use/free must still work, a second free must die naming the handle, and a use after the free must too. Two more per package build the same probe against a COPY of corelib with the guard condition replaced by an unconditional return, which must lose the diagnosis, and against a pristine copy, which must get it back)"
...
# tycho-du: the first handle program built the way the FFI docs tell you to
# build one, and the only leak proof that runs through an error unwind.
step "[4c/35] make du-check  (tycho-du: a disk-usage walker over POSIX opendir/readdir/lstat whose open directory stream is a typed handle. The BUILD is a leg -- it is the only program in the tree combining a handle with --shim, the pair docs/reference/ffi.md recommends, while tools/tycho-fh hand-builds a static library instead, so that spelling was demonstrated nowhere until this landed (the 2026-09-21 probe filed it as finding 12). Two runs identical and equal to the golden; 351 B in 4 files over 3 directories against literals rather than against the host; 200 strict walks of a clean tree open and close 600 streams and end live=0; 200 more over a chmod 000 directory error 200 times, each unwinding through or_return out of a frame that STILL OWNS its handle, and still end live=0 with opens==closes -- which is the claim the feature exists to make and the one no golden can show. The unreadable directory is counted as skipped rather than dropped, which is the distinction list_dir cannot make and the reason the shim earns its place. An unbound opener, a free: naming an undeclared function and a destructor declared at the wrong type are all refused with a real shim behind them, pinning FRICTION 123 and 126 against the program that would leak if they stopped holding. Both root-only legs state their skip)"
make -s du-check
