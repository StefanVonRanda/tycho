...
step "[1a/13] make check-links  (every relative Markdown link resolves to a real file)"
make -s check-links

step "[1a2/13] make script-check  (.py/.sh parse, no unreachable code). It ran in the pre-push hook and NOT here, which meant a script edited on a branch that was never pushed through the hook -- or pushed with --no-verify -- reached main unparsed. It reached this sweep only as a pin inside friction-check-light, scored as a side effect of a doc gate; when that gate went, the coverage would have gone with it silently)"
make -s script-check
