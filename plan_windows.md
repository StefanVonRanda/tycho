...
- The **compiler** (`src/tychoc.c`) is mostly portable C. Its POSIX surface is
  `dirent` (opendir/readdir/closedir, `src/tychoc.c:4723@opendir`), `popen`
  (`src/tychoc.c:13755@popen`), `realpath`, `access`, `vasprintf`
  (`src/tychoc.c:155@vasprintf`), and `newlocale/uselocale`
  (`src/tychoc.c:250@uselocale`). mingw-w64 provides no POSIX
