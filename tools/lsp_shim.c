/* Companion C for tools/lsp.hi (the hier language server), linked via
 * `hierc --shim`. LSP speaks JSON-RPC over stdin/stdout with Content-Length
 ...
 * hier's read_all()/print() can do. These wrap stdio through our own symbols
 * (no clash with the preamble's <stdio.h> decls). Signatures match hier's extern
 * emission: hier `int` == C `long`, hier `string` == `char*`; a returned char*
 * is arena-copied into a hier string by the FFI, so static/realloc buffers are
