...
former: its return buffer is `static __thread char *g_out` (`corelib/crypto/crypto_shim.c:14@g_out`),
...
pipeline) are in [the concurrency design note](../reference/concurrency.md); the test suite runs golden
output under ASan/LSan **and** ThreadSanitizer, as part of `make conc`.
