...
| F8 | stdlib not guessable / no `sort(xs)` | **Fixed, both compilers** — (1) added `sort.asc(xs)`/`sort.desc(xs)`: generic over any `comparable` element (int/float/string/char), stable, returns a new array — the dead-simple "sort these values" case. (2) The `strings.to_upper` "no symbol" was **not** a broken export (audit: every `core:strings` fn exports correctly); it was a package-less file silently dropping its `import`s. Both compilers now reject a file that `import`s without its own `package` decl, pointing at the fix (`add package main`) instead of the misleading "no symbol". Goldens `corelib/test/sort`, `tests/diag/import_no_package`. |
| F9/F10 | `while` / int-float mix cryptic errors | **Fixed (tychoc)** — F9: a bare `while cond:` now says *Tycho has no `while` — use `for cond:`*. F10: the int/float arithmetic error now adds *convert one side, e.g. `to_float(x)` … or `to_int(x)`*. Diagnostics-only (both compilers already reject; tychoc0's messages stay simpler by design). Goldens `tests/diag/while_keyword`, `tests/diag/int_float_mix`. |
