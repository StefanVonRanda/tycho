...
> **Overtaken, 2026-07-30 (head `53dd937`).** The counts in this section are the
...
>
> **Still not it, 2026-07-31 (head `3ddc8fd`).** That open list has since been re-scored
...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:3641`, unmoved since the previous pass), which still never states the
...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8135`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4629-4630`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:5764`, `src/tychoc.c:5773`,
   `src/tychoc.c:5792`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8135`; the open
   ...
   (`src/tychoc.c:8129`) consults the same two tables plus `generic_find`, so whatever is
...
5. ~~**Stale in-tree comments asserting constraints that the freeze retirement killed**~~
   ...
   **One claim was disproved while rewriting it**: `corelib/test/result/main.ty` says
   ...
   (*phase 10, widened at the previous pass*) — **all six sites re-read at `3ddc8fd` and
   ...
   `corelib/test/io/main.ty:51-52` and `corelib/test/result/main.ty:74-78` show the
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:10891@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5066@ncpu`, lowering at `src/tychoc.c:9935@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10185`, inside `src/tychoc.c@gen_parfor`) —
...
8. ~~**No direct spelling for N workers** (*Earlier phases*)~~ — **CLOSED
   ...
   **COSTED 2026-08-13, and half of it was never a type-system change:** The channel-drain form already lowers to
   ...
   `src/tychoc.c:731` and `server/main.ty:499-501` at the previous pass; the first drifted
...
10. ~~**This file's own coordinates drift silently, and no gate can see it**~~
    ...
    **11 of the 15 no longer point at their subject.** All eleven are into
    ...
    `corelib/httpd/httpd.ty:229`, `server/main.ty:564`, `runtime/tycho_rt.c:541`) — which
    ...
    **NARROWED 2026-07-31 at `3ddc8fd`, and the narrowing is a decision the tree has now
...
11. ~~**NEW — `docs/spec/14-ffi.md` §24.1.1 documents one of the two ways a shim returns a
    ...
    as `path@SYMBOL` refs — the old block's `src/tychoc.c:11098-11127` for
    ...
    (`corelib/io/io.ty:38`, `corelib/io/io.ty:44`) put the *status* in the return and the
    ...
    **The rule is not a preference, and that is why it belongs in the spec.** Which slot
    ...
    - A `bytes` or array payload **cannot** be an `inout` — the compiler rejects it in as
      ...
      `src/tychoc.c:3947`), because the crossable `inout` set is int/char/float/bool/ptr.
    ...
    **This is the same failure §24.1.1 was written to fix, one turn later.** That section
    ...
    one shim's comments into another's with no spec to copy. `corelib/io/io.ty:24-43` is
...
12. ~~**NEW — `cli.has` answers a narrower question than its name, and no diagnostic is
    ...
    `3ddc8fd`: unchanged, and no `supplied` exists. **A decision, not lines** — rename to
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10185`.
...
- **Phase 1** — `parallel for i in 0..<N` runs only `min(N, tycho_ncpu())` iterations concurrently (`runtime/tycho_rt.c:915-934`); iterations chunked behind one that never returns never start, and nothing warns. `TYCHO_THREADS=2` silently cut a 4-worker server to 2.
...
- ~~**Phase 2** — `to_bytes("")` is the only spelling for an empty `bytes`; there is no `bytes` literal and no zero value, so every struct default and early return carries the call.~~ **REFUSED with the number, the loops-cleanup plan — settled, not fixed, and the measurement is what settled it.** The premise that this costs anything at run time is **false**: `T_BYTES` lowers to `char *`, "the same length-headered buffer as string" (`src/tychoc.c:1292`), and `to_bytes` on a string is a **zero-cost reinterpret**, not a conversion (`:9381-9382`) — so `to_bytes("")` emits the same interned `""` a literal would, and the whole complaint is 11 characters of spelling at **10 sites in the entire tree** (4 in `corelib/test/`, 2 in `examples/corelib/result`, 1 each in `corelib/httpd/httpd.ty:121`, `corelib/image/image.ty:17`, `corelib/test/compress`, `server/main.ty`). Two candidate spellings measured against the current compiler: `b: bytes = ""` → `error: declared type bytes but value is string`; `b: bytes = []` → `error: cannot type a bare [] here -- no expected type`. The cheapest landing is a checking-mode arm in `resolve_exp` grounding a string *literal* against an expected `T_BYTES` — **~6 code lines**, the same shape as phase 3's 11-line `E_TUPLE` arm, needing no codegen and no runtime because the representation is already identical. Refused on three counts: it puts an **implicit string→bytes conversion into the type system** (a language change, ~25 lines of spec text in `04-inference.md` §6.1 / `03-types.md` / Appendix E, and a redesign this plan's Anti-scope forbids); **it cannot be used at the site this entry names** — "every struct default" is `corelib/httpd/httpd.ty:121`, and `core:httpd` is compiled by the frozen `compiler/tychoc0.ty` through `examples/webserver/run.sh` (fourth phase running that this constraint has bound); and a **real** `bytes` literal, byte-exact and `\xNN`-capable, is already costed at ~35 lines across 3 functions plus a new runtime entry point in the `\r` item above, where it belongs to the `bytes`-operators phase. **Net: 0 lines, 0 bytes of emitted code, one number written down.**
...
- **`src/tychoc.c:3398`** points at `gen_parfor` 98 lines short of where it is.
...
generated C by the compiler — `src/tychoc.c:10467-10469` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:10448-10450` for the SoA variant, both spelling it
...
at `src/tychoc.c:5061@eprint`, emitted as `tycho_eprint`, and defined as
...
> ```
>
> — `src/tychoc.c:5586-5588`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5074@.name="split"`, so it is in no package at all.
...
emitted — `src/tychoc.c:12371` skips any function type mentioning a type
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:6876@element-wise` and
`src/tychoc.c:6931@element-wise`, anchored per this file's header rule). The
...
The audit picked its subject the way the previous four did — by measuring what
the tree does not exercise. `grep -F` found a variadic parameter (`...T`)
DECLARED in exactly three files, all under `tests/`, and `zero$(` in three
fixtures plus `compiler/tychoc0.ty`, the frozen compiler no gate builds. Both
are language features whose only caller was their own fixture. Writing the first
