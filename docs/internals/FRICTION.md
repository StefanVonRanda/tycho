...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:4180`, unmoved since the previous pass), which still never states the
...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:9309`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:5334-5335`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:6734`, `src/tychoc.c:6743`,
   `src/tychoc.c:6762`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:9309`; the open
   ...
   (`src/tychoc.c:9303`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:12336@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5913@ncpu`, lowering at `src/tychoc.c:11303@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:11509`, inside `src/tychoc.c@gen_parfor`) —
...
8. ~~**No direct spelling for N workers** (*Earlier phases*)~~ — **CLOSED
   ...
   **NARROWED, 2026-07-31, and the item reads stronger than it is** (the prunner plan).
   ...
   (`docs/spec/02-grammar.md:255-256`) has no width slot in the grammar, so the only knob
   ...
   **Re-checked at `3ddc8fd` and the narrowed form holds in both halves.** The grammar
   still has no width slot — `docs/spec/02-grammar.md:255-256` is two productions, a
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:11509`.
...
- **STILL OPEN, narrowed — there is no general handler, and "narrow" is the design and
  ...
  (`runtime/tycho_rt.c:1108@mu`) — a handler that interrupts the allocator or the lock
...
- **`src/tychoc.c:3887`** points at `gen_parfor` 98 lines short of where it is.
...
generated C by the compiler — `src/tychoc.c:11830-11832` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:11809-11811` for the SoA variant, both spelling it
...
at `src/tychoc.c:5908@eprint`, emitted as `tycho_eprint`, and defined as
...
- **A match arm cannot be empty.** An arm with no body is `error: expected an
  ...
  wildcard is at `docs/spec/12-aggregates.md:813`. An unverified absence claim
...
> ```
>
> — `src/tychoc.c:6524-6526`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5921@.name="split"`, so it is in no package at all.
...
`docs/spec/12-aggregates.md:991`. That is the same failure as a stale
...
emitted — `src/tychoc.c:13817` skips any function type mentioning a type
...
- `Task(T)` is affine and non-storable (`docs/spec/03-types.md:400`), so
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:8065@element-wise` and
`src/tychoc.c:8120@element-wise`, anchored per this file's header rule). The
...
## Found by pinning the instantiation note, 2026-09-11 (head `44b02447`)

### 87. ~~`tychoc` and `tychoc1` report a different number of instantiation frames~~ — **FIXED 2026-09-11: tychoc now walks the chain (the owner's call)**

> Pinned-by: make test

**Decided and fixed the same day.** The choice was the owner's, because the two
directions were opposite edits to a user-facing diagnostic: teach `tychoc` to
walk the chain, or cap `tychoc1` at one frame. **Option A was taken** — the
outermost frame is the only line in a nested failure that the caller actually
wrote, and it was exactly the one the single-frame form dropped.

`tests/diag/generic_instantiation_note.ty` still pins the one-frame shape (the
common case, diagnosed directly rather than through a nested one), and
`tests/diag/generic_instantiation_chain.ty` now pins the nested form that could
not be written while the two disagreed.
...
`tests/diag/` golden with `${TYCHOC:-./tychoc1}` (`tests/run.sh:175@TYCHOC`),
...
its parent when it is interned), and `gen_program`'s resolve loop — where
`g_ginsts` IS in scope — builds the outward frame list into globals that a
diagnostic copies when it is raised. Capped at 8 frames.

**The predicted golden churn did not happen.** The sizing called this a
corpus-wide change wanting its own commit; `make test` came back 1039/0, the
1039th being the new fixture. No existing fixture's diagnostic ran through a
nested generic, so nothing else moved. Worth recording: the cost estimate was
the right shape and the wrong size, and only running it settled that.

**The original sizing, kept for the record.** The divergence is structural rather than a
...
(`src/tychoc.c:13826@g_inst_from`). So it can only ever name the innermost
...
## Found by running `make ci` on a box that had never run it, 2026-09-11 (head `3310a50f`)

### 88. ~~`make server-check` fails inside `make ci` and passes alone~~ — **FIXED 2026-09-11: the watchdog inherits the cleanup trap and is cancelled with TERM**

> Pinned-by: make server-check

**THE CAUSE, found by kernel-level signal tracing after four rounds of wrong
hypotheses.** `server/run.sh` is not the victim of an outside signal — **it is
the sender, and it kills itself by proxy**:

1. Its watchdogs are `( sleep N; srv_kill ) &` (`server/run.sh:686@sleep`).
   That is a **two-command** subshell, so bash keeps a real subshell — and a
   real subshell **inherits `trap cleanup EXIT INT TERM`** and `$T`.
2. It cancels a watchdog with `kill "$WD"` — the **default SIGTERM**.
3. The watchdog runs the inherited `cleanup`: `rm -rf "$T"`, on the directory
   the parent is still using.
4. Nothing ever signals the parent, so it carries on and every later step fails
   with "No such file or directory".

A `bpftrace` probe on `sys_enter_kill` (not `strace`, which cannot be used here
— it is ptrace-based and every ASan fixture fails under it, taking `make test`
down at lane 2) shows it plainly: **no signal anywhere in the run targets
`server/run.sh`'s pid**, while that pid sends 57 TERMs to 57 distinct children
in eight seconds, and each child then TERMs *itself* — the signature of a shell
whose trap handler ran and re-raised.

**Why it hid for four rounds, which is the lesson worth keeping.** The minimal
reproduction is wrong in a way that passes: `( sleep 30 ) &` is a **one-command**
subshell, and bash **execs it in place**, so there is no subshell and no trap.
Measured both ways in one script:

```text
A) ( sleep 30 ) &      -> marker SURVIVED
B) ( sleep 30; : ) &   -> CLEANUP RAN in the subshell; marker GONE
```

Every earlier attempt to reproduce used shape A and concluded, wrongly, that
subshells do not inherit the trap. They do — when they are real subshells.

**The fix: `kill -KILL "$WD"` at all five cancel sites.** SIGKILL cannot be
trapped. `trap - INT TERM` inside the subshell is **not** a fix — measured: the
kill can arrive before the subshell has executed the reset — and a
fire-and-forget timer has nothing to clean up regardless.

**Two hypotheses this replaces, both recorded above and both wrong:** an
external signal to the shared process group, and a stale PID recycled under
load. Neither survived the trace.
...
- *The EXIT trap firing in the watchdog subshell* (`server/run.sh:686@sleep`,
  `:14@trap`). bash does not run a parent's EXIT trap when a `( ... ) &`
  subshell exits; probed directly.
- *The TERM trap firing when the watchdog is killed* (`server/run.sh:691@kill`,
...
- *One earlier theory that was simply wrong*: the first occurrence followed a
  `make ci` that had been killed mid-flight, and `3310a50f`'s message records it
...
## Found by running `make ci` under clang, 2026-09-11 (head `1229adb1`)
...
tree already uses `__attribute__` (`runtime/tycho_rt.c:237@constructor`,
`:887@noinline`), and the Windows build is mingw, which is gcc, so no target
...
## Found by inspecting the whole locked surface, 2026-09-19 (head `2e561727`)
...
(`src/tychoc.c:5932@floor`) — it simply had no fixture.
...
> **Closed by options (1) AND (2) below, both in `f6f1025a`.** The extractor
...
## Found by running `make ci` on macOS / arm64 for the first time, 2026-09-19 (head `602aea34`)
...
**It went red today.** `tests/*.ty` crossed 288 → 289 at `f0078c52`, the commit
...
**(1) and (2) are done; (3) and (4) are left.**

**The drift is cleared.** Seven literals, not the three the first pass counted —
each one only became visible once the one before it was fixed, which is its own
small lesson about a gate nobody runs:

| literal | was | is |
|---|--:|--:|
| `run.sh` n_acc / leg1 (`tests/*.ty`) | 288 | **289** |
| `verdict_diff.py` EXPECT (whole tree) | 1375 | **1401** |
| `run.sh` leg15 refused | 51 | **52** |
| `run.sh` TYPE / SEMANTIC split | 166 / 273 | **175 / 288** |
| `run.sh` `--typecheck` refused | 593 | **617** |

plus `compiler/reject_class.tsv` regenerated by `scripts/classify_rejects.py`
(593 → 617 rows).

**And the three censuses were stale, which is the more interesting half.** The
AST, RESOLUTION and TYPE censuses are the legs that exist because *an
accept/reject verdict is blind to a parse that succeeds with the WRONG tree*.
They had not been recorded since `7b12d0ee`, and `f6f1025a` — the `or_return`
migration, 34 sites, -69 net lines of `.ty` — never touched them.

They were **not** re-recorded on the grounds that the corpus grew. Blessing a
census because the tree got bigger is exactly the mechanical pass that certifies
a regression. The full per-kind delta was computed first, and every one of the 32
moved kinds is accounted for:

| kind | delta | why |
|---|--:|---|
| `e.OrRet` | **+33** | the migrated sites (the 34th is inside a doc comment) |
| `s.MatchS` | −29 | the `match` blocks they replaced |
| `a.variant` / `a.bind` | −57 / −45 | their `Ok(x)`/`Err(e)` arms and bindings |
| `e.ErrE` / `s.ReturnS` / `s.PassS` | −29 / −30 / −11 | the `return Err(e)` bodies and `Ok: pass` arms |
| `s.ExprStmt` / `e.Call` | +20 / +20 | the `f() or_return` statements now in their place |
| `d.fn`, `d.struct`, `d.field`, `s.IfS`, … | +1 each | today's new fixtures |

Every decrease is the migration; every increase is a fixture. **Nothing is
unexplained**, which is the only basis on which a census golden should move.

**`corpus-check` is now lane `[1d3/13]`**, at **0.04s**. It cannot see what
