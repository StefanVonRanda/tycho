...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:3706`, unmoved since the previous pass), which still never states the
...
4. **The `send` collision** (*Phase 7*) — reproduced again at `9e8f8f2` with two scratch
   ...
   (`src/tychoc.c:8097`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4661-4662`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:5773`, `src/tychoc.c:5782`,
   `src/tychoc.c:5801`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8097`; the open
   ...
   (`src/tychoc.c:8091`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11013@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:4659`, lowering at `src/tychoc.c:9400`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10315`, inside `src/tychoc.c@gen_parfor`) —
...
8. **No direct spelling for N workers** (*Earlier phases*) — reproduced again at
   ...
   `src/tychoc.c:686` and `server/main.ty:499-501` at the previous pass; the first drifted
...
11. ~~**NEW — `docs/spec/14-ffi.md` §24.1.1 documents one of the two ways a shim returns a
    ...
    as `path@SYMBOL` refs — the old block's `src/tychoc.c:11228-11257` for
    ...
    **The rule is not a preference, and that is why it belongs in the spec.** Which slot
    ...
    - A `bytes` or array payload **cannot** be an `inout` — the compiler rejects it in as
      ...
      `src/tychoc.c:4046`), because the crossable `inout` set is int/char/float/bool/ptr.
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10315`.
...
- **`src/tychoc.c:3460`** points at `gen_parfor` 98 lines short of where it is.
...
at `src/tychoc.c:5084@eprint`, emitted as `tycho_eprint`, and defined as
...
> ```
>
> — `src/tychoc.c:5610-5612`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
