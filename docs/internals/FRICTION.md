...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8502`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   ...
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8502`; the open
   ...
   (`src/tychoc.c:8496`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11315@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5336@ncpu`, lowering at `src/tychoc.c:10359@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10609`, inside `src/tychoc.c@gen_parfor`) —
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10609`.
...
generated C by the compiler — `src/tychoc.c:10891-10893` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:10872-10874` for the SoA variant, both spelling it
...
emitted — `src/tychoc.c:12795` skips any function type mentioning a type
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:7232@element-wise` and
`src/tychoc.c:7287@element-wise`, anchored per this file's header rule). The
