...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8385`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   ...
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8385`; the open
   ...
   (`src/tychoc.c:8379`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11340@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5304@ncpu`, lowering at `src/tychoc.c:10365@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10642`, inside `src/tychoc.c@gen_parfor`) —
...
11. ~~**NEW — `docs/spec/14-ffi.md` §24.1.1 documents one of the two ways a shim returns a
    ...
    as `path@SYMBOL` refs — the old block's `src/tychoc.c:11555-11584` for
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10642`.
...
generated C by the compiler — `src/tychoc.c:10924-10926` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:10905-10907` for the SoA variant, both spelling it
...
emitted — `src/tychoc.c:12891` skips any function type mentioning a type
