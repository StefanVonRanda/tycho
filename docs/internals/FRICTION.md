...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8560`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4960-4961`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:6119`, `src/tychoc.c:6128`,
   `src/tychoc.c:6147`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8560`; the open
   ...
   (`src/tychoc.c:8554`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11541@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5408@ncpu`, lowering at `src/tychoc.c:10554@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10831`, inside `src/tychoc.c@gen_parfor`) —
...
11. ~~**NEW — `docs/spec/14-ffi.md` §24.1.1 documents one of the two ways a shim returns a
    ...
    as `path@SYMBOL` refs — the old block's `src/tychoc.c:11756-11785` for
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10831`.
...
generated C by the compiler — `src/tychoc.c:11113-11115` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:11094-11096` for the SoA variant, both spelling it
...
at `src/tychoc.c:5403@eprint`, emitted as `tycho_eprint`, and defined as
...
> ```
>
> — `src/tychoc.c:5941-5943`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5416@.name="split"`, so it is in no package at all.
...
emitted — `src/tychoc.c:13092` skips any function type mentioning a type
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:7234@element-wise` and
`src/tychoc.c:7289@element-wise`, anchored per this file's header rule). The
