...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:3742`, unmoved since the previous pass), which still never states the
...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8391`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4735-4736`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:5993`, `src/tychoc.c:6002`,
   `src/tychoc.c:6021`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8391`; the open
   ...
   (`src/tychoc.c:8385`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11204@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5233@ncpu`, lowering at `src/tychoc.c:10248@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10498`, inside `src/tychoc.c@gen_parfor`) —
...
8. ~~**No direct spelling for N workers** (*Earlier phases*)~~ — **CLOSED
   ...
   **COSTED 2026-08-13, and half of it was never a type-system change:** The channel-drain form already lowers to
   ...
   `src/tychoc.c:775` and `server/main.ty:495-497` at the previous pass; the first drifted
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10498`.
...
- **`src/tychoc.c:3499`** points at `gen_parfor` 98 lines short of where it is.
...
generated C by the compiler — `src/tychoc.c:10780-10782` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:10761-10763` for the SoA variant, both spelling it
...
at `src/tychoc.c:5228@eprint`, emitted as `tycho_eprint`, and defined as
...
> ```
>
> — `src/tychoc.c:5815-5817`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5241@.name="split"`, so it is in no package at all.
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:7121@element-wise` and
`src/tychoc.c:7176@element-wise`, anchored per this file's header rule). The
