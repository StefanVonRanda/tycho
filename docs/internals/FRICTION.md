...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:4112`, unmoved since the previous pass), which still never states the
...
8. ~~**No direct spelling for N workers** (*Earlier phases*)~~ — **CLOSED
   ...
   **COSTED 2026-08-13, and half of it was never a type-system change:** The channel-drain form already lowers to
   ...
   `src/tychoc.c:874` and `server/main.ty:563-565` at the previous pass; the first drifted
...
10. ~~**This file's own coordinates drift silently, and no gate can see it**~~
    ...
    **11 of the 15 no longer point at their subject.** All eleven are into
    ...
    `corelib/httpd/httpd.ty:229`, `server/main.ty:628`, `runtime/tycho_rt.c:627`) — which
...
- **STILL OPEN, narrowed — there is no general handler, and "narrow" is the design and
  ...
  (`runtime/tycho_rt.c:1096@mu`) — a handler that interrupts the allocator or the lock
...
- **`src/tychoc.c:3819`** points at `gen_parfor` 98 lines short of where it is.
...
at `src/tychoc.c:5840@eprint`, emitted as `tycho_eprint`, and defined as
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5853@.name="split"`, so it is in no package at all.
...
## Found by pinning the instantiation note, 2026-09-11 (head `14867a89`)
...
`diag_flush` (`src/tychoc.c:997@diag_flush`) runs some 4,800 lines BEFORE `GInst`
...
## Found by running `make ci` on a box that had never run it, 2026-09-11 (head `c4034e25`)
...
- *One earlier theory that was simply wrong*: the first occurrence followed a
  `make ci` that had been killed mid-flight, and `c4034e25`'s message records it
...
## Found by running `make ci` under clang, 2026-09-11 (head `3753ba49`)
...
## Found by inspecting the whole locked surface, 2026-09-19 (head `d69ad5cd`)
...
> **Closed by options (1) AND (2) below, both in `d363f618`.** The extractor
...
## Found by running `make ci` on macOS / arm64 for the first time, 2026-09-19 (head `53da0158`)
...
**It went red today.** `tests/*.ty` crossed 288 → 289 at `cff8fe2b`, the commit
...
They had not been recorded since `7b12d0ee`, and `d363f618` — the `or_return`
...
would have reddened at `cff8fe2b` on the day.
