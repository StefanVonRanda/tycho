...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8731`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4949-4950`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:6276`, `src/tychoc.c:6285`,
   `src/tychoc.c:6304`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8731`; the open
   ...
   (`src/tychoc.c:8725`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11608@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5488@ncpu`, lowering at `src/tychoc.c:10634@tycho_ncpu`), so a program can at least
...
at `src/tychoc.c:5483@eprint`, emitted as `tycho_eprint`, and defined as
...
- **A match arm cannot be empty.** An arm with no body is `error: expected an
  ...
  wildcard is at `docs/spec/12-aggregates.md:690`. An unverified absence claim
...
> ```
>
> — `src/tychoc.c:6093-6095`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5496@.name="split"`, so it is in no package at all.
...
`docs/spec/12-aggregates.md:868`. That is the same failure as a stale
...
emitted — `src/tychoc.c:13105` skips any function type mentioning a type
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:7460@element-wise` and
`src/tychoc.c:7515@element-wise`, anchored per this file's header rule). The
