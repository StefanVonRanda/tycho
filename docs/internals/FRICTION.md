...
2. ~~**`spawn f(x)` as a bare statement**~~ — **CLOSED 2026-07-31: the parser now names
   ...
   (`src/tychoc.c:3864`, unmoved since the previous pass), which still never states the
...
4. ~~**The `send` collision** (*Phase 7*)~~ — **CLOSED 2026-08-13: the definition is
   ...
   (`src/tychoc.c:8525`), and `sig_find` searches `g_sigs` — which holds `die` and `exit`
   as real entries (`src/tychoc.c:4941-4942`, inside `src/tychoc.c@register_builtins`)
   but **holds no entry for `send`, `recv` or `close` at all**; those three are recognised
   ad hoc during resolution (`src/tychoc.c:6094`, `src/tychoc.c:6103`,
   `src/tychoc.c:6122`). So it is not a table that omits three rows, it is three builtins
   that were never in the table. **The code is ~1 line** at `src/tychoc.c:8525`; the open
   ...
   (`src/tychoc.c:8519`) consults the same two tables plus `generic_find`, so whatever is
...
7. ~~**`parallel for` caps concurrency at `min(N, ncpu)` and nothing warns** (*Earlier
   ...
   - **The 64-chunk ceiling is documented.** `docs/spec/13-concurrency.md:81-83` now
     ...
     provenance block, `src/tychoc.c:11506@_pk > 64`, so the gate now polices it.
   ...
   **What is left is item 8**, which already carries it: the one thing the spec fix does
   ...
   - The width is now **readable from Tycho**: `ncpu()` is a registered builtin
     (`src/tychoc.c:5389@ncpu`, lowering at `src/tychoc.c:10519@tycho_ncpu`), so a program can at least
     ask. Measured on this box: `ncpu()` → 16.
   - There was an **undocumented hard ceiling of 64 chunks** — `if (_pk < 1) _pk = 1; if
     (_pk > 64) _pk = 64;` (`src/tychoc.c:10796`, inside `src/tychoc.c@gen_parfor`) —
...
8. ~~**No direct spelling for N workers** (*Earlier phases*)~~ — **CLOSED
   ...
   **COSTED 2026-08-13, and half of it was never a type-system change:** The channel-drain form already lowers to
   ...
   `src/tychoc.c:715` and `server/main.ty:499-501` at the previous pass; the first drifted
...
11. ~~**NEW — `docs/spec/14-ffi.md` §24.1.1 documents one of the two ways a shim returns a
    ...
    as `path@SYMBOL` refs — the old block's `src/tychoc.c:11721-11750` for
    ...
    **The rule is not a preference, and that is why it belongs in the spec.** Which slot
    ...
    - A `bytes` or array payload **cannot** be an `inout` — the compiler rejects it in as
      ...
      `src/tychoc.c:4235`), because the crossable `inout` set is int/char/float/bool/ptr.
...
- **Nothing above 64 workers as a real workload.** The 64-chunk probe used synthetic 50 ms
  sleeps; the real corpus ran at `ncpu()` = 16, nowhere near `src/tychoc.c:10796`.
...
- **`src/tychoc.c:3603`** points at `gen_parfor` 98 lines short of where it is.
...
generated C by the compiler — `src/tychoc.c:11078-11080` for an ordinary array
(the path the probe above took) and the same test again at
`src/tychoc.c:11059-11061` for the SoA variant, both spelling it
...
at `src/tychoc.c:5384@eprint`, emitted as `tycho_eprint`, and defined as
...
> ```
>
> — `src/tychoc.c:5922-5924`. So the rule is "no *implicit* conversion", not
> "no conversion".
>
...
- **`core:strings` is not missing `split`; `split` is a builtin.** This was
  ...
  `src/tychoc.c:5397@.name="split"`, so it is in no package at all.
...
emitted — `src/tychoc.c:13057` skips any function type mentioning a type
...
**FIXED 2026-08-13, both sites** (`src/tychoc.c:7213@element-wise` and
`src/tychoc.c:7268@element-wise`, anchored per this file's header rule). The
...
### 45. A channel can be copied too, and it makes the compiler's own warning lie

Not fixed — recorded, because it is a different subsystem and was outside the
...
The send happens through the alias, so the warning is false. No double free was
observed, so this is an analysis defect rather than a memory-safety one — but it
is the same missing decl-path guard as #43, in the channel's switch arm rather
than the handle's (`src/tychoc.c:8263` guards only reassignment).

**Open question, deliberately not guessed at:** a `sink File` parameter compiles
and ran clean. The spec's §25 says passing a handle BORROWS it and lists what a
handle may not do, but says nothing about `sink`, so there is no way to tell
"intended" from "unguarded" without a decision. It wants an answer before it
wants code.
