...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9348-9356`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9348-9356`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3061-3064`) and resolves to the place's type
   without position validation (`src/tychoc.c:6055-6057` — "only valid as an
