...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9249-9257`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9249-9257`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   ...
   without position validation (`src/tychoc.c:5961-5963` — "only valid as an
