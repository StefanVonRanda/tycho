...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9174-9182`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9174-9182`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   ...
   without position validation (`src/tychoc.c:5937-5939` — "only valid as an
