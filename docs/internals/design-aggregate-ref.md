...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9007-9015`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9007-9015`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:2976-2979`) and resolves to the place's type
   without position validation (`src/tychoc.c:5817-5819` — "only valid as an
