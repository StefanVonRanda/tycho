...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:8913-8921`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:8913-8921`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:2841-2844`) and resolves to the place's type
   without position validation (`src/tychoc.c:5743-5745` — "only valid as an
