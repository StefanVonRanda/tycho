...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:8736-8744`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:8736-8744`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:2882-2885`) and resolves to the place's type
   without position validation (`src/tychoc.c:5619-5621` — "only valid as an
