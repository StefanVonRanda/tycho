...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9476-9484`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9476-9484`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3100-3103`) and resolves to the place's type
   without position validation (`src/tychoc.c:6147-6149` — "only valid as an
