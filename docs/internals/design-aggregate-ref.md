...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:8858-8866`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:8858-8866`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:2797-2800`) and resolves to the place's type
   without position validation (`src/tychoc.c:5699-5701` — "only valid as an
