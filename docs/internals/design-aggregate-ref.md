...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9592-9600`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9592-9600`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3085-3088`) and resolves to the place's type
   without position validation (`src/tychoc.c:6256-6258` — "only valid as an
