...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9216-9224`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9216-9224`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3004-3007`) and resolves to the place's type
   without position validation (`src/tychoc.c:5935-5937` — "only valid as an
