...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9883-9891`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9883-9891`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3183-3186`) and resolves to the place's type
   without position validation (`src/tychoc.c:6483-6485` — "only valid as an
