...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9287-9295`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9287-9295`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3040-3043`) and resolves to the place's type
