...
arena** as a hidden `_ina_` parameter (`src/tychoc.c:9380-9388`), and every
...
2. **Arena interaction.** In-place `inout` is sound because the callee's
   ...
   (`src/tychoc.c:9380-9388`); the callee's own arena holds only its
...
2. **Reject `&` outside argument position.** `&` parses as a unary `E_ADDR`
   everywhere (`src/tychoc.c:3063-3066`) and resolves to the place's type
   without position validation (`src/tychoc.c:6087-6089` — "only valid as an
