...
int** (`src/tychoc.c:5441`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7472`) already enforces
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:13186` — so GNU case
...
- **Resolve** (`src/tychoc.c:7472`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:10192`) — the switch emission (D6). The
