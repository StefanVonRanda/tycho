...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` case of `resolve_stmt` (`src/tychoc.c:8279@S_MATCH`) already enforces
...
- **Resolve** (`src/tychoc.c:8279@S_MATCH`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:11905@S_MATCH`) — the switch emission (D6). The
