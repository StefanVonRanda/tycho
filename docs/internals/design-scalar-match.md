...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7607`) already enforces
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:13368` — so GNU case
...
- **Resolve** (`src/tychoc.c:7607`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:10344`) — the switch emission (D6). The
