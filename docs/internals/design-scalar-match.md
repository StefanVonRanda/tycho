...
int** (`src/tychoc.c:5481`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7549`) already enforces
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:13293` — so GNU case
...
- **Resolve** (`src/tychoc.c:7549`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:10269`) — the switch emission (D6). The
