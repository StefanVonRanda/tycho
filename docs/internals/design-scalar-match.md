...
int** (`src/tychoc.c:5963@T_BYTES`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` case of `resolve_stmt` (`src/tychoc.c:8159@S_MATCH`) already enforces
...
- **`char`** — the same machine type as `int` (both emit as `tycho_int`,
  `src/tychoc.c:1641@T_CHAR`). Arm literals are `'a'`-spelled; the byte-value ladders
...
- **Resolve** (`src/tychoc.c:8159@S_MATCH`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:11760@S_MATCH`) — the switch emission (D6). The
