...
int** (`src/tychoc.c:6771@T_BYTES`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` case of `resolve_stmt` (`src/tychoc.c:9148@S_MATCH`) already enforces
...
- **Spec** — §14.3 `docs/spec/10-statements.md:33-61`; grammar rule
...
- **`char`** — the same machine type as `int` (both emit as `tycho_int`,
  `src/tychoc.c:1840@T_CHAR`). Arm literals are `'a'`-spelled; the byte-value ladders
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:15496@-fwrapv` — so GNU case
...
- **Resolve** (`src/tychoc.c:9148@S_MATCH`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:12920@S_MATCH`) — the switch emission (D6). The
  `S_MATCH` statement and value forms both flow through it, so the value
  form (`x := match c: ...`) works without separate work.
- **Spec** — §14.3 (`docs/spec/10-statements.md:33-61`), the statements
