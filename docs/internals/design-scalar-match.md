...
int** (`src/tychoc.c:5504`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7572`) already enforces
...
- **`char`** — the same machine type as `int` (one byte, `long` in C,
  `src/tychoc.c:751`). Arm literals are `'a'`-spelled; the byte-value ladders
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:13333` — so GNU case
...
- **Resolve** (`src/tychoc.c:7572`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:10309`) — the switch emission (D6). The
