...
int** (`src/tychoc.c:5789@T_BYTES`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` case of `resolve_stmt` (`src/tychoc.c:7932@S_MATCH`) already enforces
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:14065@-fwrapv` — so GNU case
...
- **Resolve** (`src/tychoc.c:7932@S_MATCH`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:11695@S_MATCH`) — the switch emission (D6). The
