...
int** (`src/tychoc.c:5512`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7613`) already enforces
...
- **`char`** — the same machine type as `int` (one byte, `long` in C,
  `src/tychoc.c:759`). Arm literals are `'a'`-spelled; the byte-value ladders
...
- Ranges emit as **consecutive case labels** (`case 48: case 49: ... case 57:`).
  ...
  compiles with plain `cc`, no `-std` — `src/tychoc.c:13374` — so GNU case
...
- **Resolve** (`src/tychoc.c:7613`) — a scalar subject path: subject is
...
- **Codegen** (`src/tychoc.c:10350`) — the switch emission (D6). The
