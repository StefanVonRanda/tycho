...
int** (`src/tychoc.c:5341`), so the char-position ladders all compare ints.
...
- **Parse** — `parse_match` (`src/tychoc.c@parse_match`) requires an IDENT per
  ...
  error.
- **Resolve** — the `S_MATCH` pass (`src/tychoc.c:7337`) already enforces
...
- **Resolve** (`src/tychoc.c:7337`) — a scalar subject path: subject is
