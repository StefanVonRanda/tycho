...
- The **compiler** (`src/tychoc.c`) is mostly portable C. Its POSIX surface is
  `dirent` (opendir/readdir/closedir, `src/tychoc.c:4731@opendir`), `popen`
  (`src/tychoc.c:13772@popen`), `realpath`, `access`, `vasprintf`
  (`src/tychoc.c:188@vasprintf`), and `newlocale/uselocale`
  (`src/tychoc.c:225@uselocale`). mingw-w64 provides no POSIX
