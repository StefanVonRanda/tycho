...
- The **compiler** (`src/tychoc.c`) is mostly portable C. Its POSIX surface is
  `dirent` (opendir/readdir/closedir, `src/tychoc.c:4768@opendir`), `popen`
  (`src/tychoc.c:13841@popen`), `realpath`, `access`, `vasprintf`
  (`src/tychoc.c:195@vasprintf`), and `newlocale/uselocale`
  (`src/tychoc.c:232@uselocale`). mingw-w64 provides no POSIX
...
- The **runtime** (`runtime/tycho_rt.c`, embedded verbatim into every emitted
  ...
  (`runtime/tycho_rt.c:808@pthread_create` — free under `-pthread`),
...
> `tests/run.sh:339` does, and one real port fix (`iox_set_mtime` could not touch
