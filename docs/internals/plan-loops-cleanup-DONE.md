...
- [x] **Phase 41** — **two bare source→source citations are drifted, and the
  ...
  - `scripts/asan_self.sh:38` says the generic bind vector is xmalloc'd at
    ...
    an unrelated function. The real site is `src/tychoc.c:7713@gi.binds` — off
...
from `scripts/editors_check.sh` and `scripts/ci.sh` ; `src/tychoc.c:6796@r_step`
...
- `scripts/asan_self.sh:38` said the generic bind vector is xmalloc'd at
  ...
  real site is `src/tychoc.c:7706@binds` (`gi.binds = (Type *)xmalloc(...)`),
...
- `tests/rtparity/run.py:67` cited `src/tychoc.c:10343` for the inline
  ...
  and is now `src/tychoc.c:11018@_step`. Its sibling `compiler/tychoc0.ty:9513`
...
`scripts/check_citations.py@ARCHIVED`. Fences with no `fn` are classed
...
(`src/tychoc.c:6237@lt`). Verified by running it: a `u32` shifted by an `int`
