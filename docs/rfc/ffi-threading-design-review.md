...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3149`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2884-2928`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4733-4746`), there is no
...
- Parse + type gate: `src/tychoc.c:2992` (`ffi_scalar_type`), `:2998`
  (`parse_extern_fn`), `:3017` (rejects `inout`/`inout` params), `:3019`
  (rejects composite params), `:3031` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:10109-10136`. Each
  `extern "Lib"` adds `-lLib` (`:6272` `add_link`). `--link/--shim/--pkg`
  passthrough at `:10414-10418`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:10171-10174`, `:3510-3535`, `:11055-11057`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:7251-7258`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3944`; literal at `:1894`). Three distinct
...
`src/tychoc.c:11431` for `len`, `:11522` and `:11529` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7623-7636`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9856-9859`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4834-4847`; runtime `:894-901`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4805-4900`). No unsafety
