...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2862`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2616-2655`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4289-4302`), there is no
...
- Parse + type gate: `src/tychoc.c:2616` (`ffi_scalar_type`), `:2622`
  (`parse_extern_fn`), `:2641` (rejects `inout`/`inout` params), `:2643`
  (rejects composite params), `:2655` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8937-9006`. Each
  `extern "Lib"` adds `-lLib` (`:5187` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8943-8947`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8725-8728`, `:3022-3047`, `:8994-8996`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6261-6268`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3442`; literal at `:1652`). Three distinct
...
`src/tychoc.c:10010` for `len`, `:10137` and `:10144` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6649-6660`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8735-8738`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4289-4302`; runtime `:613-620`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4346-4441`). No unsafety
