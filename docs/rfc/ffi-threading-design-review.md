...
- Link line assembled in one `cc` call: `src/tychoc.c:8868-8937`. Each
  `extern "Lib"` adds `-lLib` (`:5172` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8874-8878`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8656-8659`, `:3007-3032`, `:8925-8927`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6230-6237`, `tycho_str_from_c`, NULL→`""`).
...
`len()`/`print()`/`println()` (`src/tychoc.c:5975-5979` `is_extern_str_call`,
applied at `:6017` for `len`, `:6127` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6618-6629`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8666-8669`).
