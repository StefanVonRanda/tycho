...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2823`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2577-2616`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4219-4232`), there is no
...
- Parse + type gate: `src/tychoc.c:2577` (`ffi_scalar_type`), `:2583`
  (`parse_extern_fn`), `:2602` (rejects `inout`/`inout` params), `:2604`
...
- Link line assembled in one `cc` call: `src/tychoc.c:8716-8785`. Each
  `extern "Lib"` adds `-lLib` (`:5101` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8722-8726`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8504-8507`, `:2983-3008`, `:8773-8775`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6119-6126`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3372`; literal at `:1636`). Three distinct
...
`len()`/`print()`/`println()` (`src/tychoc.c:5864-5868` `is_extern_str_call`,
applied at `:5906` for `len`, `:6016` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6507-6517`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8514-8517`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4219-4232`; runtime `:597-604`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4276-4363`). No unsafety
