...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3018`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2753-2797`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4418-4431`), there is no
...
- Parse + type gate: `src/tychoc.c:2861` (`ffi_scalar_type`), `:2867`
  (`parse_extern_fn`), `:2886` (rejects `inout`/`inout` params), `:2888`
  (rejects composite params), `:2900` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9466-9493`. Each
  `extern "Lib"` adds `-lLib` (`:5851` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9791-9795`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9548-9551`, `:3309-3334`, `:10431-10433`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6701-6708`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3733`; literal at `:1827`). Three distinct
...
`src/tychoc.c:10754` for `len`, `:10881` and `:10888` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7062-7073`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9245-9248`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4519-4532`; runtime `:858-865`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4490-4585`). No unsafety
