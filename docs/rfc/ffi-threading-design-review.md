...
full deep-copy of captures per chunk (`src/tychoc.c:4603-4616`), there is no
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3814`; literal at `:1822`). Three distinct
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7385-7396`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9576-9579`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4704-4717`; runtime `:836-843`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4675-4770`). No unsafety
