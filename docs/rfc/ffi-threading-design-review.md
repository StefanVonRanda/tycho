...
- Link line assembled in one `cc` call: `src/tychoc.c:9176-9203`. Each
  `extern "Lib"` adds `-lLib` (`:5555` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9394-9398`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9151-9154`, `:3037-3062`, `:10034-10036`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6475-6482`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10347` for `len`, `:10474` and `:10481` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6836-6847`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8955-8958`).
