...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2763`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2531-2570`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4155-4168`), there is no
...
- Parse + type gate: `src/tychoc.c:2531` (`ffi_scalar_type`), `:2537`
  (`parse_extern_fn`), `:2556` (rejects `inout`/`inout` params), `:2558`
  (rejects composite params), `:2570` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8618-8687`. Each
  `extern "Lib"` adds `-lLib` (`:5037` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8624-8628`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8416-8419`, `:2923-2948`, `:8675-8677`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6043-6050`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3312`; literal at `:1590`). Three distinct
...
`len()`/`print()`/`println()` (`src/tychoc.c:5788-5792` `is_extern_str_call`,
applied at `:5830` for `len`, `:5940` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6420-6430`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8426-8429`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4155-4168`; runtime `:551-558`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4212-4299`). No unsafety
