...
- Link line assembled in one `cc` call: `src/tychoc.c:8818-8887`. Each
  `extern "Lib"` adds `-lLib` (`:5149` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8824-8828`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8606-8609`, `:2992-3017`, `:8875-8877`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6180-6187`, `tycho_str_from_c`, NULL→`""`).
...
`len()`/`print()`/`println()` (`src/tychoc.c:5925-5929` `is_extern_str_call`,
applied at `:5967` for `len`, `:6077` for print/println). Footguns:
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8616-8619`).
