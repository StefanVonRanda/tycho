...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2988`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2723-2767`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4388-4401`), there is no
...
- Parse + type gate: `src/tychoc.c:2831` (`ffi_scalar_type`), `:2837`
  (`parse_extern_fn`), `:2856` (rejects `inout`/`inout` params), `:2858`
  (rejects composite params), `:2870` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9420-9447`. Each
  `extern "Lib"` adds `-lLib` (`:5821` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9745-9749`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9502-9505`, `:3279-3304`, `:10385-10387`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6671-6678`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3703`; literal at `:1822`). Three distinct
...
`src/tychoc.c:10708` for `len`, `:10835` and `:10842` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7032-7043`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9199-9202`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4489-4502`; runtime `:853-860`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4460-4555`). No unsafety
