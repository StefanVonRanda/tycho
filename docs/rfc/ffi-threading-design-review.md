...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2910`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2650-2689`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4304-4317`), there is no
...
- Parse + type gate: `src/tychoc.c:2753` (`ffi_scalar_type`), `:2759`
  (`parse_extern_fn`), `:2778` (rejects `inout`/`inout` params), `:2780`
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3619`; literal at `:1763`). Three distinct
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4405-4418`; runtime `:818-825`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4376-4471`). No unsafety
