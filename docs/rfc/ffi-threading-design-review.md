...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2798`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2538-2577`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4192-4205`), there is no
...
- Parse + type gate: `src/tychoc.c:2540` (`ffi_scalar_type`), `:2546`
  (`parse_extern_fn`), `:2565` (rejects `inout`/`inout` params), `:2567`
  (rejects composite params), `:2579` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9031-9058`. Each
  `extern "Lib"` adds `-lLib` (`:5455` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9249-9253`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9006-9009`, `:2988-3013`, `:9889-9891`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6373-6380`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3406`; literal at `:1553`). Three distinct
...
`src/tychoc.c:10202` for `len`, `:10329` and `:10336` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6734-6745`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8811-8814`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4192-4205`; runtime `:663-670`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4264-4359`). No unsafety
