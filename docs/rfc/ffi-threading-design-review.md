...
`src/tychoc.c:2570-2609`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4224-4237`), there is no
...
- Parse + type gate: `src/tychoc.c:2572` (`ffi_scalar_type`), `:2578`
  (`parse_extern_fn`), `:2597` (rejects `inout`/`inout` params), `:2599`
  (rejects composite params), `:2611` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9100-9127`. Each
  `extern "Lib"` adds `-lLib` (`:5504` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9318-9322`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9075-9078`, `:3020-3045`, `:9958-9960`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6424-6431`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3438`; literal at `:1582`). Three distinct
...
`src/tychoc.c:10271` for `len`, `:10398` and `:10405` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6785-6796`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8880-8883`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4224-4237`; runtime `:670-677`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4296-4391`). No unsafety
