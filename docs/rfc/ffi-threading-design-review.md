...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2869`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2623-2662`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4296-4309`), there is no
...
- Parse + type gate: `src/tychoc.c:2623` (`ffi_scalar_type`), `:2629`
  (`parse_extern_fn`), `:2648` (rejects `inout`/`inout` params), `:2650`
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3449`; literal at `:1652`). Three distinct
...
`src/tychoc.c:10055` for `len`, `:10182` and `:10189` for print/println). Footguns:
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4296-4309`; runtime `:613-620`,
