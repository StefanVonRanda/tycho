...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2754`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2494-2533`). That keeps the *language* sound — no foreign
...
- Parse + type gate: `src/tychoc.c:2496` (`ffi_scalar_type`), `:2502`
  (`parse_extern_fn`), `:2521` (rejects `inout`/`inout` params), `:2523`
  (rejects composite params), `:2535` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8975-9002`. Each
  `extern "Lib"` adds `-lLib` (`:5411` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9193-9197`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8950-8953`, `:2944-2969`, `:9833-9835`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6329-6336`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3362`; literal at `:1509`). Three distinct
...
`src/tychoc.c:10146` for `len`, `:10273` and `:10280` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6690-6701`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8767-8770`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4148-4161`; runtime `:643-650`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4220-4315`). No unsafety
