...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3020`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2755-2799`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4420-4433`), there is no
...
- Parse + type gate: `src/tychoc.c:2863` (`ffi_scalar_type`), `:2869`
  (`parse_extern_fn`), `:2888` (rejects `inout`/`inout` params), `:2890`
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3735`; literal at `:1828`). Three distinct
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4521-4534`; runtime `:858-865`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4492-4587`). No unsafety
