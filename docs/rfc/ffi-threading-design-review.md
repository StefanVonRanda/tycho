...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2993`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2728-2772`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4393-4406`), there is no
...
- Parse + type gate: `src/tychoc.c:2836` (`ffi_scalar_type`), `:2842`
  (`parse_extern_fn`), `:2861` (rejects `inout`/`inout` params), `:2863`
  (rejects composite params), `:2875` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9425-9452`. Each
  `extern "Lib"` adds `-lLib` (`:5826` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9750-9754`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9507-9510`, `:3284-3309`, `:10390-10392`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6676-6683`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3708`; literal at `:1827`). Three distinct
...
`src/tychoc.c:10713` for `len`, `:10840` and `:10847` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7037-7048`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9204-9207`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4494-4507`; runtime `:858-865`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4465-4560`). No unsafety
