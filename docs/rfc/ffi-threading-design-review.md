...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2907`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2661-2700`). That keeps the *language* sound — no foreign
...
- Parse + type gate: `src/tychoc.c:2661` (`ffi_scalar_type`), `:2667`
  (`parse_extern_fn`), `:2686` (rejects `inout`/`inout` params), `:2688`
  (rejects composite params), `:2700` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9005-9074`. Each
  `extern "Lib"` adds `-lLib` (`:5253` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9011-9015`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8793-8796`, `:3067-3092`, `:9062-9064`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6327-6334`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3487`; literal at `:1674`). Three distinct
...
`src/tychoc.c:10099` for `len`, `:10226` and `:10233` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6715-6726`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8803-8806`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4340-4353`; runtime `:613-620`,
