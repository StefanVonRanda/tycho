...
full deep-copy of captures per chunk (`src/tychoc.c:4445-4458`), there is no
...
- Link line assembled in one `cc` call: `src/tychoc.c:9286-9355`. Each
  `extern "Lib"` adds `-lLib` (`:5455` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9292-9296`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9074-9077`, `:3141-3166`, `:9343-9345`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6540-6547`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10380` for `len`, `:10507` and `:10514` for print/println). Footguns:
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9084-9087`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4445-4458`; runtime `:633-640`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4517-4612`). No unsafety
