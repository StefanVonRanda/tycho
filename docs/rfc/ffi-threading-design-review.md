...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2975`, `examples/sqlite/demo.ty`).
...
- Link line assembled in one `cc` call: `src/tychoc.c:9376-9403`. Each
  ...
  passthrough at `:9701-9705`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9458-9461`, `:3266-3291`, `:10341-10343`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6629-6636`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10654` for `len`, `:10781` and `:10788` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6990-7001`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9155-9158`).
