...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3057`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2792-2836`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4457-4470`), there is no
...
- Parse + type gate: `src/tychoc.c:2900` (`ffi_scalar_type`), `:2906`
  (`parse_extern_fn`), `:2925` (rejects `inout`/`inout` params), `:2927`
  (rejects composite params), `:2939` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9594-9621`. Each
  `extern "Lib"` adds `-lLib` (`:5943` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9919-9923`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9676-9679`, `:3348-3373`, `:10560-10562`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6820-6827`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3772`; literal at `:1837`). Three distinct
...
`src/tychoc.c:10900` for `len`, `:11027` and `:11034` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7182-7193`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9373-9376`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4558-4571`; runtime `:858-865`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4529-4624`). No unsafety
