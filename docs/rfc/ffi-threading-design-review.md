...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2961`, `examples/sqlite/demo.ty`).
...
full deep-copy of captures per chunk (`src/tychoc.c:4458-4471`), there is no
...
- Parse + type gate: `src/tychoc.c:2696` (`ffi_scalar_type`), `:2702`
  (`parse_extern_fn`), `:2721` (rejects `inout`/`inout` params), `:2723`
  (rejects composite params), `:2735` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9313-9382`. Each
  `extern "Lib"` adds `-lLib` (`:5468` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9319-9323`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9101-9104`, `:3141-3166`, `:9370-9372`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6553-6560`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3561`; literal at `:1691`). Three distinct
...
`src/tychoc.c:10407` for `len`, `:10534` and `:10541` for print/println). Footguns:
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9111-9114`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4458-4471`; runtime `:633-640`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4530-4625`). No unsafety
