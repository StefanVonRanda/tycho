...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3143`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2878-2922`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4717-4730`), there is no
...
- Parse + type gate: `src/tychoc.c:2986` (`ffi_scalar_type`), `:2992`
  (`parse_extern_fn`), `:3011` (rejects `inout`/`inout` params), `:3013`
  (rejects composite params), `:3025` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:10093-10120`. Each
  `extern "Lib"` adds `-lLib` (`:6256` `add_link`). `--link/--shim/--pkg`
  passthrough at `:10398-10402`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:10155-10158`, `:3504-3529`, `:11039-11041`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:7235-7242`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3928`; literal at `:1894`). Three distinct
...
`src/tychoc.c:11415` for `len`, `:11506` and `:11513` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7607-7620`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9840-9843`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4818-4831`; runtime `:894-901`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4789-4884`). No unsafety
