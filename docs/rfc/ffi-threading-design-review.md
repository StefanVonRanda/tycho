...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2922`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2662-2701`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4316-4329`), there is no
...
- Parse + type gate: `src/tychoc.c:2765` (`ffi_scalar_type`), `:2771`
  (`parse_extern_fn`), `:2790` (rejects `inout`/`inout` params), `:2792`
  (rejects composite params), `:2804` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9265-9292`. Each
  `extern "Lib"` adds `-lLib` (`:5743` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9590-9594`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9347-9350`, `:3213-3238`, `:10230-10232`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6556-6563`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3631`; literal at `:1763`). Three distinct
...
`src/tychoc.c:10543` for `len`, `:10670` and `:10677` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6917-6928`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9044-9047`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4417-4430`; runtime `:818-825`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4388-4483`). No unsafety
