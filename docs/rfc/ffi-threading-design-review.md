...
full deep-copy of captures per chunk (`src/tychoc.c:4380-4393`), there is no
...
- Link line assembled in one `cc` call: `src/tychoc.c:9137-9206`. Each
  `extern "Lib"` adds `-lLib` (`:5364` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9143-9147`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8925-8928`, `:3093-3118`, `:9194-9196`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6440-6447`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10231` for `len`, `:10358` and `:10365` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6828-6839`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8935-8938`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4380-4393`; runtime `:627-634`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4452-4547`). No unsafety
