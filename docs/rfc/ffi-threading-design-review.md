...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3084`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2819-2863`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4645-4658`), there is no
...
- Parse + type gate: `src/tychoc.c:2927` (`ffi_scalar_type`), `:2933`
  (`parse_extern_fn`), `:2952` (rejects `inout`/`inout` params), `:2954`
  (rejects composite params), `:2966` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9875-9902`. Each
  `extern "Lib"` adds `-lLib` (`:6181` `add_link`). `--link/--shim/--pkg`
  passthrough at `:10216-10220`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9973-9976`, `:3432-3457`, `:10857-10859`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:7097-7104`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3856`; literal at `:1845`). Three distinct
...
`src/tychoc.c:11197` for `len`, `:11324` and `:11331` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7463-7474`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9654-9657`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4746-4759`; runtime `:859-866`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4717-4812`). No unsafety
