...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2839`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2593-2632`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4266-4279`), there is no
...
- Parse + type gate: `src/tychoc.c:2593` (`ffi_scalar_type`), `:2599`
  (`parse_extern_fn`), `:2618` (rejects `inout`/`inout` params), `:2620`
  (rejects composite params), `:2632` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8833-8902`. Each
  `extern "Lib"` adds `-lLib` (`:5164` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8839-8843`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8621-8624`, `:2999-3024`, `:8890-8892`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6195-6202`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3419`; literal at `:1644`). Three distinct
...
`len()`/`print()`/`println()` (`src/tychoc.c:5940-5944` `is_extern_str_call`,
applied at `:5982` for `len`, `:6092` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6583-6594`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8631-8634`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4266-4279`; runtime `:605-612`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4323-4418`). No unsafety
