...
`src/tychoc.c:2639-2678`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4318-4331`), there is no
...
- Parse + type gate: `src/tychoc.c:2639` (`ffi_scalar_type`), `:2645`
  (`parse_extern_fn`), `:2664` (rejects `inout`/`inout` params), `:2666`
  (rejects composite params), `:2678` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8983-9052`. Each
  `extern "Lib"` adds `-lLib` (`:5231` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8989-8993`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8771-8774`, `:3045-3070`, `:9040-9042`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6305-6312`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3465`; literal at `:1652`). Three distinct
...
`src/tychoc.c:10077` for `len`, `:10204` and `:10211` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6693-6704`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8781-8784`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4318-4331`; runtime `:613-620`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4390-4485`). No unsafety
