...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2885`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2625-2664`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4279-4292`), there is no
...
- Parse + type gate: `src/tychoc.c:2627` (`ffi_scalar_type`), `:2633`
  (`parse_extern_fn`), `:2652` (rejects `inout`/`inout` params), `:2654`
  (rejects composite params), `:2666` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9214-9241`. Each
  `extern "Lib"` adds `-lLib` (`:5593` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9432-9436`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9189-9192`, `:3075-3100`, `:10072-10074`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6513-6520`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3493`; literal at `:1637`). Three distinct
...
`src/tychoc.c:10385` for `len`, `:10512` and `:10519` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6874-6885`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8993-8996`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4279-4292`; runtime `:714-721`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4351-4446`). No unsafety
