...
- Link line assembled in one `cc` call: `src/tychoc.c:9300-9369`. Each
  `extern "Lib"` adds `-lLib` (`:5455` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9306-9310`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9088-9091`, `:3141-3166`, `:9357-9359`.
...
`src/tychoc.c:10394` for `len`, `:10521` and `:10528` for print/println). Footguns:
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9098-9101`).
