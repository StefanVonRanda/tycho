...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3041`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2776-2820`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4441-4454`), there is no
...
- Parse + type gate: `src/tychoc.c:2884` (`ffi_scalar_type`), `:2890`
  (`parse_extern_fn`), `:2909` (rejects `inout`/`inout` params), `:2911`
  (rejects composite params), `:2923` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9585-9612`. Each
  `extern "Lib"` adds `-lLib` (`:5927` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9910-9914`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9667-9670`, `:3332-3357`, `:10551-10553`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6807-6814`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3756`; literal at `:1821`). Three distinct
...
`src/tychoc.c:10891` for `len`, `:11018` and `:11025` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7173-7184`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9364-9367`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4542-4555`; runtime `:836-843`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4513-4608`). No unsafety
