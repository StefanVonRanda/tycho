...
`src/tychoc.c:2587-2626`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4241-4254`), there is no
...
- Parse + type gate: `src/tychoc.c:2589` (`ffi_scalar_type`), `:2595`
  (`parse_extern_fn`), `:2614` (rejects `inout`/`inout` params), `:2616`
  (rejects composite params), `:2628` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9142-9169`. Each
  `extern "Lib"` adds `-lLib` (`:5521` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9360-9364`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9117-9120`, `:3037-3062`, `:10000-10002`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6441-6448`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3455`; literal at `:1599`). Three distinct
...
`src/tychoc.c:10313` for `len`, `:10440` and `:10447` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6802-6813`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8921-8924`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4241-4254`; runtime `:687-694`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4313-4408`). No unsafety
