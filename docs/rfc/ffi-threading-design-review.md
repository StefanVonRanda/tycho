...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2920`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2674-2713`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4353-4366`), there is no
...
- Parse + type gate: `src/tychoc.c:2674` (`ffi_scalar_type`), `:2680`
  (`parse_extern_fn`), `:2699` (rejects `inout`/`inout` params), `:2701`
  (rejects composite params), `:2713` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9089-9158`. Each
  `extern "Lib"` adds `-lLib` (`:5337` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9095-9099`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8877-8880`, `:3080-3105`, `:9146-9148`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6411-6418`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3500`; literal at `:1674`). Three distinct
...
`src/tychoc.c:10183` for `len`, `:10310` and `:10317` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6799-6810`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8887-8890`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4353-4366`; runtime `:613-620`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4425-4520`). No unsafety
