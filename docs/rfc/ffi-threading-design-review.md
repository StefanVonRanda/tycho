...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2850`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2604-2643`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4277-4290`), there is no
...
- Parse + type gate: `src/tychoc.c:2604` (`ffi_scalar_type`), `:2610`
  (`parse_extern_fn`), `:2629` (rejects `inout`/`inout` params), `:2631`
  (rejects composite params), `:2643` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8911-8980`. Each
  `extern "Lib"` adds `-lLib` (`:5175` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8917-8921`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8699-8702`, `:3010-3035`, `:8968-8970`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6249-6256`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3430`; literal at `:1652`). Three distinct
...
`src/tychoc.c:9984` for `len`, `:10111` and `:10118` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6637-6648`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8709-8712`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4277-4290`; runtime `:613-620`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4334-4429`). No unsafety
