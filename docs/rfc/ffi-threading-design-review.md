...
- Link line assembled in one `cc` call: `src/tychoc.c:9341-9368`. Each
  `extern "Lib"` adds `-lLib` (`:5767` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9666-9670`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9423-9426`, `:3231-3256`, `:10306-10308`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6594-6601`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10619` for `len`, `:10746` and `:10753` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6955-6966`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9120-9123`).
