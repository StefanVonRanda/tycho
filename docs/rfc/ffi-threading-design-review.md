...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2753`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2493-2532`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4147-4160`), there is no
...
- Parse + type gate: `src/tychoc.c:2493` (`ffi_scalar_type`), `:2499`
  (`parse_extern_fn`), `:2518` (rejects `inout`/`inout` params), `:2520`
  (rejects composite params), `:2532` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8854-8881`. Each
  `extern "Lib"` adds `-lLib` (`:5148` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8860-8864`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8655-8658`, `:2941-2966`, `:9500-9502`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6232-6239`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3361`; literal at `:1509`). Three distinct
...
`src/tychoc.c:9813` for `len`, `:9940` and `:9947` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6593-6604`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8665-8668`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4147-4160`; runtime `:643-650`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4219-4314`). No unsafety
