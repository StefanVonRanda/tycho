...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2762`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2530-2569`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4144-4157`), there is no
...
- Parse + type gate: `src/tychoc.c:2530` (`ffi_scalar_type`), `:2536`
  (`parse_extern_fn`), `:2555` (rejects `inout`/`inout` params), `:2557`
  (rejects composite params), `:2569` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:8602-8671`. Each
  `extern "Lib"` adds `-lLib` (`:5026` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8608-8612`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8400-8403`, `:2922-2947`, `:8659-8661`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6029-6036`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3311`; literal at `:1590`). Three distinct
...
`len()`/`print()`/`println()` (`src/tychoc.c:5774-5778` `is_extern_str_call`,
applied at `:5816` for `len`, `:5926` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6406-6416`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8410-8413`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4144-4157`; runtime `:551-558`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4201-4288`). No unsafety
