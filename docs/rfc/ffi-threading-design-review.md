...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2997`, `examples/sqlite/demo.ty`).
...
full deep-copy of captures per chunk (`src/tychoc.c:4506-4519`), there is no
...
- Link line assembled in one `cc` call: `src/tychoc.c:9385-9454`. Each
  `extern "Lib"` adds `-lLib` (`:5516` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9391-9395`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9172-9175`, `:3177-3202`, `:9442-9444`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6608-6615`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3597`; literal at `:1701`). Three distinct
...
`src/tychoc.c:10479` for `len`, `:10606` and `:10613` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6979-6990`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9182-9185`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4506-4519`; runtime `:643-650`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4578-4673`). No unsafety
