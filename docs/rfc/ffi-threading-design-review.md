...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:3063`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2798-2842`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4463-4476`), there is no
...
- Parse + type gate: `src/tychoc.c:2906` (`ffi_scalar_type`), `:2912`
  (`parse_extern_fn`), `:2931` (rejects `inout`/`inout` params), `:2933`
  (rejects composite params), `:2945` (rejects composite return).
- Type table: `docs/reference/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9607-9634`. Each
  `extern "Lib"` adds `-lLib` (`:5949` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9932-9936`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9689-9692`, `:3354-3379`, `:10573-10575`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6829-6836`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3778`; literal at `:1843`). Three distinct
...
`src/tychoc.c:10913` for `len`, `:11040` and `:11047` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7195-7206`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9386-9389`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4564-4577`; runtime `:858-865`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4535-4630`). No unsafety
