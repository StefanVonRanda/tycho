...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2934`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2688-2727`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4398-4411`), there is no
...
- Parse + type gate: `src/tychoc.c:2688` (`ffi_scalar_type`), `:2694`
  (`parse_extern_fn`), `:2713` (rejects `inout`/`inout` params), `:2715`
  (rejects composite params), `:2727` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9165-9234`. Each
  `extern "Lib"` adds `-lLib` (`:5382` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9171-9175`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8953-8956`, `:3094-3119`, `:9222-9224`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6458-6465`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3514`; literal at `:1685`). Three distinct
...
`src/tychoc.c:10259` for `len`, `:10386` and `:10393` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6846-6857`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8963-8966`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4398-4411`; runtime `:627-634`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4470-4565`). No unsafety
