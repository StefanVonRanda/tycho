...
- Link line assembled in one `cc` call: `src/tychoc.c:8874-8943`. Each
  `extern "Lib"` adds `-lLib` (`:5172` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8880-8884`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8662-8665`, `:3007-3032`, `:8931-8933`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6236-6243`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:9947` for `len`, `:10074` and `:10081` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6624-6635`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8672-8675`).
