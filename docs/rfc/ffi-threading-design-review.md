...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2832`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2586-2625`). That keeps the *language* sound — no foreign
...
- Parse + type gate: `src/tychoc.c:2586` (`ffi_scalar_type`), `:2592`
  (`parse_extern_fn`), `:2611` (rejects `inout`/`inout` params), `:2613`
...
- Link line assembled in one `cc` call: `src/tychoc.c:8826-8895`. Each
  `extern "Lib"` adds `-lLib` (`:5157` `add_link`). `--link/--shim/--pkg`
  passthrough at `:8832-8836`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8614-8617`, `:2992-3017`, `:8883-8885`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6188-6195`, `tycho_str_from_c`, NULL→`""`).
...
`len()`/`print()`/`println()` (`src/tychoc.c:5933-5937` `is_extern_str_call`,
applied at `:5975` for `len`, `:6085` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6576-6587`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8624-8627`).
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4316-4411`). No unsafety
