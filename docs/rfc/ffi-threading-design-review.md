...
- **FFI — shipped:** R1 `bytes` (`tests/bytes*`), R2 typed `handle`s with
  ...
  constructors (`src/tychoc.c:2933`, `examples/sqlite/demo.ty`).
...
`src/tychoc.c:2687-2726`). That keeps the *language* sound — no foreign
...
full deep-copy of captures per chunk (`src/tychoc.c:4366-4379`), there is no
...
- Parse + type gate: `src/tychoc.c:2687` (`ffi_scalar_type`), `:2693`
  (`parse_extern_fn`), `:2712` (rejects `inout`/`inout` params), `:2714`
  (rejects composite params), `:2726` (rejects composite return).
- Type table: `docs/guides/ffi.md:62-71`. `int/char/float/bool` → scalar long/double;
  `string` → `char *`; `ptr` → `void *`; void return allowed.
- Link line assembled in one `cc` call: `src/tychoc.c:9104-9173`. Each
  `extern "Lib"` adds `-lLib` (`:5350` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9110-9114`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:8892-8895`, `:3093-3118`, `:9161-9163`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6426-6433`, `tycho_str_from_c`, NULL→`""`).
...
`E_NULL` → `T_PTR` at `src/tychoc.c:3513`; literal at `:1685`). Three distinct
...
`src/tychoc.c:10198` for `len`, `:10325` and `:10332` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:6814-6825`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:8902-8905`).
...
- **`parallel for` forks `ncpu` threads with full capture copy per chunk.** K =
  `tycho_ncpu()` chunk tasks (`src/tychoc.c:4366-4379`; runtime `:627-634`,
...
5. **Affine/implicit-join edge cases.** The affine rules are enforced and look
   ...
   inout captures / cross-chunk mutation, `src/tychoc.c:4438-4533`). No unsafety
