...
- Link line assembled in one `cc` call: `src/tychoc.c:9410-9479`. Each
  `extern "Lib"` adds `-lLib` (`:5541` `add_link`). `--link/--shim/--pkg`
  passthrough at `:9416-9420`. Auto-discovered `<pkg>_shim.c` + `deps`
  pkg-config at `:9197-9200`, `:3202-3227`, `:9467-9469`.
- String return is arena-copied so Tycho never holds a foreign pointer
  (`src/tychoc.c:6633-6640`, `tycho_str_from_c`, NULL→`""`).
...
`src/tychoc.c:10504` for `len`, `:10631` and `:10638` for print/println). Footguns:
...
- *Design.* Allow a newtype-style declaration that tags a handle and names its
  ...
  task/channel finalizer mechanism (`src/tychoc.c:7004-7015`) that already runs
...
- *Incremental or fundamental.* Incremental, medium effort (codegen of a small
  C wrapper, alongside the existing shim plumbing at `src/tychoc.c:9207-9210`).
