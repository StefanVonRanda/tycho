...
(`src/tychoc.c:8939-8945`, `:8969-8973`).
...
bytes (`src/tychoc.c:8631-8646`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8349-8352`). A channel does the
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8643`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8962-8964`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8979-8983`). There is a
