...
arena" string that defaults to `&_scope` (`src/tychoc.c:7382`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8921-8927`, `:8951-8955`).
...
bytes (`src/tychoc.c:8613-8628`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8331-8334`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7382`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8625`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8944-8946`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8961-8965`). There is a
