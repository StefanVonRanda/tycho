...
bytes (`src/tychoc.c:8527-8542`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8245-8248`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7297`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8539`) generalises without a new
