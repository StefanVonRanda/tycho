...
arena" string that defaults to `&_scope` (`src/tychoc.c:7452`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:9008-9014`, `:9038-9042`).
...
bytes (`src/tychoc.c:8700-8715`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8418-8421`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7452`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8712`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:9031-9033`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9048-9052`). There is a
