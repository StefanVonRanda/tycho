...
per-proc stack of enclosing block arenas (`src/tychoc.c:10953@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11012@g_nascope`).
...
bytes (`src/tychoc.c:10647-10655`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:10205@arena_free(&_tk->root)`). A channel does the same per payload
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:10655@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:10963@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:10958-10961`). There is a
