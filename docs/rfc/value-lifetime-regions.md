...
arena" string that defaults to `&_scope` (`src/tychoc.c:9191@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:10996@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11055@g_nascope`).
...
bytes (`src/tychoc.c:10690-10698`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:10248@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:9191@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:10698@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11006@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:11001-11004`). There is a
