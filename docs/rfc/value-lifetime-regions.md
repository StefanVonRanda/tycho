...
arena" string that defaults to `&_scope` (`src/tychoc.c:7384`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8923-8929`, `:8953-8957`).
...
bytes (`src/tychoc.c:8615-8630`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8333-8336`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7384`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8627`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8946-8948`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8963-8967`). There is a
