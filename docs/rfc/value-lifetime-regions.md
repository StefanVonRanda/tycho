...
arena" string that defaults to `&_scope` (`src/tychoc.c:7416`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8972-8978`, `:9002-9006`).
...
bytes (`src/tychoc.c:8664-8679`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8382-8385`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7416`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8676`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8995-8997`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9012-9016`). There is a
