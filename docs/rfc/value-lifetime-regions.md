...
arena" string that defaults to `&_scope` (`src/tychoc.c:7236`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8764-8770`, `:8794-8798`).
...
bytes (`src/tychoc.c:8466-8481`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8184-8187`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7236`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8478`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8787-8789`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8804-8808`). There is a
