...
arena" string that defaults to `&_scope` (`src/tychoc.c:7221`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8748-8754`, `:8778-8782`).
...
bytes (`src/tychoc.c:8450-8465`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8168-8171`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7221`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8462`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8771-8773`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8788-8792`). There is a
