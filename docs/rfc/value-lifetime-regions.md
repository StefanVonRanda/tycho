...
arena" string that defaults to `&_scope` (`src/tychoc.c:7458`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:9014-9020`, `:9044-9048`).
...
bytes (`src/tychoc.c:8706-8721`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8424-8427`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7458`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8718`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:9037-9039`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9054-9058`). There is a
