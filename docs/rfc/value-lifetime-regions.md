...
arena" string that defaults to `&_scope` (`src/tychoc.c:10245@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11997@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:12056@g_nascope`).
...
bytes (`src/tychoc.c:11610-11618`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:11219@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:10245@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11689@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:12007@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:11931-11934`). There is a
