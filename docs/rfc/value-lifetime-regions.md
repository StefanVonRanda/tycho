...
arena" string that defaults to `&_scope` (`src/tychoc.c:9169@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:10974@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11033@g_nascope`).
...
bytes (`src/tychoc.c:10668-10676`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:10226@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:9169@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:10676@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:10984@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:10979-10982`). There is a
