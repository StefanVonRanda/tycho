...
arena" string that defaults to `&_scope` (`src/tychoc.c:7423`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8979-8985`, `:9009-9013`).
...
bytes (`src/tychoc.c:8671-8686`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8389-8392`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7423`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8683`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:9002-9004`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9019-9023`). There is a
