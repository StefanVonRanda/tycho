...
arena" string that defaults to `&_scope` (`src/tychoc.c:10231@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11983@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:12042@g_nascope`).
...
- a channel cell's payload arena, one per ring slot, created at channel construction
  and freed when a later send reclaims the slot (`runtime/tycho_rt.c:1095@arena_new`,
  `runtime/tycho_rt.c:1145@arena_free`);
...
list (`corelib/pool/pool.ty:57-65`). It does **not** clear `val`. So the removed
...
per-connection state across iterations (`examples/webserver/main.ty:186-196`); the
...
bytes (`src/tychoc.c:11596-11604`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:11205@arena_free(&_tk->root)`). A channel does the same per payload
into a per-cell arena (`runtime/tycho_rt.c:1055@payload bytes live here`). An `@r` value
...
(`corelib/pool/pool.ty:24-70`), including the property that makes it sound — `get`
returns a copy, so the pool is never aliased (`corelib/pool/pool.ty:43-48`).
...
in, and the compiler already picks that per write site (`src/tychoc.c:10231@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11675@copy_into`) generalises without a new
...
(`runtime/tycho_rt.c:625@g_block_pool`), blocks are released to whichever thread's pool
frees them (`runtime/tycho_rt.c:663@HBlock *nx`), and a spawned thread flushes its pool
...
(`runtime/tycho_rt.c:1055@payload bytes live here`).
...
six fields (`runtime/tycho_rt.c:291@Arena`):
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11993@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:11917-11920`). There is a
...
`TYCHO_BLOCK_DEFAULT` = 65536 (`runtime/tycho_rt.c:88@TYCHO_BLOCK_DEFAULT`,
`runtime/tycho_rt.c:670@TYCHO_BLOCK_DEFAULT`), and the first allocation in a fresh
...
non-goal (`docs/architecture.md:58`).
...
(`docs/spec/00-conventions.md:81-89`); the spec
...
> (`runtime/tycho_rt.c:88@TYCHO_BLOCK_DEFAULT`), and its live
...
`bench/README.md:12-38`, which is every measured workload in the tree, and opened the
...
`reserve` (`bench/README.md:20`). All three are *live* retention, which no region
