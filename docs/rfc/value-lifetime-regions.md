...
arena" string that defaults to `&_scope` (`src/tychoc.c:9554@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11359@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11418@g_nascope`).
...
(`src/tychoc.c:10611@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:9554@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11061@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11369@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:11364-11367`). There is a
