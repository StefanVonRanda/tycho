...
arena" string that defaults to `&_scope` (`src/tychoc.c:7323`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8862-8868`, `:8892-8896`).
...
bytes (`src/tychoc.c:8554-8569`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8272-8275`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7323`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8566`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8885-8887`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8902-8906`). There is a
