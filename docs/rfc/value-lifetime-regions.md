...
arena" string that defaults to `&_scope` (`src/tychoc.c:10078@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11830@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11889@g_nascope`).
...
(`src/tychoc.c:11052@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:10078@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11522@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11840@g_taskvars`),
