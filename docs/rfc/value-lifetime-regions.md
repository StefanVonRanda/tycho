...
arena" string that defaults to `&_scope` (`src/tychoc.c:7400`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8956-8962`, `:8986-8990`).
...
bytes (`src/tychoc.c:8648-8663`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8366-8369`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7400`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8660`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8979-8981`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8996-9000`). There is a
