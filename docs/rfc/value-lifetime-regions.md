...
arena" string that defaults to `&_scope` (`src/tychoc.c:10175@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11927@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11986@g_nascope`).
...
- a channel cell's payload arena, one per ring slot, created at channel construction
  and freed when a later send reclaims the slot (`runtime/tycho_rt.c:1083@arena_new`,
  `runtime/tycho_rt.c:1133@arena_free`);
...
bytes (`src/tychoc.c:11540-11548`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:11149@arena_free(&_tk->root)`). A channel does the same per payload
into a per-cell arena (`runtime/tycho_rt.c:1043@payload bytes live here`). An `@r` value
...
in, and the compiler already picks that per write site (`src/tychoc.c:10175@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11619@copy_into`) generalises without a new
...
(`runtime/tycho_rt.c:613@g_block_pool`), blocks are released to whichever thread's pool
frees them (`runtime/tycho_rt.c:651@HBlock *nx`), and a spawned thread flushes its pool
...
(`runtime/tycho_rt.c:1043@payload bytes live here`).
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11937@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:11861-11864`). There is a
...
`runtime/tycho_rt.c:658@TYCHO_BLOCK_DEFAULT`), and the first allocation in a fresh
