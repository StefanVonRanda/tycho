...
arena" string that defaults to `&_scope` (`src/tychoc.c:9132@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:10826@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:10885@g_nascope`).
...
- a channel cell's payload arena, one per ring slot, created at channel construction
  and freed when a later send reclaims the slot (`runtime/tycho_rt.c:891@arena_new`,
  `runtime/tycho_rt.c:941@arena_free`);
...
bytes (`src/tychoc.c:10520-10528`, `runtime/tycho_rt.c@tycho_task_new`); `wait` copies
the result out and frees the root eagerly
(`src/tychoc.c:10105@arena_free(&_tk->root)`). A channel does the same per payload
into a per-cell arena (`runtime/tycho_rt.c:851@payload bytes live here`). An `@r` value
...
in, and the compiler already picks that per write site (`src/tychoc.c:9132@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:10528@copy_into`) generalises without a new
...
(`runtime/tycho_rt.c:851@payload bytes live here`).
...
the finaliser slot tasks and handles already use (`src/tychoc.c:10836@g_taskvars`),
which fires at block end, early `return`, `break`/`continue` and `or_return`
(`src/tychoc.c:10831-10834`). There is a
