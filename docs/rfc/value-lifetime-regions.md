...
arena" string that defaults to `&_scope` (`src/tychoc.c:10117@g_cur_scope`) and a
per-proc stack of enclosing block arenas (`src/tychoc.c:11869@g_ascope`) freed
innermost-first at any exit (`src/tychoc.c:11928@g_nascope`).
...
(`src/tychoc.c:11091@arena_free(&_tk->root)`). A channel does the same per payload
...
in, and the compiler already picks that per write site (`src/tychoc.c:10117@g_cur_scope`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:11561@copy_into`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:11879@g_taskvars`),
