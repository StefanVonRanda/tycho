...
arena" string that defaults to `&_scope` (`src/tychoc.c:7309`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8848-8854`, `:8878-8882`).
...
bytes (`src/tychoc.c:8540-8555`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8258-8261`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7309`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8552`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8871-8873`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:8888-8892`). There is a
