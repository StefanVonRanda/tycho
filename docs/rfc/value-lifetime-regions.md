...
arena" string that defaults to `&_scope` (`src/tychoc.c:7408`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8964-8970`, `:8994-8998`).
...
bytes (`src/tychoc.c:8656-8671`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8374-8377`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7408`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8668`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:8987-8989`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9004-9008`). There is a
