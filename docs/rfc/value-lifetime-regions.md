...
arena" string that defaults to `&_scope` (`src/tychoc.c:7431`) and a per-proc stack
of enclosing block arenas freed innermost-first at any exit
(`src/tychoc.c:8987-8993`, `:9017-9021`).
...
bytes (`src/tychoc.c:8679-8694`, `runtime/tycho_rt.c:523-531`); `wait` copies the
result out and frees the root eagerly (`src/tychoc.c:8397-8400`). A channel does the
...
in, and the compiler already picks that per write site (`src/tychoc.c:7431`,
...
`copy_into(param, "(&_tk->root)", arg)` (`src/tychoc.c:8691`) generalises without a new
...
the finaliser slot tasks and handles already use (`src/tychoc.c:9010-9012`), which fires at block end, early
`return`, `break`/`continue` and `or_return` (`src/tychoc.c:9027-9031`). There is a
