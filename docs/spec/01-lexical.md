...
> Provenance: contextual dispatch at `src/tychoc.c:5055-5064` (top level),
> `:3689@"const"`/`:3712@"delete"` (`const`/`delete`), `:2316@soa [Struct]`/`:2869@soa []Struct` (`soa`),
> `:4281@"where"` (`where`), `:4252@"sink"` (`sink`), `:3943@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:2747-2753`.
...
> its unterminated diagnostic `:753@unterminated raw string literal`; adjacent join `:2515-2538`; `const` string fold
> `:4640-4644`; codegen pastes the escaped text into a C string literal
> `:10744@TYCHO_LIT`; that literal's decoded length `runtime/tycho_rt.c:1237@sizeof s - 1`.
...
> desugar `interp_join` / `desugar_interp`, `:2385-2460`.
