...
> Provenance: the lexer is `src/tychoc.c:264-609` (`lex`), the token kinds
> `:254-270` (`TokKind`), the keyword table `:305-347` (`keyword`).
...
> Provenance: a comment-only line is skipped without touching the indent stack `src/tychoc.c:478@*p == '#') {`; the token loop stops at `#` `:557@*p != '\n' && *p != '#'`; the trailing comment is consumed at `:845@if (*p == '#') while`.
...
> Provenance: `src/tychoc.c:277-313` (measure + INDENT/DEDENT),
> `:542@indentation too deep` (depth bound), `:685-686` (EOF flush).
...
> `src/tychoc.c:301-343`. There is no `while` keyword (the loop keyword is
...
> Provenance: contextual dispatch at `src/tychoc.c:5043-5052` (top level),
> `:3677@"const"`/`:3700@"delete"` (`const`/`delete`), `:2316@soa [Struct]`/`:2857@soa []Struct` (`soa`),
> `:4269@"where"` (`where`), `:4240@"sink"` (`sink`), `:3931@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:641-671`. `::` is lexed at `:793@TK_COLONCOLON` but no grammar
> production consumes it; `..<` at `:791@TK_DOTLT`, tested after `...` so maximal munch holds; `;` at `:816@TK_SEMI`.
...
> Provenance: `src/tychoc.c:2735-2741`.
...
> Provenance: `src/tychoc.c:352-359` (accumulation + overflow check).
...
> Provenance: `src/tychoc.c:322-347`; the leading-dot predicate is
> `tok_postfixable`, `:300-303`.
...
> Provenance: `src/tychoc.c:616-641`; the `\x` arm `:771@case 'x'`; the escape
> table `:770-850@case '0'`; the fixed-width refusal `:773@two hex digits`.
...
> Provenance: quoted piece `src/tychoc.c:376-461`; escape set `:431-440`;
> control-byte rejection `:448-451`; per-piece length bound `:644@char buf[4096]`,`:671@bn + 2 >= (int)sizeof buf`;
> raw piece `:463-612`, its re-escape table `:696-699`, its control-byte
> rejection `:475-476`, its per-piece bound `:746@rn + 2 >= (int)sizeof rbuf`,`:749@rn + 1 >= (int)sizeof rbuf`,
> its unterminated diagnostic `:753@unterminated raw string literal`; adjacent join `:2504-2527`; `const` string fold
> `:4628-4632`; codegen pastes the escaped text into a C string literal
> `:10716@TYCHO_LIT`; that literal's decoded length `runtime/tycho_rt.c:1237@sizeof s - 1`.
...
> Provenance: lexing — the identifier scanner declines the `f` of `f"…"` `src/tychoc.c:629@!(c == 'f' && p[1] == '"')`, the string scanner takes it `:376-461`;
> desugar `interp_join` / `desugar_interp`, `:2385-2449`.
