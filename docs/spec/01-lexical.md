...
> `src/tychoc.c:14215@memchr`; the same rule in the self-hosted
...
(`src/tychoc.c:3000-3003`), so `soa := 5` is accepted and the next mention of the
...
> Provenance: contextual dispatch at `src/tychoc.c:5248-5257` (top level),
> `:3830@"const"`/`:3853@"delete"` (`const`/`delete`),
> `:4422@"where"` (`where`), `:4393@"sink"` (`sink`), `:4084@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:2882-2888`.
...
> its unterminated diagnostic `:793@unterminated raw string literal`; adjacent join `:2645-2668`; `const` string fold
> `:4824-4828`; codegen pastes the escaped text into a C string literal
> `:11143@TYCHO_LIT`; that literal's decoded length `runtime/tycho_rt.c:1466@sizeof s - 1`.
...
> desugar `interp_join` / `desugar_interp`, `:2515-2590`.
