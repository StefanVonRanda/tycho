...
> Provenance: a comment-only line is skipped without touching the indent stack `src/tychoc.c:375@*p == '#') {`; the token loop stops at `#` `:401@*p != '\n' && *p != '#'`; the trailing comment is consumed at `:714@if (*p == '#') while`.
...
> `:386@indentation too deep` (depth bound), `:527-528` (EOF flush).
...
> `:3370@"const"`/`:3386@"delete"` (`const`/`delete`), `:2119@soa [Struct]`/`:2607@soa []Struct` (`soa`),
> `:3956@"where"` (`where`), `:3922@"sink"` (`sink`), `:3642@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:477-513`. `::` is lexed at `:675@TK_COLONCOLON` but no grammar
> production consumes it; `..<` at `:673@TK_DOTLT`, tested after `...` so maximal munch holds; `;` at `:698@TK_SEMI`.
...
> Provenance: `src/tychoc.c:2611-2617`.
...
> Provenance: `src/tychoc.c:452-477`; the `\x` arm `:653@case 'x'`; the escape
> table `:651-652@case '0'`; the fixed-width refusal `:655@two hex digits`.
...
> control-byte rejection `:389-391`; per-piece length bound `:517@char buf[4096]`,`:523@bn + 2 >= (int)sizeof buf`;
> raw piece `:402-448`, its re-escape table `:430-433`, its control-byte
> rejection `:434-435`, its per-piece bound `:628@rn + 2 >= (int)sizeof rbuf`,`:631@rn + 1 >= (int)sizeof rbuf`,
> its unterminated diagnostic `:635@unterminated raw string literal`; adjacent join `:2288-2300`; `const` string fold
> `:4317-4321`; codegen pastes the escaped text into a C string literal
> `:10000@tycho_str_intern`; `tycho_str_intern`'s `strlen` `runtime/tycho_rt.c:1241@strlen(s)`.
...
> Provenance: lexing — the identifier scanner declines the `f` of `f"…"` `src/tychoc.c:502@!(c == 'f' && p[1] == '"')`, the string scanner takes it `:319-400`;
