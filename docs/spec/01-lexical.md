...
> `src/tychoc.c:13987@memchr`; the same rule in the self-hosted
...
(`src/tychoc.c:2936-2939`), so `soa := 5` is accepted and the next mention of the
...
> Provenance: contextual dispatch at `src/tychoc.c:5132-5141` (top level),
> `:3766@"const"`/`:3789@"delete"` (`const`/`delete`),
> `:4358@"where"` (`where`), `:4329@"sink"` (`sink`), `:4020@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:2818-2824`.
...
> its unterminated diagnostic `:793@unterminated raw string literal`; adjacent join `:2581-2604`; `const` string fold
> `:4717-4721`; codegen pastes the escaped text into a C string literal
> `:10936@TYCHO_LIT`; that literal's decoded length `runtime/tycho_rt.c:1466@sizeof s - 1`.
...
> desugar `interp_join` / `desugar_interp`, `:2451-2526`.
