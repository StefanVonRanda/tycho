...
> Provenance: the lexer is `src/tychoc.c:271-592` (`lex`), the token kinds
> `:174-190` (`TokKind`), the keyword table `:225-267` (`keyword`).
...
> Provenance: a comment-only line is skipped without touching the indent stack `src/tychoc.c:504@*p == '#') {`; the token loop stops at `#` `:534@*p != '\n' && *p != '#'`; the trailing comment is consumed at `:860@if (*p == '#') while`.
...
> Provenance: `src/tychoc.c:284-320` (measure + INDENT/DEDENT),
> `:519@indentation too deep` (depth bound), `:589-590` (EOF flush).
...
> `src/tychoc.c:225-267`. There is no `while` keyword (the loop keyword is
...
> Provenance: contextual dispatch at `src/tychoc.c:4544-4553` (top level),
> `:3593@"const"`/`:3619@"delete"` (`const`/`delete`), `:2278@soa [Struct]`/`:2794@soa []Struct` (`soa`),
> `:4242@"where"` (`where`), `:4208@"sink"` (`sink`), `:3875@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:539-575`. `::` is lexed at `:808@TK_COLONCOLON` but no grammar
> production consumes it; `..<` at `:806@TK_DOTLT`, tested after `...` so maximal munch holds; `;` at `:831@TK_SEMI`.
...
> Provenance: `src/tychoc.c:2700-2706`.
...
> Provenance: `src/tychoc.c:359-366` (accumulation + overflow check).
...
> Provenance: `src/tychoc.c:329-354`; the leading-dot predicate is
> `tok_postfixable`, `:220-223`.
...
> Provenance: `src/tychoc.c:514-539`; the `\x` arm `:786@case 'x'`; the escape
> table `:785-786@case '0'`; the fixed-width refusal `:788@two hex digits`.
...
> Provenance: quoted piece `src/tychoc.c:379-461`; escape set `:434-443`;
...
> its unterminated diagnostic `:768@unterminated raw string literal`; adjacent join `:2363-2375`; `const` string fold
> `:4483-4487`; codegen pastes the escaped text into a C string literal
> `:10513@tycho_str_intern`; `tycho_str_intern`'s `strlen` `runtime/tycho_rt.c:1257@strlen(s)`.
...
> desugar `interp_join` / `desugar_interp`, `:2246-2308`.
