...
> `:4239@"where"` (`where`), `:4205@"sink"` (`sink`), `:3872@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:2697-2703`.
...
> its unterminated diagnostic `:768@unterminated raw string literal`; adjacent join `:2360-2372`; `const` string fold
> `:4480-4484`; codegen pastes the escaped text into a C string literal
> `:10486@tycho_str_intern`; `tycho_str_intern`'s `strlen` `runtime/tycho_rt.c:1257@strlen(s)`.
...
> desugar `interp_join` / `desugar_interp`, `:2243-2305`.
