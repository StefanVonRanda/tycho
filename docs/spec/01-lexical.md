...
> Provenance: the lexer is `src/tychoc.c:359-684` (`lex`), the token kinds
> `:349-365` (`TokKind`), the keyword table `:400-442` (`keyword`).
...
input) is part of the ending: a CRLF file lexes exactly as its LF twin does,
**blank lines included**. A `\r` anywhere else on a line is whitespace. A source
file containing a `NUL` byte MUST be rejected — silently truncating at it would
compile a program the file does not contain.

> Provenance: `src/tychoc.c@AT_EOL`, and the reader's refusal at
> `src/tychoc.c:14737@memchr`; the same rule in the self-hosted
...
> Provenance: a comment-only line is skipped without touching the indent stack `src/tychoc.c:578@*p == '#') {`; the token loop stops at `#` `:632@*p != '\n' && *p != '#'`; the trailing comment is consumed at `:920@if (*p == '#') while`.
...
> Provenance: `src/tychoc.c:372-408` (measure + INDENT/DEDENT),
> `:617@indentation too deep` (depth bound), `:760-761` (EOF flush).
...
(`src/tychoc.c:3083-3086`), so `soa := 5` is accepted and the next mention of the
...
> Provenance: contextual dispatch at `src/tychoc.c:5541-5550` (top level),
> `:4022@"const"`/`:4045@"delete"` (`const`/`delete`),
> `:4668@"where"` (`where`), `:4639@"sink"` (`sink`), `:4284@"range"` (`range`, refusal only).
...
> Provenance: `src/tychoc.c:716-746`. `::` is lexed at `:868@TK_COLONCOLON` but no grammar
> production consumes it; `..<` at `:866@TK_DOTLT`, tested after `...` so maximal munch holds; `;` at `:891@TK_SEMI`.
...
> Provenance: `src/tychoc.c:2965-2971`.
...
> Provenance: `src/tychoc.c:447-454` (accumulation + overflow check).
...
> Provenance: `src/tychoc.c:417-442`; the leading-dot predicate is
> `tok_postfixable`, `:395-398`.
...
> Provenance: `src/tychoc.c:691-716`; the `\x` arm `:846@case 'x'`; the escape
> table `:845-925@case '0'`; the fixed-width refusal `:848@two hex digits`.
...
> Provenance: quoted piece `src/tychoc.c:471-561`; escape set `:526-540`;
> control-byte rejection `:548-551`; per-piece length bound `:719@char buf[4096]`,`:746@bn + 2 >= (int)sizeof buf`;
> raw piece `:563-687`, its re-escape table `:771-774`, its control-byte
> rejection `:575-576`, its per-piece bound `:821@rn + 2 >= (int)sizeof rbuf`,`:824@rn + 1 >= (int)sizeof rbuf`,
> its unterminated diagnostic `:828@unterminated raw string literal`; adjacent join `:2728-2751`; `const` string fold
> `:5109-5113`; codegen pastes the escaped text into a C string literal
> `:11639@TYCHO_LIT`; that literal's decoded length `runtime/tycho_rt.c:1481@sizeof s - 1`.
...
> Provenance: lexing — the identifier scanner declines the `f` of `f"…"` `src/tychoc.c:704@!(c == 'f' && p[1] == '"')`, the string scanner takes it `:471-561`;
> desugar `interp_join` / `desugar_interp`, `:2598-2673`.
