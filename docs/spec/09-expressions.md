...
> Provenance: binary-op resolver `src/tychoc.c:6558-6814`; short-circuit
> lowering `:10252-10274`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6683-6713`; broadcast arm
> `src/tychoc.c:6742-6768`; the per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7206@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7315@modulo / bitwise operators`.
...
> `src/tychoc.c:6663-6669`, result at `src/tychoc.c:7122@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7282@rt`. Exhaustively pinned by
