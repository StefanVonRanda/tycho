...
> through to — shift `src/tychoc.c:6461@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6568@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6377@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6536@rt`. Exhaustively pinned by
