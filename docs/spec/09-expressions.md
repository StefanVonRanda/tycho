...
> through to — shift `src/tychoc.c:6584@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6691@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6500@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6659@rt`. Exhaustively pinned by
