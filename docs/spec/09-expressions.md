...
> Provenance: binary-op resolver `src/tychoc.c:6244-6500`; short-circuit
> lowering `:9664-9690`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6369-6399`; broadcast arm
> `src/tychoc.c:6428-6454`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6865@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6972@modulo / bitwise operators`.
...
> `src/tychoc.c:6349-6355`, result at `src/tychoc.c:6781@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6940@rt`. Exhaustively pinned by
