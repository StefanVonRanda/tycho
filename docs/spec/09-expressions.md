...
> Provenance: binary-op resolver `src/tychoc.c:6421-6677`; short-circuit
> lowering `:9857-9883`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6546-6576`; broadcast arm
> `src/tychoc.c:6605-6631`; the per-element-type operator set
> `src/tychoc.c:1393@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7042@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7149@modulo / bitwise operators`.
...
> `src/tychoc.c:6526-6532`, result at `src/tychoc.c:6958@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7117@rt`. Exhaustively pinned by
