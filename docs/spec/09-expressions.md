...
> Provenance: array ⊕ array arm `src/tychoc.c:7273-7303`; broadcast arm
> `src/tychoc.c:7332-7359`; the per-element-type operator set
> `src/tychoc.c:1460@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7881@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7998@modulo / bitwise operators`.
...
> `src/tychoc.c:7253-7259`, result at `src/tychoc.c:7796@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7965@rt`. Exhaustively pinned by
