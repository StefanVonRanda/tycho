...
> through to — shift `src/tychoc.c:6348@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6455@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6313@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6425@rt`. Exhaustively pinned by
