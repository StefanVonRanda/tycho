...
> Provenance: binary-op resolver `src/tychoc.c:6100-6356`; short-circuit
> lowering `:9455-9477`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6225-6255`; broadcast arm
> `src/tychoc.c:6284-6310`; the per-element-type operator set
> `src/tychoc.c:1344@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6697@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6804@modulo / bitwise operators`.
...
> `src/tychoc.c:6205-6211`, result at `src/tychoc.c:6613@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6772@rt`. Exhaustively pinned by
