...
> Provenance: binary-op resolver `src/tychoc.c:6508-6764`; short-circuit
> lowering `:10079-10101`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6633-6663`; broadcast arm
> `src/tychoc.c:6692-6718`; the per-element-type operator set
> `src/tychoc.c:1345@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7148@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7257@modulo / bitwise operators`.
...
> `src/tychoc.c:6613-6619`, result at `src/tychoc.c:7064@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7224@rt`. Exhaustively pinned by
