...
> Provenance: binary-op resolver `src/tychoc.c:6436-6692`; short-circuit
> lowering `:10007-10029`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6561-6591`; broadcast arm
> `src/tychoc.c:6620-6646`; the per-element-type operator set
> `src/tychoc.c:1307@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7076@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7185@modulo / bitwise operators`.
...
> `src/tychoc.c:6541-6547`, result at `src/tychoc.c:6992@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7152@rt`. Exhaustively pinned by
