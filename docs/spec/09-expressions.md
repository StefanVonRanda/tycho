...
> Provenance: binary-op resolver `src/tychoc.c:6114-6370`; short-circuit
> lowering `:9469-9491`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6239-6269`; broadcast arm
> `src/tychoc.c:6298-6324`; the per-element-type operator set
> `src/tychoc.c:1344@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6711@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6818@modulo / bitwise operators`.
...
> `src/tychoc.c:6219-6225`, result at `src/tychoc.c:6627@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6786@rt`. Exhaustively pinned by
