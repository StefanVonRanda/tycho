...
> Provenance: binary-op resolver `src/tychoc.c:6234-6490`; short-circuit
> lowering `:9654-9680`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6359-6389`; broadcast arm
> `src/tychoc.c:6418-6444`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6855@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6962@modulo / bitwise operators`.
...
> `src/tychoc.c:6339-6345`, result at `src/tychoc.c:6771@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6930@rt`. Exhaustively pinned by
