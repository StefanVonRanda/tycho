...
> Provenance: binary-op resolver `src/tychoc.c:7170-7427`; short-circuit
> lowering `:10906-10928`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:7295-7325`; broadcast arm
> `src/tychoc.c:7354-7381`; the per-element-type operator set
> `src/tychoc.c:1482@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7903@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:8020@modulo / bitwise operators`.
...
> `src/tychoc.c:7275-7281`, result at `src/tychoc.c:7818@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7987@rt`. Exhaustively pinned by
