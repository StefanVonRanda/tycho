...
> Provenance: binary-op resolver `src/tychoc.c:6603-6859`; short-circuit
> lowering `:10138-10164`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6728-6758`; broadcast arm
> `src/tychoc.c:6787-6813`; the per-element-type operator set
...
> modulo/bitwise `src/tychoc.c:7358@modulo / bitwise operators`.
...
> `src/tychoc.c:6708-6714`, result at `src/tychoc.c:7161@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7325@rt`. Exhaustively pinned by
