...
> Provenance: binary-op resolver `src/tychoc.c:7092-7349`; short-circuit
> lowering `:10864-10886`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:7217-7247`; broadcast arm
> `src/tychoc.c:7276-7303`; the per-element-type operator set
> `src/tychoc.c:1440@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7784@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7901@modulo / bitwise operators`.
...
> `src/tychoc.c:7197-7203`, result at `src/tychoc.c:7699@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7868@rt`. Exhaustively pinned by
