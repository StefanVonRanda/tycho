...
> Provenance: binary-op resolver `src/tychoc.c:6666-6922`; short-circuit
> lowering `:10392-10414`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6791-6821`; broadcast arm
> `src/tychoc.c:6850-6876`; the per-element-type operator set
> `src/tychoc.c:1418@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7353@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7462@modulo / bitwise operators`.
...
> `src/tychoc.c:6771-6777`, result at `src/tychoc.c:7268@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7429@rt`. Exhaustively pinned by
