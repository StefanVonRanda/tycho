...
> Provenance: binary-op resolver `src/tychoc.c:6278-6534`; short-circuit
> lowering `:9714-9740`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6403-6433`; broadcast arm
> `src/tychoc.c:6462-6488`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6899@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7006@modulo / bitwise operators`.
...
> `src/tychoc.c:6383-6389`, result at `src/tychoc.c:6815@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6974@rt`. Exhaustively pinned by
