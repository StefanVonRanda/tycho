...
> Provenance: binary-op resolver `src/tychoc.c:6225-6481`; short-circuit
> lowering `:9621-9647`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6350-6380`; broadcast arm
> `src/tychoc.c:6409-6435`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6846@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6953@modulo / bitwise operators`.
...
> `src/tychoc.c:6330-6336`, result at `src/tychoc.c:6762@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6921@rt`. Exhaustively pinned by
