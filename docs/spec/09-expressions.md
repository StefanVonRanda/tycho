...
> Provenance: binary-op resolver `src/tychoc.c:6802-7059`; short-circuit
> lowering `:10558-10580`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6927-6957`; broadcast arm
> `src/tychoc.c:6986-7013`; the per-element-type operator set
> `src/tychoc.c:1416@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7494@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7611@modulo / bitwise operators`.
...
> `src/tychoc.c:6907-6913`, result at `src/tychoc.c:7409@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7578@rt`. Exhaustively pinned by
