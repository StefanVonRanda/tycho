...
> Provenance: binary-op resolver `src/tychoc.c:6815-7072`; short-circuit
> lowering `:10567-10589`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6940-6970`; broadcast arm
> `src/tychoc.c:6999-7026`; the per-element-type operator set
> `src/tychoc.c:1432@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7503@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7620@modulo / bitwise operators`.
...
> `src/tychoc.c:6920-6926`, result at `src/tychoc.c:7418@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7587@rt`. Exhaustively pinned by
