...
> Provenance: array ⊕ array arm `src/tychoc.c:6327-6357`; broadcast arm
> `src/tychoc.c:6386-6412`; the per-element-type operator set
> `src/tychoc.c:1217@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6831@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6940@modulo / bitwise operators`.
...
> `src/tychoc.c:6307-6313`, result at `src/tychoc.c:6747@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6907@rt`. Exhaustively pinned by
