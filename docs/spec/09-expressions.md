...
> Provenance: binary-op resolver `src/tychoc.c:6628-6884`; short-circuit
> lowering `:10163-10189`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6753-6783`; broadcast arm
> `src/tychoc.c:6812-6838`; the per-element-type operator set
> `src/tychoc.c:1409@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7270@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7383@modulo / bitwise operators`.
...
> `src/tychoc.c:6733-6739`, result at `src/tychoc.c:7186@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7350@rt`. Exhaustively pinned by
