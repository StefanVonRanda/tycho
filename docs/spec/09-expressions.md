...
> through to — shift `src/tychoc.c:6505@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6612@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6421@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6580@rt`. Exhaustively pinned by
