...
> `src/tychoc.c:1024@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6114@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6222@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6116@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6221@rt`. Exhaustively pinned by
