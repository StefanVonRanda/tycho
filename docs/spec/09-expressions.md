...
> Provenance: binary-op resolver `src/tychoc.c:6419-6675`; short-circuit
> lowering `:9965-9987`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6544-6574`; broadcast arm
> `src/tychoc.c:6603-6629`; the per-element-type operator set
> `src/tychoc.c:1290@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7059@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7168@modulo / bitwise operators`.
...
> `src/tychoc.c:6524-6530`, result at `src/tychoc.c:6975@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7135@rt`. Exhaustively pinned by
