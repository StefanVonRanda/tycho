...
> Provenance: binary-op resolver `src/tychoc.c:6551-6807`; short-circuit
> lowering `:10245-10267`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6676-6706`; broadcast arm
> `src/tychoc.c:6735-6761`; the per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7199@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7308@modulo / bitwise operators`.
...
> `src/tychoc.c:6656-6662`, result at `src/tychoc.c:7115@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7275@rt`. Exhaustively pinned by
