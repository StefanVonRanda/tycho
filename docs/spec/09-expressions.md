...
> Provenance: binary-op resolver `src/tychoc.c:6231-6487`; short-circuit
> lowering `:9627-9653`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6356-6386`; broadcast arm
> `src/tychoc.c:6415-6441`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6852@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6959@modulo / bitwise operators`.
...
> `src/tychoc.c:6336-6342`, result at `src/tychoc.c:6768@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6927@rt`. Exhaustively pinned by
