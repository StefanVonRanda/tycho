...
> Provenance: binary-op resolver `src/tychoc.c:6580-6836`; short-circuit
> lowering `:10296-10318`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6705-6735`; broadcast arm
> `src/tychoc.c:6764-6790`; the per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7228@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7337@modulo / bitwise operators`.
...
For a **nullary** variant, `v == V` remains legal and equivalent — `==` is
structural over the whole value and a nullary variant carries nothing to differ
in ([§5.5](03-types.md#55-equality-and-ordering)). The two spellings genuinely
...
> `src/tychoc.c:6685-6691`, result at `src/tychoc.c:7144@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7304@rt`. Exhaustively pinned by
