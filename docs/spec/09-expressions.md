...
> Provenance: binary-op resolver `src/tychoc.c:6322-6578`; short-circuit
> lowering `:9758-9784`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6447-6477`; broadcast arm
> `src/tychoc.c:6506-6532`; the per-element-type operator set
> `src/tychoc.c:1382@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6943@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7050@modulo / bitwise operators`.
...
> `src/tychoc.c:6427-6433`, result at `src/tychoc.c:6859@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7018@rt`. Exhaustively pinned by
