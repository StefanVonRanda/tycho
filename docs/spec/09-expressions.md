...
> Provenance: binary-op resolver `src/tychoc.c:6728-6984`; short-circuit
> lowering `:10470-10492`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6853-6883`; broadcast arm
> `src/tychoc.c:6912-6938`; the per-element-type operator set
> `src/tychoc.c:1424@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7415@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7524@modulo / bitwise operators`.
...
> `src/tychoc.c:6833-6839`, result at `src/tychoc.c:7330@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7491@rt`. Exhaustively pinned by
