...
> Provenance: binary-op resolver `src/tychoc.c:6175-6431`; short-circuit
> lowering `:9571-9597`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6300-6330`; broadcast arm
> `src/tychoc.c:6359-6385`; the per-element-type operator set
> `src/tychoc.c:1345@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6796@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6903@modulo / bitwise operators`.
...
> `src/tychoc.c:6280-6286`, result at `src/tychoc.c:6712@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6871@rt`. Exhaustively pinned by
