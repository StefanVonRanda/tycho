...
> Provenance: binary-op resolver `src/tychoc.c:6183-6439`; short-circuit
> lowering `:9579-9605`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6308-6338`; broadcast arm
> `src/tychoc.c:6367-6393`; the per-element-type operator set
> `src/tychoc.c:1345@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6804@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6911@modulo / bitwise operators`.
...
> `src/tychoc.c:6288-6294`, result at `src/tychoc.c:6720@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6879@rt`. Exhaustively pinned by
