...
> Provenance: binary-op resolver `src/tychoc.c:6024-6280`; short-circuit
> lowering `:9355-9377`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6149-6179`; broadcast arm
> `src/tychoc.c:6208-6234`; the per-element-type operator set
> `src/tychoc.c:1298@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6610@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6717@modulo / bitwise operators`.
...
> `src/tychoc.c:6129-6135`, result at `src/tychoc.c:6526@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6685@rt`. Exhaustively pinned by
