...
> Provenance: binary-op resolver `src/tychoc.c:6300-6556`; short-circuit
> lowering `:9736-9762`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6425-6455`; broadcast arm
> `src/tychoc.c:6484-6510`; the per-element-type operator set
> `src/tychoc.c:1360@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6921@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7028@modulo / bitwise operators`.
...
> `src/tychoc.c:6405-6411`, result at `src/tychoc.c:6837@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6996@rt`. Exhaustively pinned by
