...
> `src/tychoc.c:1203@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6425@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6532@modulo / bitwise operators`.
...
> the one that requires a match: `src/tychoc.c:6500@rt`. Exhaustively pinned by
