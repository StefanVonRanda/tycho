...
> Provenance: binary-op resolver `src/tychoc.c:6853-7110`; short-circuit
> lowering `:10609-10631`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6978-7008`; broadcast arm
> `src/tychoc.c:7037-7064`; the per-element-type operator set
> `src/tychoc.c:1417@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7545@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7662@modulo / bitwise operators`.
...
> `src/tychoc.c:6958-6964`, result at `src/tychoc.c:7460@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7629@rt`. Exhaustively pinned by
