...
> Provenance: binary-op resolver `src/tychoc.c:6406-6662`; short-circuit
> lowering `:9842-9868`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6531-6561`; broadcast arm
> `src/tychoc.c:6590-6616`; the per-element-type operator set
> `src/tychoc.c:1382@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7027@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7134@modulo / bitwise operators`.
...
> `src/tychoc.c:6511-6517`, result at `src/tychoc.c:6943@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7102@rt`. Exhaustively pinned by
