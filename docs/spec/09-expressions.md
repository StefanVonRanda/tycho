...
> Provenance: binary-op resolver `src/tychoc.c:6470-6726`; short-circuit
> lowering `:10041-10063`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6595-6625`; broadcast arm
> `src/tychoc.c:6654-6680`; the per-element-type operator set
> `src/tychoc.c:1307@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7110@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7219@modulo / bitwise operators`.
...
> `src/tychoc.c:6575-6581`, result at `src/tychoc.c:7026@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7186@rt`. Exhaustively pinned by
