...
> Provenance: binary-op resolver `src/tychoc.c:6492-6748`; short-circuit
> lowering `:9991-10017`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6617-6647`; broadcast arm
> `src/tychoc.c:6676-6702`; the per-element-type operator set
> `src/tychoc.c:1393@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7133@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7246@modulo / bitwise operators`.
...
> `src/tychoc.c:6597-6603`, result at `src/tychoc.c:7049@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7213@rt`. Exhaustively pinned by
