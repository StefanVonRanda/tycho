...
> `src/tychoc.c:1174@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6364@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6471@modulo / bitwise operators`.
...
> `src/tychoc.c:6103-6109`, result at `src/tychoc.c:6329@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6441@rt`. Exhaustively pinned by
