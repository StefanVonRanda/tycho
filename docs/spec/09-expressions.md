...
> Provenance: binary-op resolver `src/tychoc.c:6824-7081`; short-circuit
> lowering `:10580-10602`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6949-6979`; broadcast arm
> `src/tychoc.c:7008-7035`; the per-element-type operator set
> `src/tychoc.c:1438@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7516@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7633@modulo / bitwise operators`.
...
> `src/tychoc.c:6929-6935`, result at `src/tychoc.c:7431@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7600@rt`. Exhaustively pinned by
