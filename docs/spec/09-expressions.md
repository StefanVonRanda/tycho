...
> Provenance: binary-op resolver `src/tychoc.c:7112-7369`; short-circuit
...
> Provenance: array ⊕ array arm `src/tychoc.c:7237-7267`; broadcast arm
> `src/tychoc.c:7296-7323`; the per-element-type operator set
> `src/tychoc.c:1460@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7845@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7962@modulo / bitwise operators`.
...
> `src/tychoc.c:7217-7223`, result at `src/tychoc.c:7760@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7929@rt`. Exhaustively pinned by
