...
> Provenance: binary-op resolver `src/tychoc.c:6190-6446`; short-circuit
> lowering `:9586-9612`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6315-6345`; broadcast arm
> `src/tychoc.c:6374-6400`; the per-element-type operator set
> `src/tychoc.c:1352@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:6811@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:6918@modulo / bitwise operators`.
...
> `src/tychoc.c:6295-6301`, result at `src/tychoc.c:6727@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:6886@rt`. Exhaustively pinned by
