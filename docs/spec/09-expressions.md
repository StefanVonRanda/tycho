...
> Provenance: binary-op resolver `src/tychoc.c:7184-7447`; short-circuit
> lowering `:10986-11008`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:7309-7345`; broadcast arm
> `src/tychoc.c:7374-7401`; the per-element-type operator set
> `src/tychoc.c:1488@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7946@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:8063@modulo / bitwise operators`.
...
> `src/tychoc.c:7289-7295`, result at `src/tychoc.c:7861@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:8030@rt`. Exhaustively pinned by
