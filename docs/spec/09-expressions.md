...
> Provenance: binary-op resolver `src/tychoc.c:6589-6845`; short-circuit
> lowering `:10313-10335`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6714-6744`; broadcast arm
> `src/tychoc.c:6773-6799`; the per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7245@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7354@modulo / bitwise operators`.
...
> `src/tychoc.c:6694-6700`, result at `src/tychoc.c:7161@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7321@rt`. Exhaustively pinned by
