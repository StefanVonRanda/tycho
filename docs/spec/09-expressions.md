...
> Provenance: binary-op resolver `src/tychoc.c:6569-6825`; short-circuit
> lowering `:10264-10286`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6694-6724`; broadcast arm
> `src/tychoc.c:6753-6779`; the per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7217@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7326@modulo / bitwise operators`.
...
> `src/tychoc.c:6674-6680`, result at `src/tychoc.c:7133@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7293@rt`. Exhaustively pinned by
