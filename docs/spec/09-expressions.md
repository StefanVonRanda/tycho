...
> Provenance: binary-op resolver `src/tychoc.c:6636-6892`; short-circuit
> lowering `:10171-10197`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6761-6791`; broadcast arm
> `src/tychoc.c:6820-6846`; the per-element-type operator set
> `src/tychoc.c:1409@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7278@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7391@modulo / bitwise operators`.
...
> `src/tychoc.c:6741-6747`, result at `src/tychoc.c:7194@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7358@rt`. Exhaustively pinned by
