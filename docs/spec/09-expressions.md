...
> Provenance: binary-op resolver `src/tychoc.c:7170-7433`; short-circuit
> lowering `:10940-10962`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:7295-7331`; broadcast arm
> `src/tychoc.c:7360-7387`; the per-element-type operator set
> `src/tychoc.c:1482@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7932@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:8049@modulo / bitwise operators`.
...
> `src/tychoc.c:7275-7281`, result at `src/tychoc.c:7847@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:8016@rt`. Exhaustively pinned by
