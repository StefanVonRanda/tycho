...
> Provenance: binary-op resolver `src/tychoc.c:6778-7035`; short-circuit
> lowering `:10522-10544`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6903-6933`; broadcast arm
> `src/tychoc.c:6962-6989`; the per-element-type operator set
> `src/tychoc.c:1424@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7466@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7575@modulo / bitwise operators`.
...
> `src/tychoc.c:6883-6889`, result at `src/tychoc.c:7381@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7542@rt`. Exhaustively pinned by
