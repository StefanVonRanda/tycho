...
> Provenance: binary-op resolver `src/tychoc.c:6517-6773`; short-circuit
> lowering `:10016-10042`; value-control `parse_value_ctrl`/`ctrl_rewrite_tails`;
...
> Provenance: array ⊕ array arm `src/tychoc.c:6642-6672`; broadcast arm
> `src/tychoc.c:6701-6727`; the per-element-type operator set
> `src/tychoc.c:1393@elem_arith_ok`; the arms an array operand still falls
> through to — shift `src/tychoc.c:7158@shift operators require integer operands`,
> modulo/bitwise `src/tychoc.c:7271@modulo / bitwise operators`.
...
> `src/tychoc.c:6622-6628`, result at `src/tychoc.c:7074@lt`. The bitwise arm is
> the one that requires a match: `src/tychoc.c:7238@rt`. Exhaustively pinned by
