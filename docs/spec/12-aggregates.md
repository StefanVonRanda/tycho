...
> `pop`-empty abort `:13358@pop from an empty array`,`:13557@pop from an empty array`; `reserve` `:6282-6308`,`:9660-9666`; tuple
> arity `:2410@a tuple has at most 8 elements`,`:2414@a tuple type needs at least two elements`, index `:5196-5204`; destructuring `:3505-3519`,`:7253-7269`;
> map read (pure `map_get`, no insert) `:5507-5522`; map place insert+zero
> `:10529-10538`; `keys()` insertion order — the walk `:13516@m.elive[e]` over the append-only entries array `:13471@m->ecount++`; `delete` → `map_del`
> `:3353-3377`,`:6205-6211`; subscript parse + rules `:4036-4088`, dispatch
> `:4110-4118`; `or_return` `:5383-5400`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13358@pop`); it is not
...
> Provenance: two-array arm `src/tychoc.c:6578-6608`, broadcast arm
> `src/tychoc.c:6637-6663`; per-element-type operator set
> `src/tychoc.c:1393@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7116@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7108@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7098@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7101@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7174@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6648-6653`; the fresh spine
> `src/tychoc.c:10748@arena_alloc`, the per-element emit shared with the scalar
...
9-element tuple is a compile error, `src/tychoc.c:5822@least` (min) and `:5823@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5379-5381`).
...
destructuring list (`src/tychoc.c:7563-7585`). At most 8 targets are permitted.
...
(`src/tychoc.c:5507-5522`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3350-3366`,`:6203-6209`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12627-12630`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5391-5399`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5385-5389`).
