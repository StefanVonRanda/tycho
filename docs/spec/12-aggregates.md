...
> `pop`-empty abort `:12834@pop from an empty array`,`:13023@pop from an empty array`; `reserve` `:5943-5969`,`:9211-9217`; tuple
...
> `:10059-10068`; `keys()` insertion order — the walk `:12992@m.elive[e]` over the append-only entries array `:12947@m->ecount++`; `delete` → `map_del`
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:11974`); it is not
...
> `src/tychoc.c:10278@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10262@gen_arith_op`, operands never reordered
> `src/tychoc.c:10259@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10282@tycho_ew_len`, and the abort
...
the order in which each key was first inserted (`src/tychoc.c:12107-12110`; the
