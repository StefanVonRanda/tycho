...
> Provenance: array element restriction `src/tychoc.c:2298-2299`,`:2315-2316`;
> `pop`-empty abort `:13918@pop from an empty array`,`:13918@pop from an empty array`; `reserve` `:6914-6940`,`:10351-10357`; tuple
> arity `:2499@a tuple has at most 8 elements`,`:2504@a tuple type needs at least two elements`, index `:5590-5598`; destructuring `:3771-3785`,`:8425-8441`;
> map read (pure `map_get`, no insert) `:6097-6112`; map place insert+zero
> `:11203-11212`; `keys()` insertion order — the walk `:14080@m.elive[e]` over the append-only entries array `:13832@m->ecount++`; `delete` → `map_del`
> `:3619-3643`,`:6837-6843`; subscript parse + rules `:4223-4275`, dispatch
> `:4298-4306`; `or_return` `:5950-5967`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13719@pop`); it is not
...
> `src/tychoc.c:2581@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2599@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:2155-2156` (`[$N]T`) and `src/tychoc.c:2177-2178` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:2068-2069`. Pinned both ways by
...
(`src/tychoc.c:1439@TK_PERCENT` rather than the `char` arm at
`:1443@et == T_CHAR`), which is why it has a fixture of its own.
...
> Provenance: two-array arm `src/tychoc.c:6949-6979`, broadcast arm
> `src/tychoc.c:7008-7035`; per-element-type operator set
> `src/tychoc.c:1438@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7566@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7558@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7540@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7551@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7620@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:7019-7025`; the fresh spine
> `src/tychoc.c:11106@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:11070@gen_arith_op`, operands never reordered
> `src/tychoc.c:11067@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:11110@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:3190@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7508@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2504@least` (min) and `:2054@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5670-5672`).
...
destructuring list (`src/tychoc.c:7993-8016`). At most 8 targets are permitted.
...
(`src/tychoc.c:5801-5816`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3616-3632`,`:6835-6841`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12924-12927`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5588-5596`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5679-5683`).
