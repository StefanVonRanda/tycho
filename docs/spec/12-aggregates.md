...
> `pop`-empty abort `:12830@pop from an empty array`,`:13019@pop from an empty array`; `reserve` `:5943-5969`,`:9211-9217`; tuple
> arity `:2307@a tuple has at most 8 elements`,`:2311@a tuple type needs at least two elements`, index `:4923-4931`; destructuring `:3363-3377`,`:6884-6900`;
> map read (pure `map_get`, no insert) `:5226-5241`; map place insert+zero
> `:10055-10064`; `keys()` insertion order — the walk `:12988@m.elive[e]` over the append-only entries array `:12943@m->ecount++`; `delete` → `map_del`
> `:3211-3235`,`:5872-5878`; subscript parse + rules `:3848-3900`, dispatch
> `:3922-3930`; `or_return` `:5102-5119`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:11970`); it is not
...
> Provenance: two-array arm `src/tychoc.c:6239-6269`, broadcast arm
> `src/tychoc.c:6298-6324`; per-element-type operator set
> `src/tychoc.c:1344@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6753@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6745@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6735@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6738@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6806@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6309-6314`; the fresh spine
> `src/tychoc.c:10274@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10258@gen_arith_op`, operands never reordered
> `src/tychoc.c:10255@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10278@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2920@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6703@division by zero`.
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5098-5100`).
...
destructuring list (`src/tychoc.c:7172-7194`). At most 8 targets are permitted.
...
(`src/tychoc.c:5226-5241`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3208-3224`,`:5870-5876`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12103-12106`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5110-5118`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5104-5108`).
