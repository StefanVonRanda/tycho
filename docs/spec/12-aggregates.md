...
> `pop`-empty abort `:12031@pop from an empty array`,`:12197@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
...
> `:9896-9905`; `keys()` insertion order — the walk `:12166@m.elive[e]` over the append-only entries array `:12121@m->ecount++`; `delete` → `map_del`
...
> `src/tychoc.c:6326@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6318@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6308@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6311@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6380@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6193-6198`; the fresh spine
> `src/tychoc.c:9639@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9623@gen_arith_op`, operands never reordered
> `src/tychoc.c:9620@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9643@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2540@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6276@division by zero`.
