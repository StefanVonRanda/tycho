...
> Provenance: array element restriction `src/tychoc.c:2017-2018`,`:2034-2035`;
> `pop`-empty abort `:13257@pop from an empty array`,`:13257@pop from an empty array`; `reserve` `:6402-6428`,`:9736-9742`; tuple
> arity `:2285@a tuple has at most 8 elements`,`:2289@a tuple type needs at least two elements`, index `:5145-5153`; destructuring `:3431-3445`,`:7853-7869`;
> map read (pure `map_get`, no insert) `:5629-5644`; map place insert+zero
> `:10541-10550`; `keys()` insertion order — the walk `:13419@m.elive[e]` over the append-only entries array `:13171@m->ecount++`; `delete` → `map_del`
> `:3279-3303`,`:6325-6331`; subscript parse + rules `:3883-3935`, dispatch
> `:3958-3966`; `or_return` `:5505-5522`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13058@pop`); it is not
...
> `src/tychoc.c:2354@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2372@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:1995-1996` (`[$N]T`) and `src/tychoc.c:2017-2018` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:1908-1909`. Pinned both ways by
...
(`src/tychoc.c:1291@TK_PERCENT` rather than the `char` arm at
`:1295@et == T_CHAR`), which is why it has a fixture of its own.
...
> Provenance: two-array arm `src/tychoc.c:6544-6574`, broadcast arm
> `src/tychoc.c:6603-6629`; per-element-type operator set
> `src/tychoc.c:1290@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7101@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7093@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7083@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7086@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7155@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6614-6619`; the fresh spine
> `src/tychoc.c:10521@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10505@gen_arith_op`, operands never reordered
> `src/tychoc.c:10502@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10525@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2838@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7051@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2289@least` (min) and `:2269@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5332-5334`).
...
destructuring list (`src/tychoc.c:7528-7551`). At most 8 targets are permitted.
...
(`src/tychoc.c:5463-5478`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3276-3292`,`:6323-6329`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12339-12342`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5250-5258`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5341-5345`).
