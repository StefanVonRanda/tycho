...
> `pop`-empty abort `:13318@pop from an empty array`,`:13517@pop from an empty array`; `reserve` `:6264-6290`,`:9632-9638`; tuple
...
> `:10501-10510`; `keys()` insertion order — the walk `:13476@m.elive[e]` over the append-only entries array `:13431@m->ecount++`; `delete` → `map_del`
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13318@pop`); it is not
...
> `src/tychoc.c:10720@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10704@gen_arith_op`, operands never reordered
> `src/tychoc.c:10701@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10724@tycho_ew_len`, and the abort
...
the order in which each key was first inserted (`src/tychoc.c:12587-12590`; the
