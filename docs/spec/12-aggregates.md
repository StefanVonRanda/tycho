...
> `pop`-empty abort `:13285@pop from an empty array`,`:13484@pop from an empty array`; `reserve` `:6250-6276`,`:9599-9605`; tuple
> arity `:2409@a tuple has at most 8 elements`,`:2413@a tuple type needs at least two elements`, index `:5164-5172`; destructuring `:3504-3518`,`:7215-7231`;
> map read (pure `map_get`, no insert) `:5475-5490`; map place insert+zero
> `:10468-10477`; `keys()` insertion order — the walk `:13443@m.elive[e]` over the append-only entries array `:13398@m->ecount++`; `delete` → `map_del`
> `:3352-3376`,`:6173-6179`; subscript parse + rules `:4004-4056`, dispatch
> `:4078-4086`; `or_return` `:5351-5368`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13285@pop`); it is not
...
> Provenance: two-array arm `src/tychoc.c:6546-6576`, broadcast arm
> `src/tychoc.c:6605-6631`; per-element-type operator set
> `src/tychoc.c:1393@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7084@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7076@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7066@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7069@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7137@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6616-6621`; the fresh spine
> `src/tychoc.c:10687@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10671@gen_arith_op`, operands never reordered
> `src/tychoc.c:10668@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10691@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2977@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7034@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:5790@least` (min) and `:5791@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5347-5349`).
...
destructuring list (`src/tychoc.c:7525-7547`). At most 8 targets are permitted.
...
(`src/tychoc.c:5475-5490`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3349-3365`,`:6171-6177`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12554-12557`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5359-5367`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5353-5357`).
