...
> `pop`-empty abort `:11993@pop from an empty array`,`:12159@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
> arity `:2081@a tuple has at most 8 elements`,`:2085@a tuple type needs at least two elements`, index `:4823-4831`; destructuring `:3282-3296`,`:6757-6773`;
> map read (pure `map_get`, no insert) `:5126-5141`; map place insert+zero
> `:9896-9905`; `keys()` insertion order — the walk `:12128@m.elive[e]` over the append-only entries array `:12083@m->ecount++`; `delete` → `map_del`
...
> `src/tychoc.c:2156@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2157@void`; docs/internals/plan-loops-cleanup-DONE.md phase 54). The fixed forms test both:
...
(`src/tychoc.c:1146@TK_PERCENT` rather than the `char` arm at
`:1150@et == T_CHAR`), which is why it has a fixture of its own.
...
> `src/tychoc.c:1145@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6298@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6290@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6280@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6283@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6352@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6193-6198`; the fresh spine
> `src/tychoc.c:9611@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9595@gen_arith_op`, operands never reordered
> `src/tychoc.c:9592@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9615@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2540@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6248@division by zero`.
