...
> `pop`-empty abort `:13459@pop from an empty array`,`:13459@pop from an empty array`; `reserve` `:6648-6674`,`:10023-10029`; tuple
> arity `:2365@a tuple has at most 8 elements`,`:2370@a tuple type needs at least two elements`, index `:5391-5399`; destructuring `:3631-3645`,`:8115-8131`;
> map read (pure `map_get`, no insert) `:5875-5890`; map place insert+zero
> `:10828-10837`; `keys()` insertion order — the walk `:13621@m.elive[e]` over the append-only entries array `:13373@m->ecount++`; `delete` → `map_del`
> `:3479-3503`,`:6571-6577`; subscript parse + rules `:4083-4135`, dispatch
> `:4158-4166`; `or_return` `:5751-5768`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13260@pop`); it is not
...
> `src/tychoc.c:2447@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2465@void`; the loops-cleanup plan). The fixed forms test both:
...
> Provenance: two-array arm `src/tychoc.c:6683-6713`, broadcast arm
> `src/tychoc.c:6742-6768`; per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7248@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7240@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7230@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7233@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7302@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6753-6758`; the fresh spine
> `src/tychoc.c:10701@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10685@gen_arith_op`, operands never reordered
> `src/tychoc.c:10682@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10705@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2838@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7198@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2370@least` (min) and `:1974@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5471-5473`).
...
destructuring list (`src/tychoc.c:7683-7706`). At most 8 targets are permitted.
...
(`src/tychoc.c:5602-5617`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3476-3492`,`:6569-6575`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12541-12544`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5389-5397`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5480-5484`).
