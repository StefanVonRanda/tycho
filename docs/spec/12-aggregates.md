...
> `pop`-empty abort `:12404@pop from an empty array`,`:12570@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
...
> `:9896-9905`; `keys()` insertion order — the walk `:12539@m.elive[e]` over the append-only entries array `:12494@m->ecount++`; `delete` → `map_del`
...
> `src/tychoc.c:9913@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9897@gen_arith_op`, operands never reordered
> `src/tychoc.c:9894@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9917@tycho_ew_len`, and the abort
