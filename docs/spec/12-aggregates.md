...
> `pop`-empty abort `:12322@pop from an empty array`,`:12488@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
...
> `:9896-9905`; `keys()` insertion order — the walk `:12457@m.elive[e]` over the append-only entries array `:12412@m->ecount++`; `delete` → `map_del`
...
> `src/tychoc.c:2216@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2217@void`; the loops-cleanup plan). The fixed forms test both:
...
> `src/tychoc.c:6429@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6421@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6411@IS_BOUNDED`; element-type mismatch
...
> `src/tychoc.c:9834@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9818@gen_arith_op`, operands never reordered
> `src/tychoc.c:9815@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9838@tycho_ew_len`, and the abort
