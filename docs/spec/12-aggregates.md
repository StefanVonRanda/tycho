...
> `pop`-empty abort `:12495@pop from an empty array`,`:12684@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
...
> `:9896-9905`; `keys()` insertion order — the walk `:12653@m.elive[e]` over the append-only entries array `:12608@m->ecount++`; `delete` → `map_del`
...
> `src/tychoc.c:2231@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2249@void`; the loops-cleanup plan). The fixed forms test both:
...
(`src/tychoc.c:1220@TK_PERCENT` rather than the `char` arm at
`:1224@et == T_CHAR`), which is why it has a fixture of its own.
...
> `src/tychoc.c:1219@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6489@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6481@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6471@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6474@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6542@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6193-6198`; the fresh spine
> `src/tychoc.c:9955@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9939@gen_arith_op`, operands never reordered
> `src/tychoc.c:9936@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9959@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2886@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6439@division by zero`.
