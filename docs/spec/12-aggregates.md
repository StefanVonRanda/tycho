...
> `pop`-empty abort `:12854@pop from an empty array`,`:13043@pop from an empty array`; `reserve` `:5963-5989`,`:9231-9237`; tuple
> arity `:2307@a tuple has at most 8 elements`,`:2311@a tuple type needs at least two elements`, index `:4930-4938`; destructuring `:3370-3384`,`:6904-6920`;
> map read (pure `map_get`, no insert) `:5233-5248`; map place insert+zero
> `:10079-10088`; `keys()` insertion order — the walk `:13012@m.elive[e]` over the append-only entries array `:12967@m->ecount++`; `delete` → `map_del`
> `:3218-3242`,`:5892-5898`; subscript parse + rules `:3855-3907`, dispatch
> `:3929-3937`; `or_return` `:5109-5126`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:11994`); it is not
...
> `src/tychoc.c:2372@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2390@void`; the loops-cleanup plan). The fixed forms test both:
...
> Provenance: two-array arm `src/tychoc.c:6259-6289`, broadcast arm
> `src/tychoc.c:6318-6344`; per-element-type operator set
> `src/tychoc.c:1344@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6773@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6765@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6755@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6758@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6826@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6329-6334`; the fresh spine
> `src/tychoc.c:10298@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10282@gen_arith_op`, operands never reordered
> `src/tychoc.c:10279@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10302@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2920@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6723@division by zero`.
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5105-5107`).
...
destructuring list (`src/tychoc.c:7192-7214`). At most 8 targets are permitted.
...
(`src/tychoc.c:5233-5248`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3215-3231`,`:5890-5896`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12127-12130`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5117-5125`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5111-5115`).
