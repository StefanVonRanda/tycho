...
> `pop`-empty abort `:13570@pop from an empty array`,`:13769@pop from an empty array`; `reserve` `:6415-6441`,`:9870-9876`; tuple
...
> `:10739-10748`; `keys()` insertion order — the walk `:13728@m.elive[e]` over the append-only entries array `:13683@m->ecount++`; `delete` → `map_del`
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13570@pop`); it is not
...
> `src/tychoc.c:10958@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10942@gen_arith_op`, operands never reordered
> `src/tychoc.c:10939@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10962@tycho_ew_len`, and the abort
...
the order in which each key was first inserted (`src/tychoc.c:12837-12840`; the
