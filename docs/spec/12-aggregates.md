...
> `pop`-empty abort `:12966@pop from an empty array`,`:13155@pop from an empty array`; `reserve` `:6004-6030`,`:9313-9319`; tuple
> arity `:2309@a tuple has at most 8 elements`,`:2313@a tuple type needs at least two elements`, index `:4963-4971`; destructuring `:3403-3417`,`:6969-6985`;
> map read (pure `map_get`, no insert) `:5274-5289`; map place insert+zero
> `:10161-10170`; `keys()` insertion order — the walk `:13124@m.elive[e]` over the append-only entries array `:13079@m->ecount++`; `delete` → `map_del`
> `:3251-3275`,`:5933-5939`; subscript parse + rules `:3888-3940`, dispatch
> `:3962-3970`; `or_return` `:5150-5167`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:12106`); it is not
...
> Provenance: two-array arm `src/tychoc.c:6300-6330`, broadcast arm
> `src/tychoc.c:6359-6385`; per-element-type operator set
> `src/tychoc.c:1345@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6838@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6830@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6820@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6823@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6891@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6370-6375`; the fresh spine
> `src/tychoc.c:10380@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10364@gen_arith_op`, operands never reordered
> `src/tychoc.c:10361@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10384@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2920@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6788@division by zero`.
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5146-5148`).
...
destructuring list (`src/tychoc.c:7257-7279`). At most 8 targets are permitted.
...
(`src/tychoc.c:5274-5289`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3248-3264`,`:5931-5937`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12239-12242`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5158-5166`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5152-5156`).
