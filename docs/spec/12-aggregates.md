...
> `pop`-empty abort `:14347@pop from an empty array`,`:14347@pop from an empty array`; `reserve` `:7224-7250`,`:10677-10683`; tuple
> arity `:2563@a tuple has at most 8 elements`,`:2568@a tuple type needs at least two elements`, index `:5864-5872`; destructuring `:3891-3905`,`:8735-8751`;
> map read (pure `map_get`, no insert) `:6386-6401`; map place insert+zero
> `:11529-11538`; `keys()` insertion order — the walk `:14509@m.elive[e]` over the append-only entries array `:14261@m->ecount++`; `delete` → `map_del`
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:14146@pop`); it is not
...
> Provenance: two-array arm `src/tychoc.c:7295-7325`, broadcast arm
> `src/tychoc.c:7354-7381`; per-element-type operator set
> `src/tychoc.c:1482@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7953@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7945@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7927@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7938@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:8007@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:7365-7371`; the fresh spine
> `src/tychoc.c:11509@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:11473@gen_arith_op`, operands never reordered
> `src/tychoc.c:11470@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:11513@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:3205@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7895@division by zero`.
...
destructuring list (`src/tychoc.c:8339-8362`). At most 8 targets are permitted.
...
the order in which each key was first inserted (`src/tychoc.c:13293-13296`; the
