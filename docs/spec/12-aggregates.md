...
> `pop`-empty abort `:13422@pop from an empty array`,`:13422@pop from an empty array`; `reserve` `:6641-6667`,`:10008-10014`; tuple
> arity `:2365@a tuple has at most 8 elements`,`:2369@a tuple type needs at least two elements`, index `:5384-5392`; destructuring `:3624-3638`,`:8100-8116`;
> map read (pure `map_get`, no insert) `:5868-5883`; map place insert+zero
> `:10813-10822`; `keys()` insertion order — the walk `:13584@m.elive[e]` over the append-only entries array `:13336@m->ecount++`; `delete` → `map_del`
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13223@pop`); it is not
...
> `src/tychoc.c:10686@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10670@gen_arith_op`, operands never reordered
> `src/tychoc.c:10667@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10690@tycho_ew_len`, and the abort
...
destructuring list (`src/tychoc.c:7668-7691`). At most 8 targets are permitted.
...
the order in which each key was first inserted (`src/tychoc.c:12504-12507`; the
