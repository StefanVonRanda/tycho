...
> `pop`-empty abort `:12490@pop from an empty array`,`:12679@pop from an empty array`; `reserve` `:5827-5853`,`:9052-9058`; tuple
> arity `:2168@a tuple has at most 8 elements`,`:2172@a tuple type needs at least two elements`, index `:4823-4831`; destructuring `:3282-3296`,`:6757-6773`;
> map read (pure `map_get`, no insert) `:5126-5141`; map place insert+zero
> `:9896-9905`; `keys()` insertion order — the walk `:12648@m.elive[e]` over the append-only entries array `:12603@m->ecount++`; `delete` → `map_del`
...
> instead of an allow-list (`src/tychoc.c:2244@void`; the loops-cleanup plan). The fixed forms test both:
...
(`src/tychoc.c:1215@TK_PERCENT` rather than the `char` arm at
`:1219@et == T_CHAR`), which is why it has a fixture of its own.
...
> `src/tychoc.c:1214@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6484@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6476@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6466@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6469@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6537@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6193-6198`; the fresh spine
> `src/tychoc.c:9950@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9934@gen_arith_op`, operands never reordered
> `src/tychoc.c:9931@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9954@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2886@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6434@division by zero`.
