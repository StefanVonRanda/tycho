...
> `pop`-empty abort `:13566@pop from an empty array`,`:13765@pop from an empty array`; `reserve` `:6415-6441`,`:9866-9872`; tuple
> arity `:2433@a tuple has at most 8 elements`,`:2437@a tuple type needs at least two elements`, index `:5320-5328`; destructuring `:3578-3592`,`:7414-7430`;
> map read (pure `map_get`, no insert) `:5631-5646`; map place insert+zero
> `:10735-10744`; `keys()` insertion order — the walk `:13724@m.elive[e]` over the append-only entries array `:13679@m->ecount++`; `delete` → `map_del`
> `:3426-3450`,`:6338-6344`; subscript parse + rules `:4109-4161`, dispatch
> `:4183-4191`; `or_return` `:5507-5524`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13566@pop`); it is not
...
> `src/tychoc.c:2509@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2527@void`; the loops-cleanup plan). The fixed forms test both:
...
> Provenance: two-array arm `src/tychoc.c:6718-6748`, broadcast arm
> `src/tychoc.c:6777-6803`; per-element-type operator set
> `src/tychoc.c:1399@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7277@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7269@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7259@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7262@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7335@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6788-6793`; the fresh spine
> `src/tychoc.c:10954@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10938@gen_arith_op`, operands never reordered
> `src/tychoc.c:10935@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10958@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2999@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7227@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:5946@least` (min) and `:5947@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5503-5505`).
...
destructuring list (`src/tychoc.c:7724-7746`). At most 8 targets are permitted.
...
(`src/tychoc.c:5631-5646`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3423-3439`,`:6336-6342`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12833-12836`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5515-5523`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5509-5513`).
