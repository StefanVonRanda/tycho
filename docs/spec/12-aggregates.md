...
> Provenance: array element restriction `src/tychoc.c:2198-2199`,`:2215-2216`;
> `pop`-empty abort `:13452@pop from an empty array`,`:13452@pop from an empty array`; `reserve` `:6641-6667`,`:10016-10022`; tuple
> arity `:2365@a tuple has at most 8 elements`,`:2369@a tuple type needs at least two elements`, index `:5384-5392`; destructuring `:3624-3638`,`:8108-8124`;
> map read (pure `map_get`, no insert) `:5868-5883`; map place insert+zero
> `:10821-10830`; `keys()` insertion order — the walk `:13614@m.elive[e]` over the append-only entries array `:13366@m->ecount++`; `delete` → `map_del`
> `:3472-3496`,`:6564-6570`; subscript parse + rules `:4076-4128`, dispatch
> `:4151-4159`; `or_return` `:5744-5761`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13253@pop`); it is not
...
> `src/tychoc.c:2445@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2463@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:2075-2076` (`[$N]T`) and `src/tychoc.c:2097-2098` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:1988-1989`. Pinned both ways by
...
> Provenance: two-array arm `src/tychoc.c:6676-6706`, broadcast arm
> `src/tychoc.c:6735-6761`; per-element-type operator set
> `src/tychoc.c:1370@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7241@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7233@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7223@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7226@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7295@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6746-6751`; the fresh spine
> `src/tychoc.c:10694@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10678@gen_arith_op`, operands never reordered
> `src/tychoc.c:10675@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10698@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2838@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7191@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2369@least` (min) and `:1974@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5464-5466`).
...
destructuring list (`src/tychoc.c:7676-7699`). At most 8 targets are permitted.
...
(`src/tychoc.c:5595-5610`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3469-3485`,`:6562-6568`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12534-12537`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5382-5390`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5473-5477`).
