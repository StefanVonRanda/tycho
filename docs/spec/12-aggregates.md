...
> `src/tychoc.c:6406@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6398@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6388@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6391@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6459@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6193-6198`; the fresh spine
> `src/tychoc.c:9811@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:9795@gen_arith_op`, operands never reordered
> `src/tychoc.c:9792@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:9815@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2540@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6356@division by zero`.
