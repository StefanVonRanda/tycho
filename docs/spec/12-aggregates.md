...
> `pop`-empty abort `:13541@pop from an empty array`,`:13740@pop from an empty array`; `reserve` `:6403-6429`,`:9841-9847`; tuple
> arity `:2433@a tuple has at most 8 elements`,`:2437@a tuple type needs at least two elements`, index `:5308-5316`; destructuring `:3578-3592`,`:7390-7406`;
> map read (pure `map_get`, no insert) `:5619-5634`; map place insert+zero
> `:10710-10719`; `keys()` insertion order — the walk `:13699@m.elive[e]` over the append-only entries array `:13654@m->ecount++`; `delete` → `map_del`
> `:3426-3450`,`:6326-6332`; subscript parse + rules `:4109-4161`, dispatch
> `:4183-4191`; `or_return` `:5495-5512`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13541@pop`); it is not
...
> Provenance: two-array arm `src/tychoc.c:6699-6729`, broadcast arm
> `src/tychoc.c:6758-6784`; per-element-type operator set
> `src/tychoc.c:1399@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7253@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7245@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7235@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7238@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7311@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6769-6774`; the fresh spine
> `src/tychoc.c:10929@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10913@gen_arith_op`, operands never reordered
> `src/tychoc.c:10910@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10933@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2999@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7203@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:5934@least` (min) and `:5935@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5491-5493`).
...
destructuring list (`src/tychoc.c:7700-7722`). At most 8 targets are permitted.
...
map-delete rebinding the map (`src/tychoc.c:3423-3439`,`:6324-6330`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12808-12811`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5503-5511`).
