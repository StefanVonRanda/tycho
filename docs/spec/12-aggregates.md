...
> Provenance: array element restriction `src/tychoc.c:2063-2064`,`:2080-2081`;
> `pop`-empty abort `:12816@pop from an empty array`,`:13005@pop from an empty array`; `reserve` `:5929-5955`,`:9197-9203`; tuple
> arity `:2307@a tuple has at most 8 elements`,`:2311@a tuple type needs at least two elements`, index `:4909-4917`; destructuring `:3349-3363`,`:6870-6886`;
> map read (pure `map_get`, no insert) `:5212-5227`; map place insert+zero
> `:10041-10050`; `keys()` insertion order — the walk `:12974@m.elive[e]` over the append-only entries array `:12929@m->ecount++`; `delete` → `map_del`
> `:3197-3221`,`:5858-5864`; subscript parse + rules `:3834-3886`, dispatch
> `:3908-3916`; `or_return` `:5088-5105`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:11956`); it is not
...
> `src/tychoc.c:2365@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2383@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:2046-2047` (`[$N]T`) and `src/tychoc.c:2065-2066` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:1980-1981`. Pinned both ways by
...
(`src/tychoc.c:1345@TK_PERCENT` rather than the `char` arm at
`:1349@et == T_CHAR`), which is why it has a fixture of its own.
...
> Provenance: two-array arm `src/tychoc.c:6225-6255`, broadcast arm
> `src/tychoc.c:6284-6310`; per-element-type operator set
> `src/tychoc.c:1344@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:6739@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:6731@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:6721@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:6724@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:6792@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6295-6300`; the fresh spine
> `src/tychoc.c:10260@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10244@gen_arith_op`, operands never reordered
> `src/tychoc.c:10241@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10264@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2920@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:6689@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2009` (min) and `:2005` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5084-5086`).
...
destructuring list (`src/tychoc.c:7158-7180`). At most 8 targets are permitted.
...
(`src/tychoc.c:5212-5227`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3194-3210`,`:5856-5862`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12089-12092`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5096-5104`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5090-5094`).
