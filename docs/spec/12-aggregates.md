...
> `pop`-empty abort `:13371@pop from an empty array`,`:13371@pop from an empty array`; `reserve` `:6491-6517`,`:9850-9856`; tuple
> arity `:2340@a tuple has at most 8 elements`,`:2344@a tuple type needs at least two elements`, index `:5234-5242`; destructuring `:3486-3500`,`:7942-7958`;
> map read (pure `map_get`, no insert) `:5718-5733`; map place insert+zero
> `:10655-10664`; `keys()` insertion order — the walk `:13533@m.elive[e]` over the append-only entries array `:13285@m->ecount++`; `delete` → `map_del`
> `:3334-3358`,`:6414-6420`; subscript parse + rules `:3938-3990`, dispatch
> `:4013-4021`; `or_return` `:5594-5611`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:13172@pop`); it is not
...
> `src/tychoc.c:2409@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2427@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:2050-2051` (`[$N]T`) and `src/tychoc.c:2072-2073` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:1963-1964`. Pinned both ways by
...
(`src/tychoc.c:1346@TK_PERCENT` rather than the `char` arm at
`:1350@et == T_CHAR`), which is why it has a fixture of its own.
...
> Provenance: two-array arm `src/tychoc.c:6633-6663`, broadcast arm
> `src/tychoc.c:6692-6718`; per-element-type operator set
> `src/tychoc.c:1345@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7190@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7182@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7172@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7175@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:7244@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:6703-6708`; the fresh spine
> `src/tychoc.c:10635@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:10619@gen_arith_op`, operands never reordered
> `src/tychoc.c:10616@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:10639@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:2838@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7140@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2344@least` (min) and `:2324@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5421-5423`).
...
destructuring list (`src/tychoc.c:7617-7640`). At most 8 targets are permitted.
...
(`src/tychoc.c:5552-5567`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3331-3347`,`:6412-6418`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:12453-12456`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:5339-5347`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:5430-5434`).
