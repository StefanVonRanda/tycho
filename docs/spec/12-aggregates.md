...
> Provenance: array element restriction `src/tychoc.c:2342-2343`,`:2359-2360`;
> `pop`-empty abort `:14381@pop from an empty array`,`:14381@pop from an empty array`; `reserve` `:7224-7250`,`:10711-10717`; tuple
> arity `:2563@a tuple has at most 8 elements`,`:2568@a tuple type needs at least two elements`, index `:5864-5872`; destructuring `:3891-3905`,`:8764-8780`;
> map read (pure `map_get`, no insert) `:6386-6401`; map place insert+zero
> `:11563-11572`; `keys()` insertion order — the walk `:14543@m.elive[e]` over the append-only entries array `:14295@m->ecount++`; `delete` → `map_del`
> `:3739-3763`,`:7147-7153`; subscript parse + rules `:4447-4499`, dispatch
> `:4522-4530`; `or_return` `:6224-6241`.
...
`[T]`. `pop(a)` on an **empty** array MUST abort (`src/tychoc.c:14180@pop`); it is not
...
> `src/tychoc.c:2644@elem`, and its diagnostic now states that as the whole rule
> instead of an allow-list (`src/tychoc.c:2663@void`; the loops-cleanup plan). The fixed forms test both:
> `src/tychoc.c:2199-2200` (`[$N]T`) and `src/tychoc.c:2221-2222` (`[N]T`);
> `bounded[N]T` at `src/tychoc.c:2112-2113`. Pinned both ways by
...
(`src/tychoc.c:1483@TK_PERCENT` rather than the `char` arm at
`:1487@et == T_CHAR`), which is why it has a fixture of its own.
...
> Provenance: two-array arm `src/tychoc.c:7295-7331`, broadcast arm
> `src/tychoc.c:7360-7387`; per-element-type operator set
> `src/tychoc.c:1482@elem_arith_ok`; fixed-length mismatch
> `src/tychoc.c:7982@on a fixed array requires the same static length`; mixed
> kinds `src/tychoc.c:7974@cannot mix a fixed array and a growable array`;
> `bounded`/`[$N]T` `src/tychoc.c:7956@IS_BOUNDED`; element-type mismatch
> `src/tychoc.c:7967@arr_elem(lt) != arr_elem(rt)`; scalar must land at the
> element type `src/tychoc.c:8036@requires the scalar to have the array's element type`,
> its literal adaptation `src/tychoc.c:7371-7377`; the fresh spine
> `src/tychoc.c:11543@arena_alloc`, the per-element emit shared with the scalar
> case `src/tychoc.c:11507@gen_arith_op`, operands never reordered
> `src/tychoc.c:11504@int la = is_array`; the runtime length check, emitted only
> when both sides are arrays `src/tychoc.c:11547@tycho_ew_len`, and the abort
> itself `runtime/tycho_rt.c:3205@arithmetic on arrays of different lengths`;
> literal-zero divisor `src/tychoc.c:7924@division by zero`.
...
9-element tuple is a compile error, `src/tychoc.c:2568@least` (min) and `:2098@most` (max)). Tuples are first-class
...
- **Positional access and places.** `t.0`, `t.1`, … read an element; `t.0 = v`
  ...
  `0 .. n-1` is a compile error (`src/tychoc.c:5944-5946`).
...
destructuring list (`src/tychoc.c:8368-8391`). At most 8 targets are permitted.
...
(`src/tychoc.c:6075-6090`). When a non-zero default is wanted, use `m.get`
(§18.5).
...
map-delete rebinding the map (`src/tychoc.c:3736-3752`,`:7145-7151`); it removes
...
the order in which each key was first inserted (`src/tychoc.c:13327-13330`; the
...
- otherwise returns that `Err` from the **enclosing function**, which MUST itself
  ...
  compile error (`src/tychoc.c:6547@propagates`).

On an **`Option`**, `v := opt or_return` binds `v` on `Some(v)` and returns
`None`; the enclosing function MUST return an `Option` (`src/tychoc.c:6537@or_return`).
