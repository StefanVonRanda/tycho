...
> Provenance: parser entry `src/tychoc.c:5062-5108` (`parse_program`); the
...
> Provenance: `parse_package_decl` `src/tychoc.c:5343@parse_package_decl`, `parse_import_decl` `:5350@parse_import_decl`, `parse_const`
> (`src/tychoc.c:4990-5009`).
...
> Provenance: `parse_fn`, `src/tychoc.c@parse_fn` (params `src/tychoc.c:4719-4758`,
> variadic-last `src/tychoc.c:4745-4747`, `where` `src/tychoc.c:4698@where`).
...
> Provenance: `parse_stmt`, `src/tychoc.c:3652-4192`; `ExprStmt` restriction
> `:4282-4289`; compound-assign hoist `:3719-3761`.
...
> Provenance: `parse_if` (`src/tychoc.c:3562@parse_if`), `parse_match`
> (`src/tychoc.c:3678@parse_match`, value form `src/tychoc.c:3809@parse_match`),
> `for`/`parallel` (`src/tychoc.c:3787-3979`; the three-clause header
> `src/tychoc.c:3831-3876`, `0..<N` `src/tychoc.c:4221-4246`, the `range`
> refusal `src/tychoc.c:4316@"range"`), `select` (`src/tychoc.c:4136@TK_SELECT`),
> value-control routing (`src/tychoc.c:4106@parse_value_ctrl`,
> `src/tychoc.c:4409@parse_value_ctrl`, `src/tychoc.c:4423@parse_value_ctrl`,
> `src/tychoc.c:4409@parse_value_ctrl`, `src/tychoc.c:4500@parse_value_ctrl`).
...
> Provenance: precedence chain `src/tychoc.c:3146-3214`; postfix `:2991-3069`;
> `parse_primary` `:2680-2988`; unary `:3077-3083`.
...
> `parse_expr`, `src/tychoc.c:3146-3214`; postfix/unary `:2991-3083`.
