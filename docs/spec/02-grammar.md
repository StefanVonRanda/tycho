...
> Provenance: parser entry `src/tychoc.c:4618-4635` (`parse_program`); the
...
> Provenance: `parse_package_decl` `src/tychoc.c:4847@parse_package_decl`, `parse_import_decl` `:4854@parse_import_decl`, `parse_const`
> (`src/tychoc.c:4597-4616`).
...
> Provenance: `parse_fn`, `src/tychoc.c:3989-4081` (params `:4326-4365`,
> variadic-last `:4352-4354`, `where` `:4044-4073`).
...
> Provenance: `parse_extern_fn` (`src/tychoc.c:4113-4184`), `parse_subscript`
> (`:4100-4152`).
...
> Provenance: `parse_type_inner`, `src/tychoc.c:1962-2263`.
...
> Provenance: `parse_stmt`, `src/tychoc.c:3365-3853`; `ExprStmt` restriction
> `:3935-3942`; compound-assign hoist `:3430-3472`.
...
> Provenance: `parse_if` (`src/tychoc.c:3281@parse_if`), `parse_match`
> (`src/tychoc.c:3387@parse_match`, value form `src/tychoc.c:3518@parse_match`),
> `for`/`parallel` (`src/tychoc.c:3498-3690`; the three-clause header
> `src/tychoc.c:3542-3587`, `0..<N` `src/tychoc.c:3882-3907`, the `range`
> refusal `src/tychoc.c:3967@"range"`), `select` (`src/tychoc.c:3452-3488`),
> value-control routing (`src/tychoc.c:3765@parse_value_ctrl`,
> `src/tychoc.c:4060@parse_value_ctrl`, `src/tychoc.c:4074@parse_value_ctrl`,
> `src/tychoc.c:4060@parse_value_ctrl`, `src/tychoc.c:4105@parse_value_ctrl`).
...
> Provenance: precedence chain `src/tychoc.c:2937-3005`; postfix `:2782-2860`;
> `parse_primary` `:2477-2779`; unary `:2868-2874`.
...
> `parse_expr`, `src/tychoc.c:2937-3005`; postfix/unary `:2782-2874`.
