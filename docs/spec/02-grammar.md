...
> Provenance: parser entry `src/tychoc.c:4594-4619` (`parse_program`); the
...
> Provenance: `parse_package_decl` `src/tychoc.c:4750@parse_package_decl`, `parse_import_decl` `:4757@parse_import_decl`, `parse_const`
> (`src/tychoc.c:4575-4592`).
...
> Provenance: `parse_fn`, `src/tychoc.c:3826-3936` (params `:3849-3874`,
> variadic-last `:3876-3878`, `where` `:3890-3928`).
...
> Provenance: `parse_extern_fn` (`src/tychoc.c:4076-4151`), `parse_subscript`
> (`:3956-4008`).
...
> Provenance: `parse_type_inner`, `src/tychoc.c:1964-2262`.
...
> Provenance: `parse_stmt`, `src/tychoc.c:3282-3777`; `ExprStmt` restriction
> `:3765-3772`; compound-assign hoist `:3244-3280`.
...
> Provenance: `parse_if` (`src/tychoc.c:3198@parse_if`), `parse_match`
> (`src/tychoc.c:3304@parse_match`, value form `src/tychoc.c:3429@parse_match`),
> `for`/`parallel` (`src/tychoc.c:3409-3620`; the three-clause header
> `src/tychoc.c:3453-3502`, `0..<N` `src/tychoc.c:3525-3550`, the `range`
> refusal `src/tychoc.c:3925@"range"`), `select` (`src/tychoc.c:3363-3399`),
> value-control routing (`src/tychoc.c:3698@parse_value_ctrl`,
> `src/tychoc.c:4018@parse_value_ctrl`, `src/tychoc.c:4032@parse_value_ctrl`,
> `src/tychoc.c:4018@parse_value_ctrl`, `src/tychoc.c:4063@parse_value_ctrl`).
...
> Provenance: precedence chain `src/tychoc.c:2778-2846`; postfix `:2638-2727`;
> `parse_primary` `:2347-2635`; unary `:2735-2741`.
...
> `parse_expr`, `src/tychoc.c:2778-2846`; postfix/unary `:2638-2741`.
