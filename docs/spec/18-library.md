...
2. store `1` to the flag `shutdown_requested` reads
   (`corelib/signal/signal_shim.c:148@sigx_flag`);
3. load the registered descriptor (`corelib/signal/signal_shim.c:149@sigx_fd`);
4. if that descriptor is non-negative, call `shutdown(fd, SHUT_RDWR)`
   (`corelib/signal/signal_shim.c:150@shutdown`) — a bare syscall that allocates
...
- **The descriptor is registered before the first `sigaction`**
  (`corelib/signal/signal_shim.c:200@sigx_fd`), so a signal arriving mid-install
...
- **The handler is installed with `sa_flags = 0`**
  (`corelib/signal/signal_shim.c:205@sa_flags`), that is, **without
