...
builtin). `docs/reference/corelib.md:61-63`.
...
digit), `hex_val` (0..15 or −1). The lexer/parser workhorse. `docs/reference/corelib.md:64-67`.
...
string). (`push`/`pop`/`len`/`range` are builtins.) `docs/reference/corelib.md:114-121`.
...
`docs/reference/corelib.md:122-129`.
...
`docs/reference/corelib.md:130-138`.
...
(`[0, n)`), `shuffle(&st, xs)` (Fisher-Yates, new array). `docs/reference/corelib.md:165-172`.

### 32.10 `time`

Wraps the `clock()` (monotonic ns) and `now()` (UNIX seconds) builtins: stopwatch
`start()`, `elapsed_ns`/`elapsed_us`/`elapsed_ms`; conversions
`ns_to_us`/`ns_to_ms`/`ns_to_s`; wall clock `unix_secs()`. `docs/reference/corelib.md:173-182`.
...
and `dtx_offset_at(tz, secs)`. Core tier. `docs/reference/corelib.md:183-198`; shim
...
four common escapes. `docs/reference/corelib.md:218-241`.
...
(TSV = `parse_delim(s, 9)`); `get(rows, r, c)` (bounds-safe). `docs/reference/corelib.md:242-252`.
...
`docs/reference/corelib.md:260-268`.
...
`char.hex_val`. Same `0x00` decode caveat as `base64`. `docs/reference/corelib.md:269-272`.
...
Same `0x00` decode caveat. `docs/reference/corelib.md:273-278`.
...
`docs/reference/corelib.md:279-287`.
...
`to_hex`. All return a non-negative int in `[0, 2^32)`. `docs/reference/corelib.md:288-299`.
...
Bit-exact against RFC 1321 vectors. `docs/reference/corelib.md:300-305`.
...
password hash — use a KDF). Bit-exact against NIST vectors. `docs/reference/corelib.md:306-310`.
...
builtins' sentinels elsewhere. `docs/reference/corelib.md:399-445`; shim
...
unspawnable program. `docs/reference/corelib.md:446-470`.
...
done; §31.1). `docs/reference/corelib.md:199-205`.
...
operations park behind a mutex (`runtime/tycho_rt.c:1108@mu`,
`runtime/tycho_rt.c:1108@pthread_mutex_lock`); a handler that interrupts the
...
2. store `1` to the flag `shutdown_requested` reads
   (`corelib/signal/signal_shim.c:65@sigx_flag`);
3. load the registered descriptor (`corelib/signal/signal_shim.c:66@sigx_fd`);
4. if that descriptor is non-negative, call `shutdown(fd, SHUT_RDWR)`
   (`corelib/signal/signal_shim.c:70@shutdown`) — a bare syscall that allocates
...
- **The descriptor is registered before the first `sigaction`**
  (`corelib/signal/signal_shim.c:107@sigx_fd`), so a signal arriving mid-install
...
- **The handler is installed with `sa_flags = 0`**
  (`corelib/signal/signal_shim.c:112@sa_flags`), that is, **without
...
`0x00` (text APIs). `docs/reference/corelib.md:206-217`.
...
`docs/reference/corelib.md:321-336`.
