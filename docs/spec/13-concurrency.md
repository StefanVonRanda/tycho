...
> Provenance: `0..<N` parsed at `src/tychoc.c:3766-3791`; parallel-only refusal
> `src/tychoc.c:3835@par_here`; literal-zero refusal `src/tychoc.c:3838@ival != 0`;
> any other loop shape under `parallel` refused at `src/tychoc.c:3739@S_FORRANGE`
> (it is the only node the chunker accepts). Chunk fan-out `K = min(ncpu(), N)`
> `src/tychoc.c:10454-10455`, capped at 64 by `src/tychoc.c:11162@_pk > 64`
> (the chunk-handle array `src/tychoc.c:11163@_pts[64]` is the reason for the
> number); each chunk is a real OS thread,
> `runtime/tycho_rt.c:808@pthread_create`. A capture is deep-copied only when
> `src/tychoc.c:11173@type_is_heap(ct)` holds, and `type_is_heap`
> (`src/tychoc.c:1310-1332`) has no channel arm, so a `Channel(T)` capture is
