...
> Provenance: `0..<N` parsed at `src/tychoc.c:3983-4008`; parallel-only refusal
> `src/tychoc.c:4052@par_here`; literal-zero refusal `src/tychoc.c:4055@ival != 0`;
> any other loop shape under `parallel` refused at `src/tychoc.c:3956@S_FORRANGE`
> (it is the only node the chunker accepts). Chunk fan-out `K = min(ncpu(), N)`
> `src/tychoc.c:10967-10968`, capped at 64 by `src/tychoc.c:11733@_pk > 64`
> (the chunk-handle array `src/tychoc.c:11734@_pts[64]` is the reason for the
> number); each chunk is a real OS thread,
> `runtime/tycho_rt.c:986@pthread_create`. A capture is deep-copied only when
> `src/tychoc.c:11744@type_is_heap(ct)` holds, and `type_is_heap`
> (`src/tychoc.c:1436-1458`) has no channel arm, so a `Channel(T)` capture is
