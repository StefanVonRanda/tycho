...
> Provenance: `0..<N` parsed at `src/tychoc.c:3613-3638`; parallel-only refusal
> `src/tychoc.c:4019@par_here`; literal-zero refusal `src/tychoc.c:4022@ival != 0`;
> any other loop shape under `parallel` refused at `src/tychoc.c:3883@S_FORRANGE`
> (it is the only node the chunker accepts). Chunk fan-out `K = min(ncpu(), N)`
> `src/tychoc.c:10754-10755`, capped at 64 by `src/tychoc.c:11466@_pk > 64`
> (the chunk-handle array `src/tychoc.c:11467@_pts[64]` is the reason for the
> number); each chunk is a real OS thread,
> `runtime/tycho_rt.c:790@pthread_create`. A capture is deep-copied only when
> `src/tychoc.c:11477@type_is_heap(ct)` holds, and `type_is_heap`
