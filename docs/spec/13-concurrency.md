...
> `src/tychoc.c:10695-10696`, capped at 64 by `src/tychoc.c:11403@_pk > 64`
> (the chunk-handle array `src/tychoc.c:11404@_pts[64]` is the reason for the
> number); each chunk is a real OS thread,
> `runtime/tycho_rt.c:808@pthread_create`. A capture is deep-copied only when
> `src/tychoc.c:11414@type_is_heap(ct)` holds, and `type_is_heap`
