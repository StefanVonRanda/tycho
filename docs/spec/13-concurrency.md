...
> `src/tychoc.c:3948@par_here`; literal-zero refusal `src/tychoc.c:3951@ival != 0`;
...
> `src/tychoc.c:10620-10621`, capped at 64 by `src/tychoc.c:11320@_pk > 64`
> (the chunk-handle array `src/tychoc.c:11321@_pts[64]` is the reason for the
> number); each chunk is a real OS thread,
> `runtime/tycho_rt.c:790@pthread_create`. A capture is deep-copied only when
> `src/tychoc.c:11331@type_is_heap(ct)` holds, and `type_is_heap`
