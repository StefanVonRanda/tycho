...
> Provenance: `parse_stmt` `src/tychoc.c:3108-3578` (`parse_if` `:2929@parse_if`,
> `parse_match` `:3035@parse_match`, `for` `:3245-3446`, `select` `:3189-3225`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3498@TK_COLON`; the three-clause header
...
> label at `src/tychoc.c:11397@_post%d`; the `range()` refusal
> `src/tychoc.c:3632@was removed: write`. There is no step in the implementation
