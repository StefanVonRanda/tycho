...
> Provenance: `parse_stmt` `src/tychoc.c:3247-3727` (`parse_if` `:3163@parse_if`,
> `parse_match` `:3269@parse_match`, `for` `:3384-3585`, `select` `:3328-3364`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3742@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3418-3467`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3793@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7559-7564`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11113-11116` with the
> label at `src/tychoc.c:11980@_post%d`; the `range()` refusal
> `src/tychoc.c:3876@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1617-1623`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:11286-11290`).
>
