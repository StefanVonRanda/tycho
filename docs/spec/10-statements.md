...
> Provenance: `parse_stmt` `src/tychoc.c:3414-3925` (`parse_if` `:3330@parse_if`,
> `parse_match` `:3436@parse_match`, `for` `:3551-3752`, `select` `:3495-3531`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3941@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3585-3634`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3992@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7955-7960`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11620-11623` with the
> label at `src/tychoc.c:12495@_post%d`; the `range()` refusal
> `src/tychoc.c:4089@was removed: write`. There is no step in the implementation
