...
> Provenance: `parse_stmt` `src/tychoc.c:3438-3920` (`parse_if` `:3260@parse_if`,
> `parse_match` `:3482@parse_match`, `for` `:3575-3757`, `select` `:3519-3555`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3833@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3506-3551`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3867@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7867-7872`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11314-11317` with the
> label at `src/tychoc.c:12153@_post%d`; the `range()` refusal
> `src/tychoc.c:3932@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1627-1633`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:11479-11483`).
