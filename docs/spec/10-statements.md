...
> Provenance: `parse_stmt` `src/tychoc.c:3474-3956` (`parse_if` `:3290@parse_if`,
> `parse_match` `:3518@parse_match`, `for` `:3611-3793`, `select` `:3555-3591`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3869@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3542-3587`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3903@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7945-7950`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11393-11396` with the
> label at `src/tychoc.c:12232@_post%d`; the `range()` refusal
> `src/tychoc.c:3968@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1627-1633`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:11558-11562`).
