...
> Provenance: `parse_stmt` `src/tychoc.c:3108-3578` (`parse_if` `:2923@parse_if`,
> `parse_match` `:3029@parse_match`, `for` `:3245-3446`, `select` `:3189-3225`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3492@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3279-3328`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3549@parse_stmt`; loop scoping and
...
> label at `src/tychoc.c:11391@_post%d`; the `range()` refusal
> `src/tychoc.c:3626@was removed: write`. There is no step in the implementation
