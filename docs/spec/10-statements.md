...
> Provenance: `parse_stmt` `src/tychoc.c:3450-3932` (`parse_if` `:3272@parse_if`,
> `parse_match` `:3494@parse_match`, `for` `:3587-3769`, `select` `:3531-3567`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3845@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3518-3563`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3879@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7887-7892`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11334-11337` with the
> label at `src/tychoc.c:12173@_post%d`; the `range()` refusal
> `src/tychoc.c:3944@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1627-1633`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:11499-11503`).
