...
> Provenance: `parse_stmt` `src/tychoc.c:3259-3739` (`parse_if` `:3175@parse_if`,
> `parse_match` `:3281@parse_match`, `for` `:3396-3597`, `select` `:3340-3376`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3754@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3430-3479`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:3805@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:7571-7576`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11139-11142` with the
> label at `src/tychoc.c:12006@_post%d`; the `range()` refusal
> `src/tychoc.c:3888@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1617-1623`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:11312-11316`).
>
