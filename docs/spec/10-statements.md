...
> Provenance: `parse_stmt` `src/tychoc.c:3695-4235` (`parse_if` `:3513@parse_if`,
> `parse_match` `:3741@parse_match`, `for` `:3832-4064`, `select` `src/tychoc.c:4068@TK_SELECT`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:4142@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3763-3808`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:4176@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:8500-8505`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:12041-12044` with the
> label at `src/tychoc.c:12953@_post%d`; the `range()` refusal
> `src/tychoc.c:4249@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1717-1723`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:12206-12210`).
