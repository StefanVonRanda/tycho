...
> Provenance: `parse_stmt` `src/tychoc.c:3576-4089` (`parse_if` `:3392@parse_if`,
> `parse_match` `:3620@parse_match`, `for` `:3713-3918`, `select` `:3657-3693`). Loop and `match` behaviors marked
...
> Provenance: bare `for:` `src/tychoc.c:3994@TK_COLON`; the three-clause header
> scan and its five required-clause refusals `src/tychoc.c:3644-3689`; `init`
> parsed by `parse_stmt` itself `src/tychoc.c:4028@parse_stmt`; loop scoping and
> the post clause resolved outside the body block `src/tychoc.c:8315-8320`;
> `continue` emitted as `goto _post<id>` `src/tychoc.c:11835-11838` with the
> label at `src/tychoc.c:12704@_post%d`; the `range()` refusal
> `src/tychoc.c:4101@was removed: write`. There is no step in the implementation
> at all: `Stmt` carries `r_start` and `r_stop` only (`src/tychoc.c:1674-1680`)
> and every `S_FORRANGE` emits `h_i < _stopN; h_i += 1`
> (`src/tychoc.c:12000-12004`).
