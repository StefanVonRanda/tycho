...
> Provenance: entry point `src/tychoc.c:9357@no 'main' procedure` (no `main`),
> `:9400@takes no parameters` and `:9403@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14225-14369@compile_package`, driver `:14454-14629@int main(`;
> `extern` `parse_extern_fn` `:4590-4693@parse_extern_fn`; the C compiler
> invocation `:14655@system(cmd)`.
...
`main` at `src/tychoc.c:13994@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14578@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4658@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4600@never mangled`). Two `extern` declarations naming the
...
executable with a single C-compiler invocation (`src/tychoc.c:14654@-fwrapv`,
run at `src/tychoc.c:14655@system(cmd)`).
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:14654@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5316@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14642@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14641@march`).
- The default C compiler is `cc` (`src/tychoc.c:14492@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14521@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14498-14500`):
...
(`src/tychoc.c:14524@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6284@payload-less` handles a
...
name (`src/tychoc.c:14234@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14577@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14259@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5272-5273`). A bound package name,
...
`src/tychoc.c:14227@g_cur_pkg_prefix`, reset at `src/tychoc.c:14248@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2603@check_pkg_private`,
`src/tychoc.c:3275@check_pkg_private`, `src/tychoc.c:6287@check_pkg_private`,
`src/tychoc.c:6607@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:5018@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:5031@importer_dir`), independent of the current working
...
(`src/tychoc.c:5023@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14155@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14154@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14212@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14230@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14244@prog->v`).
...
when the package is imported (`src/tychoc.c:14191@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5395@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14191@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5344@SKIPS`).
