...
> Provenance: entry point `src/tychoc.c:7869@no 'main' procedure` (no `main`),
> `:7894-7895@'main' must be` (the signature rule); compilation unit
> `compile_package` `:12481-12486@compile_package`, driver `:12540-12644@int main(`;
> `extern` `parse_extern_fn` `:3844-3914@parse_extern_fn`; the C compiler
> invocation `:12758@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:7869@no 'main' procedure`) and MUST
...
`:7894-7895@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
