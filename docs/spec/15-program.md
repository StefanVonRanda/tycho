...
> Provenance: entry point `src/tychoc.c:7839@no 'main' procedure` (no `main`),
> `:7864-7865@'main' must be` (the signature rule); compilation unit
> `compile_package` `:12372-12377@compile_package`, driver `:12489-12593@int main(`;
> `extern` `parse_extern_fn` `:3844-3914@parse_extern_fn`; the C compiler
> invocation `:12633@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:7839@no 'main' procedure`) and MUST
...
`:7864-7865@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
