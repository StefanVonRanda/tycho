...
> Provenance: entry point `src/tychoc.c:9185@no 'main' procedure` (no `main`),
> `:9228@takes no parameters` and `:9231@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14014-14133@compile_package`, driver `:14216-14384@int main(`;
> `extern` `parse_extern_fn` `:4512-4615@parse_extern_fn`; the C compiler
> invocation `:14380@system(cmd)`.
...
`main` at `src/tychoc.c:13779@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14303@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14379@-fwrapv`,
run at `src/tychoc.c:14380@system(cmd)`).
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:14379@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5232@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14367@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14366@march`).
- The default C compiler is `cc` (`src/tychoc.c:14224@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14246@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14253-14255`):
...
(`src/tychoc.c:14249@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6147@payload-less` handles a
...
name (`src/tychoc.c:13993@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14302@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14018@merge_pkg`).
...
`src/tychoc.c:13986@g_cur_pkg_prefix`, reset at `src/tychoc.c:14007@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2525@check_pkg_private`,
`src/tychoc.c:3197@check_pkg_private`, `src/tychoc.c:6150@check_pkg_private`,
`src/tychoc.c:6470@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:4940@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:4953@importer_dir`), independent of the current working
...
(`src/tychoc.c:4945@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13914@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13913@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:13971@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:13989@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14003@prog->v`).
...
when the package is imported (`src/tychoc.c:13950@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5311@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:13950@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5260@SKIPS`).
