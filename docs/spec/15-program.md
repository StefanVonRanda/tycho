...
> Provenance: entry point `src/tychoc.c:9919@no 'main' procedure` (no `main`),
> `:9962@takes no parameters` and `:9965@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14817-14961@compile_package`, driver `:15063-15248@int main(`;
> `extern` `parse_extern_fn` `:4859-4962@parse_extern_fn`; the C compiler
> invocation `:15374@system(cmd)`.
...
`main` at `src/tychoc.c:14657@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:15279@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4929@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4871@never mangled`). Two `extern` declarations naming the
...
executable with a single C-compiler invocation (`src/tychoc.c:15373@-fwrapv`,
run at `src/tychoc.c:15374@system(cmd)`).
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:15373@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5727@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15353@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15345@march`).
...
- The default C compiler is `cc` (`src/tychoc.c:15185@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:15221@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:15145-15148`):
...
(`src/tychoc.c:15222@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6742@payload-less` handles a
...
name (`src/tychoc.c:14903@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15278@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14933@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5642-5643`). A bound package name,
...
`src/tychoc.c:14896@g_cur_pkg_prefix`, reset at `src/tychoc.c:14917@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2713@check_pkg_private`,
`src/tychoc.c:3442@check_pkg_private`, `src/tychoc.c:6745@check_pkg_private`,
`src/tychoc.c:7113@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:5390@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:5403@importer_dir`), independent of the current working
...
(`src/tychoc.c:5395@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14818@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14817@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14881@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14899@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14911@prog->v`).
...
when the package is imported (`src/tychoc.c:14860@add_pkg_deps`):
...
- **`deps` file (pkg-config).** The package's sibling
  ...
  (`src/tychoc.c:5806@could not resolve dependency`); the missing library
...
the note at `src/tychoc.c:5755@SKIPS`).
