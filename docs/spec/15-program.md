...
> Provenance: entry point `src/tychoc.c:9485@no 'main' procedure` (no `main`),
> `:9528@takes no parameters` and `:9531@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14412-14556@compile_package`, driver `:14641-14816@int main(`;
> `extern` `parse_extern_fn` `:4629-4732@parse_extern_fn`; the C compiler
> invocation `:14842@system(cmd)`.
...
`main` at `src/tychoc.c:14181@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14765@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4697@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4639@never mangled`). Two `extern` declarations naming the
...
executable with a single C-compiler invocation (`src/tychoc.c:14841@-fwrapv`,
run at `src/tychoc.c:14842@system(cmd)`).
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:14841@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5408@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14829@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14828@march`).
- The default C compiler is `cc` (`src/tychoc.c:14679@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14708@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14685-14687`):
...
(`src/tychoc.c:14711@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6376@payload-less` handles a
...
name (`src/tychoc.c:14421@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14764@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14446@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5363-5364`). A bound package name,
...
`src/tychoc.c:14414@g_cur_pkg_prefix`, reset at `src/tychoc.c:14435@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2642@check_pkg_private`,
`src/tychoc.c:3314@check_pkg_private`, `src/tychoc.c:6379@check_pkg_private`,
`src/tychoc.c:6726@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:5109@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:5122@importer_dir`), independent of the current working
...
(`src/tychoc.c:5114@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14342@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14341@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14399@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14417@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14431@prog->v`).
...
when the package is imported (`src/tychoc.c:14378@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5487@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14378@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5436@SKIPS`).
