...
`src/tychoc.c:14121@g_cur_pkg_prefix`, reset at `src/tychoc.c:14142@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14049@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14048@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14106@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14124@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14138@prog->v`).
...
when the package is imported (`src/tychoc.c:14085@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14085@file_exists`); a `deps` file with
