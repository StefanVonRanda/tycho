...
> `:8614@takes no parameters` and `:8617@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13489-13494@compile_package`, driver `:13606-13710@int main(`;
> `extern` `parse_extern_fn` `:4284-4354@parse_extern_fn`; the C compiler
> invocation `:13831@system(cmd)`.
...
`main` at `src/tychoc.c:12387`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12589-12592`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12800-12804`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12835-12849`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12834`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12833`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12742`,
  `:12758`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12775-12781`):
...
`src/tychoc.c:12649-12658`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12559-12578`).
...
name (`src/tychoc.c:12568-12571`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12412`, `:12792`; `compile_package` starts the merge at
that name with an empty prefix, `:12579-12582`).
...
`:12552`, reset at `:12572`; [packages.md](../guides/packages.md) §"How it builds").
...
(`merge_pkg` `src/tychoc.c:12521-12585`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12525-12528`):
