...
`main` at `src/tychoc.c:13747@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4569@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4511@never mangled`). Two `extern` declarations naming the
...
(`src/tychoc.c:14210@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6136@payload-less` handles a
...
they are not reserved words (`src/tychoc.c:5177-5178`). A bound package name,
...
`src/tychoc.c:13954@g_cur_pkg_prefix`, reset at `src/tychoc.c:13975@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
`src/tychoc.c:3186@check_pkg_private`, `src/tychoc.c:6139@check_pkg_private`,
`src/tychoc.c:6459@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:4929@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:4942@importer_dir`), independent of the current working
...
(`src/tychoc.c:4934@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13882@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13881@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:13939@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:13957@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:13971@prog->v`).
...
when the package is imported (`src/tychoc.c:13918@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5300@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:13918@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5249@SKIPS`).
