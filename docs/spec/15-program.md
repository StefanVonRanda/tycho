...
> Provenance: entry point `src/tychoc.c:8727@no 'main' procedure` (no `main`),
> `:8758@takes no parameters` and `:8761@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13667-13672@compile_package`, driver `:13784-13888@int main(`;
> `extern` `parse_extern_fn` `:4345-4423@parse_extern_fn`; the C compiler
> invocation `:14009@system(cmd)`.
...
`main` at `src/tychoc.c:12565`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12767-12770`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12978-12982`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:13013-13027`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:13012`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:13011`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12920`,
  `:12936`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12953-12959`):
...
`src/tychoc.c:12827-12836`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12737-12756`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5335-5347` handles a
...
name (`src/tychoc.c:12746-12749`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12590`, `:12970`; `compile_package` starts the merge at
that name with an empty prefix, `:12757-12760`).
...
`src/tychoc.c:4291-4303`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4384-4393`, `is_imported_pkg` `:4405-4411`).
...
they are not reserved words (`src/tychoc.c:4273-4277`). A bound package name,
...
(`pkg_mangle` `src/tychoc.c:1722-1724`; `g_cur_pkg_prefix` set per file at
`:12730`, reset at `:12750`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4413-4420`; enforced at the
qualified-reference sites `:2125`, `:2951`, `:5339`, `:5604`). Every other top-level
...
`src/tychoc.c:4350-4365`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4369-4378`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12699-12763`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12703-12706`):
...
the note at `src/tychoc.c:4598-4600`).
