...
> Provenance: entry point `src/tychoc.c:8962@no 'main' procedure` (no `main`),
> `:9004@takes no parameters` and `:9007@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13768-13773@compile_package`, driver `:13963-14062@int main(`;
> `extern` `parse_extern_fn` `:4320-4398@parse_extern_fn`; the C compiler
...
`main` at `src/tychoc.c:13533@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14025@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4464@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4406@never mangled`). Two `extern` declarations naming the
...
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5076@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14089@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14088@march`).
- The default C compiler is `cc` (`src/tychoc.c:13971@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:13993@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14000-14002`):
...
(`src/tychoc.c:13996@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5986@payload-less` handles a
...
name (`src/tychoc.c:13747@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14024@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:13772@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5037-5038`). A bound package name,
...
`src/tychoc.c:13740@g_cur_pkg_prefix`, reset at `src/tychoc.c:13761@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2421@check_pkg_private`,
`src/tychoc.c:3087@check_pkg_private`, `src/tychoc.c:5989@check_pkg_private`,
`src/tychoc.c:6309@check_pkg_private`). Every other top-level
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13668@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13667@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:13725@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:13743@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:13757@prog->v`).
...
when the package is imported (`src/tychoc.c:13704@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5155@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:13704@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5104@SKIPS`).
