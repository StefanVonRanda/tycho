...
> Provenance: entry point `src/tychoc.c:9688@no 'main' procedure` (no `main`),
> `:9731@takes no parameters` and `:9734@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14640-14784@compile_package`, driver `:14886-15070@int main(`;
> `extern` `parse_extern_fn` `:4775-4878@parse_extern_fn`; the C compiler
> invocation `:15103@system(cmd)`.
...
`main` at `src/tychoc.c:14409@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
that joins independently-compiled packages (`src/tychoc.c@compile_package`; [packages.md](../reference/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:15019@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4843@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4785@never mangled`). Two `extern` declarations naming the
...
executable with a single C-compiler invocation (`src/tychoc.c:15102@-fwrapv`,
run at `src/tychoc.c:15103@system(cmd)`).
The invocation has the shape:

```text
cc {-O3 | -O0 -g} -fwrapv [-march=native | -march=<level>] -pthread -o OUT gen.c \
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:15102@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5604@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15090@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15082@march`).
- **A raised ISA baseline is opt-in** via `--target <level>`, whose levels are the
  ...
  to this line, so an unflagged build is byte-identical to one made before the
  flag existed; any other level adds `-march=<level>` and the binary MUST NOT be
...
- The default C compiler is `cc` (`src/tychoc.c:14926@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14961@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14932-14935`):
...
(`src/tychoc.c:14964@-L`). Every library/package name that reaches the shell — from
...
is not semantically significant ([packages.md](../reference/packages.md) §"Surface
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6575@payload-less` handles a
...
name (`src/tychoc.c:14649@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15018@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14674@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5558-5559`). A bound package name,
...
([packages.md](../reference/packages.md) §"Surface syntax"). **Import cycles are an
error** (§28.5).
...
`src/tychoc.c:14642@g_cur_pkg_prefix`, reset at `src/tychoc.c:14663@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2627@check_pkg_private`,
`src/tychoc.c:3356@check_pkg_private`, `src/tychoc.c:6578@check_pkg_private`,
`src/tychoc.c:6925@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:5304@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:5317@importer_dir`), independent of the current working
...
(`src/tychoc.c:5309@cannot find the corelib`). `core:` is currently the only collection root
([packages.md](../reference/packages.md) §"The `core:` collection").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14570@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14569@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14627@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14645@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14659@prog->v`).
...
when the package is imported (`src/tychoc.c:14606@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5683@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14606@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5632@SKIPS`).
