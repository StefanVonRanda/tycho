...
> Provenance: entry point `src/tychoc.c:8583@no 'main' procedure` (no `main`),
> `:8608-8609@'main' must be` (the signature rule); compilation unit
> `compile_package` `:13464-13469@compile_package`, driver `:13581-13685@int main(`;
> `extern` `parse_extern_fn` `:4284-4354@parse_extern_fn`; the C compiler
> invocation `:13806@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:8583@no 'main' procedure`) and MUST
...
`:8608-8609@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
...
`main` at `src/tychoc.c:12377`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12579-12582`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12790-12794`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12825-12839`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12824`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12823`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12732`,
  `:12748`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12765-12771`):
...
`src/tychoc.c:12639-12648`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12549-12568`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5258-5270` handles a
...
name (`src/tychoc.c:12558-12561`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12402`, `:12782`; `compile_package` starts the merge at
that name with an empty prefix, `:12569-12572`).
...
`src/tychoc.c:4230-4242`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4323-4332`, `is_imported_pkg` `:4336-4342`).
...
they are not reserved words (`src/tychoc.c:4212-4216`). A bound package name,
...
(`pkg_mangle` `src/tychoc.c:1714-1716`; `g_cur_pkg_prefix` set per file at
`:12542`, reset at `:12562`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4344-4351`; enforced at the
qualified-reference sites `:2116`, `:2921`, `:5262`, `:5527`). Every other top-level
...
`src/tychoc.c:4289-4304`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4308-4317`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12511-12575`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12515-12518`):
...
the note at `src/tychoc.c:4529-4531`).
