...
> Provenance: entry point `src/tychoc.c:8712@no 'main' procedure` (no `main`),
> `:8743@takes no parameters` and `:8746@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13652-13657@compile_package`, driver `:13769-13873@int main(`;
> `extern` `parse_extern_fn` `:4338-4408@parse_extern_fn`; the C compiler
> invocation `:13994@system(cmd)`.
...
`main` at `src/tychoc.c:12550`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12752-12755`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12963-12967`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12998-13012`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12997`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12996`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12905`,
  `:12921`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12938-12944`):
...
`src/tychoc.c:12812-12821`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12722-12741`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5320-5332` handles a
...
name (`src/tychoc.c:12731-12734`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12575`, `:12955`; `compile_package` starts the merge at
that name with an empty prefix, `:12742-12745`).
...
`src/tychoc.c:4284-4296`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4377-4386`, `is_imported_pkg` `:4390-4396`).
...
they are not reserved words (`src/tychoc.c:4266-4270`). A bound package name,
...
(`pkg_mangle` `src/tychoc.c:1715-1717`; `g_cur_pkg_prefix` set per file at
`:12715`, reset at `:12735`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4398-4405`; enforced at the
qualified-reference sites `:2118`, `:2944`, `:5324`, `:5589`). Every other top-level
...
`src/tychoc.c:4343-4358`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4362-4371`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12684-12748`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12688-12691`):
...
the note at `src/tychoc.c:4583-4585`).
