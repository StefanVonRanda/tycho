...
`main` at `src/tychoc.c:14487@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14648@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14647@has no .ty files`).
