...
> Provenance: entry point `src/tychoc.c:8315@no 'main' procedure` (no `main`),
> `:8340-8341@'main' must be` (the signature rule); compilation unit
> `compile_package` `:13180-13185@compile_package`, driver `:13239-13343@int main(`;
> `extern` `parse_extern_fn` `:4075-4145@parse_extern_fn`; the C compiler
> invocation `:13514@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:8315@no 'main' procedure`) and MUST
...
`:8340-8341@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
