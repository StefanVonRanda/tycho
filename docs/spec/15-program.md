...
> Provenance: entry point `src/tychoc.c:9237@no 'main' procedure` (no `main`),
> `:9280@takes no parameters` and `:9283@returns nothing or` (the two signature
...
`main` at `src/tychoc.c:13864@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14397@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14473@-fwrapv`,
run at `src/tychoc.c:14474@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14461@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14460@march`).
- The default C compiler is `cc` (`src/tychoc.c:14311@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14340@--cc`).
...
(`src/tychoc.c:14343@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6182@payload-less` handles a
...
name (`src/tychoc.c:14078@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14396@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14103@merge_pkg`).
...
`src/tychoc.c:14071@g_cur_pkg_prefix`, reset at `src/tychoc.c:14092@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
`src/tychoc.c:3232@check_pkg_private`, `src/tychoc.c:6185@check_pkg_private`,
`src/tychoc.c:6505@check_pkg_private`). Every other top-level
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13999@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13998@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14056@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14074@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14088@prog->v`).
...
when the package is imported (`src/tychoc.c:14035@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14035@file_exists`); a `deps` file with
