...
> Provenance: entry point `src/tychoc.c:9303@no 'main' procedure` (no `main`),
> `:9334@takes no parameters` and `:9337@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14318-14323@compile_package`, driver `:14436-14540@int main(`;
> `extern` `parse_extern_fn` `:4625-4703@parse_extern_fn`; the C compiler
> invocation `:14660@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:9303@no 'main' procedure`), a `main`
...
`main` at `src/tychoc.c:14080@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14574@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14659@-fwrapv`,
run at `src/tychoc.c:14660@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14656@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14655@march`).
- The default C compiler is `cc` (`src/tychoc.c:14497@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14518@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14525-14527`):
...
(`src/tychoc.c:14521@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c:14300@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14573@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14325@merge_pkg`).
...
`src/tychoc.c:14293@g_cur_pkg_prefix`, reset at `src/tychoc.c:14314@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14221@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14220@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14278@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14296@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14310@prog->v`).
...
when the package is imported (`src/tychoc.c:14257@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14257@file_exists`); a `deps` file with
