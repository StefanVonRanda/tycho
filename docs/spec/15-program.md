...
> Provenance: entry point `src/tychoc.c:7191@no 'main' procedure` (no `main`),
> `:7216-7217@'main' must be` (the signature rule); compilation unit
> `compile_package` `:11532-11537@compile_package`, driver `:11681-11785@int main(`;
> `extern` `parse_extern_fn` `:3530-3600@parse_extern_fn`; the C compiler
> invocation `:11791@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:7191@no 'main' procedure`) and MUST
...
`:7216-7217@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
