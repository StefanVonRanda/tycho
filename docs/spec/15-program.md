...
> Provenance: entry point `src/tychoc.c:8522@no 'main' procedure` (no `main`),
> `:8547-8548@'main' must be` (the signature rule); compilation unit
> `compile_package` `:13403-13408@compile_package`, driver `:13520-13624@int main(`;
> `extern` `parse_extern_fn` `:4234-4304@parse_extern_fn`; the C compiler
> invocation `:13745@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:8522@no 'main' procedure`) and MUST
...
`:8547-8548@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
...
`main` at `src/tychoc.c:12316`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12518-12521`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12729-12733`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12764-12778`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12763`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12762`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12671`,
  `:12687`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12704-12710`):
...
`src/tychoc.c:12578-12587`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12488-12507`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5208-5220` handles a
...
name (`src/tychoc.c:12497-12500`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12341`, `:12721`; `compile_package` starts the merge at
that name with an empty prefix, `:12508-12511`).
...
`src/tychoc.c:4180-4192`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4273-4282`, `is_imported_pkg` `:4286-4292`).
...
they are not reserved words (`src/tychoc.c:4162-4166`). A bound package name,
...
`:12481`, reset at `:12501`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4294-4301`; enforced at the
qualified-reference sites `:2070`, `:2875`, `:5212`, `:5477`). Every other top-level
...
`src/tychoc.c:4239-4254`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4258-4267`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12450-12514`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12454-12457`):
...
the note at `src/tychoc.c:4479-4481`).
