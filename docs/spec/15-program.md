...
> Provenance: entry point `src/tychoc.c:8756@no 'main' procedure` (no `main`),
> `:8787@takes no parameters` and `:8790@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13711-13716@compile_package`, driver `:13829-13933@int main(`;
> `extern` `parse_extern_fn` `:4353-4431@parse_extern_fn`; the C compiler
> invocation `:14054@system(cmd)`.
...
`main` at `src/tychoc.c:12594`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12796-12799`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:13007-13011`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:13042-13056`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:13041`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:13040`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12949`,
  `:12965`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12982-12988`):
...
`src/tychoc.c:12856-12865`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12766-12785`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5343-5355` handles a
...
name (`src/tychoc.c:12775-12778`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12619`, `:12999`; `compile_package` starts the merge at
that name with an empty prefix, `:12786-12789`).
...
`src/tychoc.c:4299-4311`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4392-4401`, `is_imported_pkg` `:4413-4419`).
...
they are not reserved words (`src/tychoc.c:4281-4285`). A bound package name,
...
`:12759`, reset at `:12779`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4421-4428`; enforced at the
qualified-reference sites `:2133`, `:2959`, `:5347`, `:5612`). Every other top-level
...
`src/tychoc.c:4358-4373`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4377-4386`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12728-12792`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12732-12735`):
...
the note at `src/tychoc.c:4606-4608`).
