...
> Provenance: entry point `src/tychoc.c:9767@no 'main' procedure` (no `main`),
> `:9810@takes no parameters` and `:9813@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14719-14863@compile_package`, driver `:14965-15150@int main(`;
> `extern` `parse_extern_fn` `:4818-4921@parse_extern_fn`; the C compiler
> invocation `:15183@system(cmd)`.
...
`main` at `src/tychoc.c:14488@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:15099@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:15182@-fwrapv`,
run at `src/tychoc.c:15183@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15170@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15162@march`).
...
- The default C compiler is `cc` (`src/tychoc.c:15005@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:15041@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:15011-15014`):
...
(`src/tychoc.c:15044@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6633@payload-less` handles a
...
name (`src/tychoc.c:14728@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15098@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14753@merge_pkg`).
...
`src/tychoc.c:14721@g_cur_pkg_prefix`, reset at `src/tychoc.c:14742@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
`src/tychoc.c:3399@check_pkg_private`, `src/tychoc.c:6636@check_pkg_private`,
`src/tychoc.c:7004@check_pkg_private`). Every other top-level
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14649@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14648@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14706@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14724@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14738@prog->v`).
...
when the package is imported (`src/tychoc.c:14685@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14685@file_exists`); a `deps` file with
