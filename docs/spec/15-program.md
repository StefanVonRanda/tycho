...
> Provenance: entry point `src/tychoc.c:8720@no 'main' procedure` (no `main`),
> `:8751@takes no parameters` and `:8754@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13660-13665@compile_package`, driver `:13777-13881@int main(`;
> `extern` `parse_extern_fn` `:4338-4416@parse_extern_fn`; the C compiler
> invocation `:14002@system(cmd)`.
...
`main` at `src/tychoc.c:12558`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12760-12763`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12971-12975`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:13006-13020`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:13005`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:13004`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12913`,
  `:12929`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12946-12952`):
...
`src/tychoc.c:12820-12829`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12730-12749`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5328-5340` handles a
...
name (`src/tychoc.c:12739-12742`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12583`, `:12963`; `compile_package` starts the merge at
that name with an empty prefix, `:12750-12753`).
...
(`pkg_prefix_for` `:4377-4386`, `is_imported_pkg` `:4398-4404`).
...
`:12723`, reset at `:12743`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4406-4413`; enforced at the
qualified-reference sites `:2118`, `:2944`, `:5332`, `:5597`). Every other top-level
...
(`merge_pkg` `src/tychoc.c:12692-12756`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12696-12699`):
...
the note at `src/tychoc.c:4591-4593`).
