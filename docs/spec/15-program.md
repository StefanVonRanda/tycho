...
> `compile_package` `:14115-14234@compile_package`, driver `:14319-14494@int main(`;
> `extern` `parse_extern_fn` `:4547-4650@parse_extern_fn`; the C compiler
> invocation `:14520@system(cmd)`.
...
program** compiled alone (`src/tychoc.c:14443@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14519@-fwrapv`,
run at `src/tychoc.c:14520@system(cmd)`).
...
- The default C compiler is `cc` (`src/tychoc.c:14357@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14386@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14363-14365`):
...
(`src/tychoc.c:14389@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c:14124@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14442@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14149@merge_pkg`).
...
`src/tychoc.c:14117@g_cur_pkg_prefix`, reset at `src/tychoc.c:14138@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14045@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14044@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14102@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14120@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14134@prog->v`).
...
when the package is imported (`src/tychoc.c:14081@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14081@file_exists`); a `deps` file with
