...
> Provenance: entry point `src/tychoc.c:9863@no 'main' procedure` (no `main`),
> `:9906@takes no parameters` and `:9909@returns nothing or` (the two signature
...
`main` at `src/tychoc.c:14601@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:15223@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:15309@-fwrapv`,
run at `src/tychoc.c:15310@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15297@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15289@march`).
...
- The default C compiler is `cc` (`src/tychoc.c:15129@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:15165@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:15089-15092`):
...
(`src/tychoc.c:15166@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6720@payload-less` handles a
...
name (`src/tychoc.c:14847@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15222@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14877@merge_pkg`).
...
`src/tychoc.c:14840@g_cur_pkg_prefix`, reset at `src/tychoc.c:14861@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
`src/tychoc.c:3420@check_pkg_private`, `src/tychoc.c:6723@check_pkg_private`,
`src/tychoc.c:7091@check_pkg_private`). Every other top-level
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14762@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14761@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14825@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14843@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14855@prog->v`).
...
when the package is imported (`src/tychoc.c:14804@add_pkg_deps`):
