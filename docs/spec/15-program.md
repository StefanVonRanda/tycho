...
> Provenance: entry point `src/tychoc.c:8722@no 'main' procedure` (no `main`),
> `:8753@takes no parameters` and `:8756@returns nothing or` (the two signature
...
`main` at `src/tychoc.c:13262@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:13754@compile_package`). A single-file program has
...
- An `extern` declaration has **no body**: it ends at the newline after its
  signature (`src/tychoc.c:4380@an extern fn has no body`), so a following indented
  block is a parse error.
- Its `name` is the **literal C symbol** and is **not** package-mangled (§28.3),
  because a C symbol is global (`src/tychoc.c:4322@never mangled`). Two `extern` declarations naming the
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:13813@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:13812@march`).
- The default C compiler is `cc` (`src/tychoc.c:13700@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:13722@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:13729-13731`):
...
(`src/tychoc.c:13725@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5774@payload-less` handles a
...
name (`src/tychoc.c:13476@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:13753@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:13501@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:4893-4894`). A bound package name,
...
`src/tychoc.c:13469@g_cur_pkg_prefix`, reset at `src/tychoc.c:13490@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
qualified-reference sites `src/tychoc.c:2337@check_pkg_private`,
`src/tychoc.c:3003@check_pkg_private`, `src/tychoc.c:5777@check_pkg_private`,
`src/tychoc.c:6092@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:4726@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:4739@importer_dir`), independent of the current working
...
(`src/tychoc.c:4731@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13397@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13396@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:13454@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:13472@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:13486@prog->v`).
...
when the package is imported (`src/tychoc.c:13433@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5010@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:13433@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:4959@SKIPS`).
