...
> Provenance: entry point `src/tychoc.c:8301@no 'main' procedure` (no `main`),
> `:8326-8327@'main' must be` (the signature rule); compilation unit
> `compile_package` `:13166-13171@compile_package`, driver `:13225-13329@int main(`;
> `extern` `parse_extern_fn` `:4075-4145@parse_extern_fn`; the C compiler
> invocation `:13500@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:8301@no 'main' procedure`) and MUST
...
`:8326-8327@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
