...
> Provenance: entry point `src/tychoc.c:7181@no 'main' procedure` (no `main`),
> `:7206-7207@'main' must be` (the signature rule); compilation unit
> `compile_package` `:11522-11527@compile_package`, driver `:11681-11785@int main(`;
> `extern` `parse_extern_fn` `:3530-3600@parse_extern_fn`; the C compiler
> invocation `:11781@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:7181@no 'main' procedure`) and MUST
...
`:7206-7207@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
`parse_program`, `compiler/tychoc0.ty:3861@'main' must be`.
