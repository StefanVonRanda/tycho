...
> Provenance: entry point `src/tychoc.c:8735@no 'main' procedure` (no `main`),
> `:8766@takes no parameters` and `:8769@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13690-13695@compile_package`, driver `:13808-13912@int main(`;
> `extern` `parse_extern_fn` `:4353-4431@parse_extern_fn`; the C compiler
> invocation `:14033@system(cmd)`.
...
`main` at `src/tychoc.c:12573`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12775-12778`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12986-12990`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:13021-13035`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:13020`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:13019`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12928`,
  `:12944`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12961-12967`):
...
`src/tychoc.c:12835-12844`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12745-12764`).
...
name (`src/tychoc.c:12754-12757`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12598`, `:12978`; `compile_package` starts the merge at
that name with an empty prefix, `:12765-12768`).
...
`:12738`, reset at `:12758`; [packages.md](../guides/packages.md) §"How it builds").
...
(`merge_pkg` `src/tychoc.c:12707-12771`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12711-12714`):
