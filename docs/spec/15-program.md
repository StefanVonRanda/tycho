...
> Provenance: entry point `src/tychoc.c:8687@no 'main' procedure` (no `main`),
> `:8718@takes no parameters` and `:8721@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13627-13632@compile_package`, driver `:13744-13848@int main(`;
> `extern` `parse_extern_fn` `:4338-4408@parse_extern_fn`; the C compiler
> invocation `:13969@system(cmd)`.
...
`main` at `src/tychoc.c:12525`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12727-12730`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12938-12942`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12973-12987`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12972`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12971`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12880`,
  `:12896`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12913-12919`):
...
`src/tychoc.c:12787-12796`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12697-12716`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5312-5324` handles a
...
name (`src/tychoc.c:12706-12709`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12550`, `:12930`; `compile_package` starts the merge at
that name with an empty prefix, `:12717-12720`).
...
`:12690`, reset at `:12710`; [packages.md](../guides/packages.md) §"How it builds").
...
qualified-reference sites `:2118`, `:2944`, `:5316`, `:5581`). Every other top-level
...
(`merge_pkg` `src/tychoc.c:12659-12723`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12663-12666`):
