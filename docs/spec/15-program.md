...
> Provenance: entry point `src/tychoc.c:9933@no 'main' procedure` (no `main`),
> `:9976@takes no parameters` and `:9979@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14831-14975@compile_package`, driver `:15077-15262@int main(`;
> `extern` `parse_extern_fn` `:4873-4976@parse_extern_fn`; the C compiler
> invocation `:15388@system(cmd)`.
...
`main` at `src/tychoc.c:14671@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:15293@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:15387@-fwrapv`,
run at `src/tychoc.c:15388@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15367@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15359@march`).
...
- The default C compiler is `cc` (`src/tychoc.c:15199@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:15235@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:15159-15162`):
...
(`src/tychoc.c:15236@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c:14917@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15292@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14947@merge_pkg`).
...
`src/tychoc.c:14910@g_cur_pkg_prefix`, reset at `src/tychoc.c:14931@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14832@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14831@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14895@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14913@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14925@prog->v`).
...
when the package is imported (`src/tychoc.c:14874@add_pkg_deps`):
