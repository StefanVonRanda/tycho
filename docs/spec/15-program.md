...
> Provenance: entry point `src/tychoc.c:9885@no 'main' procedure` (no `main`),
> `:9928@takes no parameters` and `:9931@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14783-14927@compile_package`, driver `:15029-15214@int main(`;
> `extern` `parse_extern_fn` `:4859-4962@parse_extern_fn`; the C compiler
> invocation `:15340@system(cmd)`.
...
`main` at `src/tychoc.c:14623@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:15245@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:15339@-fwrapv`,
run at `src/tychoc.c:15340@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:15319@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:15311@march`).
...
- The default C compiler is `cc` (`src/tychoc.c:15151@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:15187@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:15111-15114`):
...
(`src/tychoc.c:15188@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c:14869@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:15244@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14899@merge_pkg`).
...
`src/tychoc.c:14862@g_cur_pkg_prefix`, reset at `src/tychoc.c:14883@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14784@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14783@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14847@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14865@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14877@prog->v`).
...
when the package is imported (`src/tychoc.c:14826@add_pkg_deps`):
