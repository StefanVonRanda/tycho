...
> Provenance: entry point `src/tychoc.c:8671@no 'main' procedure` (no `main`),
> `:8702@takes no parameters` and `:8705@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13606-13611@compile_package`, driver `:13723-13827@int main(`;
> `extern` `parse_extern_fn` `:4333-4403@parse_extern_fn`; the C compiler
> invocation `:13948@system(cmd)`.
...
`main` at `src/tychoc.c:12504`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12706-12709`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12917-12921`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12952-12966`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12951`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12950`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12859`,
  `:12875`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12892-12898`):
...
`src/tychoc.c:12766-12775`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12676-12695`).
...
name (`src/tychoc.c:12685-12688`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12529`, `:12909`; `compile_package` starts the merge at
that name with an empty prefix, `:12696-12699`).
...
`:12669`, reset at `:12689`; [packages.md](../guides/packages.md) §"How it builds").
...
(`merge_pkg` `src/tychoc.c:12638-12702`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12642-12645`):
