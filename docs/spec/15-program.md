...
> `compile_package` `:14014-14126@compile_package`, driver `:14209-14377@int main(`;
...
program** compiled alone (`src/tychoc.c:14296@compile_package`). A single-file program has
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14360@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14359@march`).
- The default C compiler is `cc` (`src/tychoc.c:14217@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14239@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14246-14248`):
...
(`src/tychoc.c:14242@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14295@detect_package`;
