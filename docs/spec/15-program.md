...
> Provenance: entry point `src/tychoc.c:8704@no 'main' procedure` (no `main`),
> `:8735@takes no parameters` and `:8738@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13644-13649@compile_package`, driver `:13761-13865@int main(`;
> `extern` `parse_extern_fn` `:4338-4408@parse_extern_fn`; the C compiler
> invocation `:13986@system(cmd)`.
...
`main` at `src/tychoc.c:12542`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12744-12747`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12955-12959`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12990-13004`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12989`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12988`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12897`,
  `:12913`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12930-12936`):
...
`src/tychoc.c:12804-12813`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12714-12733`).
...
name (`src/tychoc.c:12723-12726`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12567`, `:12947`; `compile_package` starts the merge at
that name with an empty prefix, `:12734-12737`).
...
`:12707`, reset at `:12727`; [packages.md](../guides/packages.md) §"How it builds").
...
(`merge_pkg` `src/tychoc.c:12676-12740`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12680-12683`):
