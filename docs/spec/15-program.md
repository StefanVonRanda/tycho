...
> Provenance: entry point `src/tychoc.c:9114@no 'main' procedure` (no `main`),
> `:9157@takes no parameters` and `:9160@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13921-14033@compile_package`, driver `:14116-14253@int main(`;
> `extern` `parse_extern_fn` `:4476-4579@parse_extern_fn`; the C compiler
> invocation `:14249@system(cmd)`.
...
`main` at `src/tychoc.c:13686@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14178@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14248@-fwrapv`,
run at `src/tychoc.c:14249@system(cmd)`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14242@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14241@march`).
- The default C compiler is `cc` (`src/tychoc.c:14124@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14146@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14153-14155`):
...
(`src/tychoc.c:14149@-L`). Every library/package name that reaches the shell — from
...
name (`src/tychoc.c:13900@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14177@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:13925@merge_pkg`).
...
`src/tychoc.c:13893@g_cur_pkg_prefix`, reset at `src/tychoc.c:13914@g_cur_pkg_prefix`; [packages.md](../guides/packages.md) §"How it builds").
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:13821@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:13820@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:13878@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:13896@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:13910@prog->v`).
...
when the package is imported (`src/tychoc.c:13857@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:13857@file_exists`); a `deps` file with
