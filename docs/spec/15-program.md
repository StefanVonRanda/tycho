...
> Provenance: entry point `src/tychoc.c:8630@no 'main' procedure` (no `main`),
> `:8661@takes no parameters` and `:8664@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:13540-13545@compile_package`, driver `:13657-13761@int main(`;
> `extern` `parse_extern_fn` `:4305-4375@parse_extern_fn`; the C compiler
> invocation `:13882@system(cmd)`.
...
`main` at `src/tychoc.c:12438`). `args()` is never empty for a normally-launched
program.
...
`src/tychoc.c:12640-12643`; [packages.md](../guides/packages.md) §"How it builds").
...
program** compiled alone (`src/tychoc.c:12851-12855`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:12886-12900`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`:12885`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`:12884`).
- The default C compiler is `cc`; `--cc <compiler>` overrides it (`:12793`,
  `:12809`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:12826-12832`):
...
`src/tychoc.c:12700-12709`). The `extern "Lib"` libraries, the auto-discovered
...
syntax"; merge at `src/tychoc.c:12610-12629`).
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:5279-5291` handles a
...
name (`src/tychoc.c:12619-12622`, checked against `pkg_basename` of the import
...
name (`detect_package` `:12463`, `:12843`; `compile_package` starts the merge at
that name with an empty prefix, `:12630-12633`).
...
`src/tychoc.c:4251-4263`). The plain form `import "PATH"` binds the package
...
(`pkg_prefix_for` `:4344-4353`, `is_imported_pkg` `:4357-4363`).
...
they are not reserved words (`src/tychoc.c:4233-4237`). A bound package name,
...
`:12603`, reset at `:12623`; [packages.md](../guides/packages.md) §"How it builds").
...
with a diagnostic (`check_pkg_private` `src/tychoc.c:4365-4372`; enforced at the
qualified-reference sites `:2116`, `:2942`, `:5283`, `:5548`). Every other top-level
...
`src/tychoc.c:4310-4325`): the environment variable `TYCHO_CORELIB` if set and
...
(`:4329-4338`). `core:` is currently the only collection root
...
(`merge_pkg` `src/tychoc.c:12572-12636`). For each package directory the
implementation MUST:
...
when the package is imported (`merge_pkg` `src/tychoc.c:12576-12579`):
...
the note at `src/tychoc.c:4550-4552`).
