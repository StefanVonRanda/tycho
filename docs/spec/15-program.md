...
> Provenance: entry point `src/tychoc.c:9440@no 'main' procedure` (no `main`),
> `:9483@takes no parameters` and `:9486@returns nothing or` (the two signature
> rules, diagnosed separately); compilation unit
> `compile_package` `:14331-14475@compile_package`, driver `:14560-14735@int main(`;
> `extern` `parse_extern_fn` `:4592-4695@parse_extern_fn`; the C compiler
> invocation `:14761@system(cmd)`.
...
`main` at `src/tychoc.c:14100@tycho_argc`). `args()` is never empty for a normally-launched
program.
...
program** compiled alone (`src/tychoc.c:14684@compile_package`). A single-file program has
...
executable with a single C-compiler invocation (`src/tychoc.c:14760@-fwrapv`,
run at `src/tychoc.c:14761@system(cmd)`).
...
- **`-fwrapv` is required semantics, not an optimization flag.** It makes signed
  ...
  (`src/tychoc.c:14760@-fwrapv`).
- **`-lm` is always passed**, so bare libc math externs (e.g. `extern fn sqrt`)
  link with no `"m"` annotation (`src/tychoc.c:5371@-lm is always passed`).
...
- **Optimization / debug:** `-O3` is the portable default; `-g` selects `-O0 -g`
  (unoptimized with debug info) instead (`src/tychoc.c:14748@optdbg`).
- **`-march=native` is opt-in** via `--native`. It is host-CPU-specific and MUST
  NOT be assumed portable across machines (`src/tychoc.c:14747@march`).
- The default C compiler is `cc` (`src/tychoc.c:14598@"cc"`); `--cc <compiler>`
  overrides it (`src/tychoc.c:14627@--cc`).

Three CLI options let a program splice additional flags onto this line for FFI
(`src/tychoc.c:14604-14606`):
...
(`src/tychoc.c:14630@-L`). Every library/package name that reaches the shell — from
...
— `geom.Point`, `geom.add`, `geom.Red` (`src/tychoc.c:6339@payload-less` handles a
...
name (`src/tychoc.c:14340@declares`, checked against `src/tychoc.c@pkg_basename` of
...
name (`src/tychoc.c@detect_package`, called at `src/tychoc.c:14683@detect_package`;
`compile_package` starts the merge at that name with an empty prefix,
`src/tychoc.c:14365@merge_pkg`).
...
they are not reserved words (`src/tychoc.c:5326-5327`). A bound package name,
...
`src/tychoc.c:14333@g_cur_pkg_prefix`, reset at `src/tychoc.c:14354@g_cur_pkg_prefix`; [packages.md](../reference/packages.md) §"How it builds").
...
`src/tychoc.c:3277@check_pkg_private`, `src/tychoc.c:6342@check_pkg_private`,
`src/tychoc.c:6689@check_pkg_private`). Every other top-level
...
- **`core:` collection.** If `PATH` begins with `core:`, it resolves to
  ...
  and binds `utf8` (`src/tychoc.c:5072@core:`, `src/tychoc.c@pkg_basename`).
- **Relative imports.** Any other `PATH` resolves relative to the **importing
  package's own directory** (`src/tychoc.c:5085@importer_dir`), independent of the current working
...
(`src/tychoc.c:5077@cannot find the corelib`). `core:` is currently the only collection root
...
2. **Scan its `.ty` files in sorted order.** All `*.ty` files in the directory
   ...
   sort at `src/tychoc.c:14261@qsort`). A directory with no `.ty` files is an
   error (`src/tychoc.c:14260@has no .ty files`).
3. **Load imports first (post-order).** Every import found in the package's file
   ...
   parsed (`src/tychoc.c:14318@load imported packages first`).
4. **Require the declared package name.** Every file in the directory MUST carry
   ...
   (`src/tychoc.c:14336@but has no`).
5. **Append into the one shared AST**, mangling each package's names by its
   `P__` prefix (§28.3) into the single flat namespace (`src/tychoc.c:14350@prog->v`).
...
when the package is imported (`src/tychoc.c:14297@add_pkg_deps`):
...
- **`deps` file (pkg-config).** When a shim is present, the package's sibling
  ...
  (`src/tychoc.c:5450@could not resolve dependency`); the missing library
  then surfaces as a link error at the `cc` stage. Note that
  `deps` is consulted **only alongside a shim** (`src/tychoc.c:14297@file_exists`); a `deps` file with
...
the note at `src/tychoc.c:5399@SKIPS`).
