...
> Provenance: entry point `src/tychoc.c:7797@no 'main' procedure` (no `main`),
> `:7822-7823@'main' must be` (the signature rule); compilation unit
> `compile_package` `:12330-12335@compile_package`, driver `:12489-12593@int main(`;
> `extern` `parse_extern_fn` `:3802-3872@parse_extern_fn`; the C compiler
> invocation `:12591@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:7797@no 'main' procedure`) and MUST
...
`:7822-7823@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
