...
> Provenance: entry point `src/tychoc.c:8296@no 'main' procedure` (no `main`),
> `:8321-8322@'main' must be` (the signature rule); compilation unit
> `compile_package` `:13161-13166@compile_package`, driver `:13220-13324@int main(`;
> `extern` `parse_extern_fn` `:4070-4140@parse_extern_fn`; the C compiler
> invocation `:13491@system(cmd)`.
...
diagnoses `no 'main' procedure`, `src/tychoc.c:8296@no 'main' procedure`) and MUST
...
`:8321-8322@'main' must be`; `compiler/tychoc0.ty` checks it at parse time in
