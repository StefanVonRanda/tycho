...
> Provenance: `Sig` builtins `src/tychoc.c:4634-4735`; conversion magic
> `:6217-6273`; `len` `:6275-6281`; `keys`/`push`/`pop`/`reserve` `:6333-6428`;
> `m.get` sugar `:5764-5777`,`:5629-5644`; `zero$` `:5712-5738`; concurrency
> magic `:6091-6174`; `map_*` removal `:2530-2531`; `die` codegen `:10494-10495`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4634-4735`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4641-4648`,`:4718-4719`,`:4728-4729`; `eprint` codegen `:10191@tycho_eprint`; `die` codegen
> `:10494-10495`.
...
> Provenance: conversion magic `src/tychoc.c:6051-6107`; `chr` and `to_char` `Sig`
> `:5217@.name="to_char"`, their shared codegen `:9867-9869`;
> `is_null`/`to_ptr` `Sig` `:4730-4731`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1044-1048` /
> `sized_conv_target` `:1033-1043`, resolved inline at `:6233-6239`. The abort
...
> Provenance: `len` `src/tychoc.c:6275-6281`; `push` `:6340-6380`; `pop`
> `:6381-6401`; `reserve` `:6402-6428`.
...
> Provenance: `zero$` `src/tychoc.c:5712-5738`; `defaultable` predicate `:8612@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6116-6123`; `channel` `:6124-6139`; `send`
> `:6140-6148`; `recv` `:6149-6154`; `close` `:6130-6174`; `ncpu` `Sig` `:5216@.name="ncpu"`;
> task/channel method sugar `:5739-5753`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11162@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4712-4713`;
> `read_file`/`write_file`/`list_dir` `:4725-4727`.
...
> Provenance: `src/tychoc.c:4731-4734`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5218@.name="die"`, codegen `:10494-10495`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4636-4737` or the
> `resolve_expr` magic block (`case E_CALL:` `:6067@case E_CALL:`, running
> through `reserve` at `:6725@"reserve"`).
