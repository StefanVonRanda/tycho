...
> Provenance: `Sig` builtins `src/tychoc.c:4768-4866`; conversion magic
> `:6052-6108`; `len` `:6110-6116`; `keys`/`push`/`pop`/`reserve` `:6166-6261`;
> `m.get` sugar `:5604-5617`,`:5462-5477`; `zero$` `:5552-5578`; concurrency
> magic `:5951-6034`; `map_*` removal `:2632-2633`; `die` codegen `:9682-9683`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4768-4866`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4773-4777`,`:4847-4848`,`:4857-4858`; `eprint` codegen `:10315@tycho_eprint`; `die` codegen
> `:9682-9683`.
...
> Provenance: conversion magic `src/tychoc.c:6052-6108`; `chr` and `to_char` `Sig`
> `:5278@.name="to_char"`, their shared codegen `:9722-9724`;
> `is_null`/`to_ptr` `Sig` `:4859-4860`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1119-1123` /
> `sized_conv_target` `:1108-1118`, resolved inline at `:6068-6074`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4850-4851`, `split` `:5285@.name="split"`;
> `len` magic
> `:6110-6116`; `char_at` `Sig` `src/tychoc.c:5284@.name="char_at"`, codegen `:9532-9539`
> (`tycho_str_get`, the same call `s[i]` emits at `:10842@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:6110-6116`; `push` `:6173-6213`; `pop`
> `:6214-6234`; `reserve` `:6235-6261`.
...
> Provenance: `keys` `src/tychoc.c:6166-6171`; `m.get` sugar `:5604-5617`,
> `:5462-5477`; `map_*` removal (parse error) `:2632-2633`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6355-6368`, codegen `:10218-10220`,
> `gen_hash` `src/tychoc.c:9956@gen_hash`; its type-emission gate is `hash_keyused`
> `src/tychoc.c:1534-1538`, OR'd into the hash-function gates so a `hash()` on
...
> Provenance: `zero$` `src/tychoc.c:5552-5578`; `defaultable` predicate `:8596@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5951-5958`; `channel` `:5959-5974`; `send`
> `:5975-5983`; `recv` `:5984-5989`; `close` `:5990-6034`; `ncpu` `Sig` `:5277@.name="ncpu"`;
> task/channel method sugar `:5579-5593`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11305@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4843-4844`;
> `read_file`/`write_file`/`list_dir` `:4854-4856`.
...
> Provenance: `src/tychoc.c:4862-4865`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5279@.name="die"`, codegen `:9682-9683`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4768-4866` or the
> `resolve_expr` magic block (`case E_CALL:` `:6068@case E_CALL:`, running
> through `reserve` at `:6712@"reserve"`).
