...
> Provenance: `Sig` builtins `src/tychoc.c:5140-5264`; conversion magic
> `:6953-7018`; `len` `:7020-7026`; `keys`/`push`/`pop`/`reserve` `:7078-7173`;
> `m.get` sugar `:6465-6478`,`:6330-6345`; `zero$` `:6413-6439`; concurrency
> magic `:6827-6910`; `map_*` removal `:2886-2887`; `die` codegen `:11405-11406`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:5140-5264`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:5271-5278`,`:5354-5355`,`:5364-5365`; `eprint` codegen `:10978@tycho_eprint`; `die` codegen
> `:11405-11406`.
...
> Provenance: conversion magic `src/tychoc.c:6653-6709`; `chr` and `to_char` `Sig`
> `:5788@.name="to_char"`, their shared codegen `:10731-10733`;
> `is_null`/`to_ptr` `Sig` `:5366-5367`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1282-1286` /
> `sized_conv_target` `:1271-1281`, resolved inline at `:6978-6984`. The abort
...
> Provenance: `len` `src/tychoc.c:7275@"len"`; `push` `src/tychoc.c:7338@"push"`;
> `pop` `src/tychoc.c:7377@"pop"`; `reserve` `src/tychoc.c:7398@"reserve"`.
...
> Provenance: `zero$` `src/tychoc.c:6413-6439`; `defaultable` predicate `:9365@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:7134@"wait"`; `channel` `src/tychoc.c:7145@"channel"`;
> `send` `src/tychoc.c:7153@"send"`; `recv` `src/tychoc.c:7162@"recv"`;
> `close` `src/tychoc.c:7168@"close"`; `ncpu` `Sig` `:5787@.name="ncpu"`;
> task/channel method sugar `:6440-6454`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:12009@_pk > 64`.
...
> Provenance: `clock` `src/tychoc.c:5785@.name="clock"`, `now` `src/tychoc.c:5786@.name="now"`;
> `read_file` `src/tychoc.c:5796@.name="read_file"`, `write_file` `src/tychoc.c:5797@.name="write_file"`,
> `list_dir` `src/tychoc.c:5798@.name="list_dir"`.
...
> Provenance: `src/tychoc.c:5260-5263`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5789@.name="die"`, codegen `:11405-11406`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:5266-5373` or the
> `resolve_expr` magic block (`case E_CALL:` `:6699@case E_CALL:`, running
> through `reserve` at `:7398@"reserve"`).
