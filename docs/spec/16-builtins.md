...
> `:6989-7054`; `len` `:7056-7062`; `keys`/`push`/`pop`/`reserve` `:7114-7209`;
> `m.get` sugar `:6480-6493`,`:6345-6360`; `zero$` `:6428-6454`; concurrency
> magic `:6863-6946`; `map_*` removal `:2886-2887`; `die` codegen `:11441-11442`.
...
> Provenance: `src/tychoc.c:5271-5278`,`:5354-5355`,`:5364-5365`; `eprint` codegen `:11014@tycho_eprint`; `die` codegen
> `:11441-11442`.
...
> Provenance: conversion magic `src/tychoc.c:6668-6745`; `chr` and `to_char` `Sig`
> `:5788@.name="to_char"`, their shared codegen `:10767-10769`;
> `is_null`/`to_ptr` `Sig` `:5366-5367`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1282-1286` /
> `sized_conv_target` `:1271-1281`, resolved inline at `:7014-7020`. The abort
...
> Provenance: `len` `src/tychoc.c:7311@"len"`; `push` `src/tychoc.c:7374@"push"`;
> `pop` `src/tychoc.c:7413@"pop"`; `reserve` `src/tychoc.c:7434@"reserve"`.
...
> Provenance: `zero$` `src/tychoc.c:6428-6454`; `defaultable` predicate `:9401@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:7170@"wait"`; `channel` `src/tychoc.c:7181@"channel"`;
> `send` `src/tychoc.c:7189@"send"`; `recv` `src/tychoc.c:7198@"recv"`;
> `close` `src/tychoc.c:7204@"close"`; `ncpu` `Sig` `:5787@.name="ncpu"`;
> task/channel method sugar `:6455-6469`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:12045@_pk > 64`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5789@.name="die"`, codegen `:11441-11442`; `exit` `Sig`
...
> `resolve_expr` magic block (`case E_CALL:` `:6735@case E_CALL:`, running
> through `reserve` at `:7434@"reserve"`).
