...
> Provenance: `Sig` builtins `src/tychoc.c:4860-4958`; conversion magic
> `:6169-6225`; `len` `:6227-6233`; `keys`/`push`/`pop`/`reserve` `:6283-6378`;
> `m.get` sugar `:5716-5729`,`:5574-5589`; `zero$` `:5664-5690`; concurrency
> magic `:6043-6126`; `map_*` removal `:2648-2649`; `die` codegen `:9871-9872`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4860-4958`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4865-4869`,`:4939-4940`,`:4949-4950`; `eprint` codegen `:10504@tycho_eprint`; `die` codegen
> `:9871-9872`.
...
> Provenance: conversion magic `src/tychoc.c:6169-6225`; `chr` and `to_char` `Sig`
> `:5390@.name="to_char"`, their shared codegen `:9911-9913`;
> `is_null`/`to_ptr` `Sig` `:4951-4952`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1130-1134` /
> `sized_conv_target` `:1119-1129`, resolved inline at `:6185-6191`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4942-4943`, `split` `:5397@.name="split"`;
> `len` magic
> `:6227-6233`; `char_at` `Sig` `src/tychoc.c:5396@.name="char_at"`, codegen `:9721-9728`
> (`tycho_str_get`, the same call `s[i]` emits at `:11031@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:6227-6233`; `push` `:6290-6330`; `pop`
> `:6331-6351`; `reserve` `:6352-6378`.
...
> Provenance: `keys` `src/tychoc.c:6283-6288`; `m.get` sugar `:5716-5729`,
> `:5574-5589`; `map_*` removal (parse error) `:2648-2649`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6472-6485`, codegen `:10407-10409`,
> `gen_hash` `src/tychoc.c:10145@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5664-5690`; `defaultable` predicate `:8771@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6068-6075`; `channel` `:6076-6091`; `send`
> `:6092-6100`; `recv` `:6101-6106`; `close` `:6082-6126`; `ncpu` `Sig` `:5389@.name="ncpu"`;
> task/channel method sugar `:5691-5705`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11506@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4935-4936`;
> `read_file`/`write_file`/`list_dir` `:4946-4948`.
...
> Provenance: `src/tychoc.c:4954-4957`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5391@.name="die"`, codegen `:9871-9872`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4860-4958` or the
> `resolve_expr` magic block (`case E_CALL:` `:6185@case E_CALL:`, running
> through `reserve` at `:6829@"reserve"`).
