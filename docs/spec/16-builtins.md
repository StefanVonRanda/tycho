...
> Provenance: `Sig` builtins `src/tychoc.c:4542-4574`; conversion magic
> `:5690-5746`; `len` `:5748-5754`; `keys`/`push`/`pop`/`reserve` `:5804-5893`;
> `m.get` sugar `:5294-5307`,`:5162-5177`; `zero$` `:5252-5268`; concurrency
> magic `:5628-5672`; `map_*` removal `:2489-2490`; `die` codegen `:9211-9212`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4542-4574`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4546-4550`,`:4555-4556`,`:4565-4566`; `eprint` codegen `:9819@tycho_eprint`; `die` codegen
> `:9211-9212`.
...
> Provenance: conversion magic `src/tychoc.c:5690-5746`; `chr` and `to_char` `Sig`
> `:4986@.name="to_char"`, their shared codegen `:9251-9253`;
> `is_null`/`to_ptr` `Sig` `:4567-4568`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1057-1061` /
> `sized_conv_target` `:1046-1056`, resolved inline at `:5706-5712`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4558-4559`, `split` `:4993@.name="split"`;
> `len` magic
> `:5748-5754`; `char_at` `Sig` `src/tychoc.c:4992@.name="char_at"`, codegen `:9061-9068`
> (`tycho_str_get`, the same call `s[i]` emits at `:10346@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:5748-5754`; `push` `:5811-5845`; `pop`
> `:5846-5866`; `reserve` `:5867-5893`.
...
> Provenance: `keys` `src/tychoc.c:5804-5809`; `m.get` sugar `:5294-5307`,
> `:5162-5177`; `map_*` removal (parse error) `:2489-2490`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:5987-6000`, codegen `:9722-9724`,
> `gen_hash` `src/tychoc.c:9460@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5252-5268`; `defaultable` predicate `:8135@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5628-5635`; `channel` `:5636-5646`; `send`
> `:5647-5655`; `recv` `:5656-5661`; `close` `:5662-5672`; `ncpu` `Sig` `:4985@.name="ncpu"`;
> task/channel method sugar `:5269-5283`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:10779@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4551-4552`;
> `read_file`/`write_file`/`list_dir` `:4562-4564`.
...
> Provenance: `src/tychoc.c:4570-4573`.
...
> Provenance: `die` `Sig` `src/tychoc.c:4987@.name="die"`, codegen `:9211-9212`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4542-4574` or the
> `resolve_expr` magic block (`case E_CALL:` `:5706@case E_CALL:`, running
> through `reserve` at `:6344@"reserve"`).
