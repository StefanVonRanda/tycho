...
> Provenance: `Sig` builtins `src/tychoc.c:4556-4654`; conversion magic
> `:6124-6180`; `len` `:6182-6188`; `keys`/`push`/`pop`/`reserve` `:6238-6333`;
> `m.get` sugar `:5671-5684`,`:5536-5551`; `zero$` `:5619-5645`; concurrency
> magic `:5998-6081`; `map_*` removal `:2454-2455`; `die` codegen `:10369-10370`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4556-4654`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4563-4567`,`:4637-4638`,`:4647-4648`; `eprint` codegen `:10066@tycho_eprint`; `die` codegen
> `:10369-10370`.
...
> Provenance: conversion magic `src/tychoc.c:5958-6014`; `chr` and `to_char` `Sig`
> `:5124@.name="to_char"`, their shared codegen `:9742-9744`;
> `is_null`/`to_ptr` `Sig` `:4649-4650`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:971-975` /
> `sized_conv_target` `:960-970`, resolved inline at `:6140-6146`. The abort
> both share is `runtime/tycho_rt.c:1400@out of byte range`. Conformance:
...
> Provenance: `len` `src/tychoc.c:6182-6188`; `push` `:6245-6285`; `pop`
> `:6286-6306`; `reserve` `:6307-6333`.
...
> Provenance: `zero$` `src/tychoc.c:5619-5645`; `defaultable` predicate `:8499@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6023-6030`; `channel` `:6031-6046`; `send`
> `:6047-6055`; `recv` `:6056-6061`; `close` `:6037-6081`; `ncpu` `Sig` `:5123@.name="ncpu"`;
> task/channel method sugar `:5646-5660`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11037@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4631-4632`;
> `read_file`/`write_file`/`list_dir` `:4644-4646`.
...
> Provenance: `src/tychoc.c:4650-4653`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5125@.name="die"`, codegen `:10369-10370`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4558-4656` or the
> `resolve_expr` magic block (`case E_CALL:` `:5974@case E_CALL:`, running
> through `reserve` at `:6630@"reserve"`).
