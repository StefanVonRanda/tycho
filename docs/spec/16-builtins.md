...
> Provenance: `Sig` builtins `src/tychoc.c:4672-4704`; conversion magic
> `:5890-5946`; `len` `:5948-5954`; `keys`/`push`/`pop`/`reserve` `:6004-6099`;
> `m.get` sugar `:5442-5455`,`:5300-5315`; `zero$` `:5390-5416`; concurrency
> magic `:5789-5872`; `map_*` removal `:2562-2563`; `die` codegen `:9504-9505`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4672-4704`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4676-4680`,`:4685-4686`,`:4695-4696`; `eprint` codegen `:10116@tycho_eprint`; `die` codegen
> `:9504-9505`.
...
> Provenance: conversion magic `src/tychoc.c:5890-5946`; `chr` and `to_char` `Sig`
> `:5116@.name="to_char"`, their shared codegen `:9544-9546`;
> `is_null`/`to_ptr` `Sig` `:4697-4698`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1119-1123` /
> `sized_conv_target` `:1108-1118`, resolved inline at `:5906-5912`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4688-4689`, `split` `:5123@.name="split"`;
> `len` magic
> `:5948-5954`; `char_at` `Sig` `src/tychoc.c:5122@.name="char_at"`, codegen `:9354-9361`
> (`tycho_str_get`, the same call `s[i]` emits at `:10643@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:5948-5954`; `push` `:6011-6051`; `pop`
> `:6052-6072`; `reserve` `:6073-6099`.
...
> Provenance: `keys` `src/tychoc.c:6004-6009`; `m.get` sugar `:5442-5455`,
> `:5300-5315`; `map_*` removal (parse error) `:2562-2563`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6193-6206`, codegen `:10019-10021`,
> `gen_hash` `src/tychoc.c:9757@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5390-5416`; `defaultable` predicate `:8418@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5789-5796`; `channel` `:5797-5812`; `send`
> `:5813-5821`; `recv` `:5822-5827`; `close` `:5828-5872`; `ncpu` `Sig` `:5115@.name="ncpu"`;
> task/channel method sugar `:5417-5431`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11106@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4681-4682`;
> `read_file`/`write_file`/`list_dir` `:4692-4694`.
...
> Provenance: `src/tychoc.c:4700-4703`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5117@.name="die"`, codegen `:9504-9505`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4672-4704` or the
> `resolve_expr` magic block (`case E_CALL:` `:5906@case E_CALL:`, running
> through `reserve` at `:6550@"reserve"`).
