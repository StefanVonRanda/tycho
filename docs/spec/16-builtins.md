...
> Provenance: `src/tychoc.c:4510-4514`,`:4519-4520`,`:4529-4530`; `eprint` codegen `:9445@tycho_eprint`; `die` codegen
> `:9150-9151`.
...
> `:4752@.name="to_char"`, their shared codegen `:9190-9192`;
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4522-4523`, `split` `:4759@.name="split"`;
> `len` magic
> `:5708-5714`; `char_at` `Sig` `src/tychoc.c:4758@.name="char_at"`, codegen `:9000-9007`
> (`tycho_str_get`, the same call `s[i]` emits at `:9962@tycho_str_get`), tychoc0
...
> Provenance: `zero$` `src/tychoc.c:5216-5232`; `defaultable` predicate `:7815@"defaultable"`.
...
> `:5607-5615`; `recv` `:5616-5621`; `close` `:5622-5632`; `ncpu` `Sig` `:4751@.name="ncpu"`;
...
> 64 is `src/tychoc.c:10385@_pk > 64`.
...
> Provenance: `die` `Sig` `src/tychoc.c:4753@.name="die"`, codegen `:9150-9151`; `exit` `Sig`
...
> `resolve_expr` magic block (`case E_CALL:` `:5460@case E_CALL:`, running
> through `reserve` at `:6072@"reserve"`).
