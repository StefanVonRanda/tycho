...
> Provenance: `Sig` builtins `src/tychoc.c:4854-4952`; conversion magic
> `:6163-6219`; `len` `:6221-6227`; `keys`/`push`/`pop`/`reserve` `:6277-6372`;
> `m.get` sugar `:5710-5723`,`:5568-5583`; `zero$` `:5658-5684`; concurrency
> magic `:6037-6120`; `map_*` removal `:2648-2649`; `die` codegen `:9856-9857`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4854-4952`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4859-4863`,`:4933-4934`,`:4943-4944`; `eprint` codegen `:10489@tycho_eprint`; `die` codegen
> `:9856-9857`.
...
> Provenance: conversion magic `src/tychoc.c:6163-6219`; `chr` and `to_char` `Sig`
> `:5384@.name="to_char"`, their shared codegen `:9896-9898`;
> `is_null`/`to_ptr` `Sig` `:4945-4946`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1130-1134` /
> `sized_conv_target` `:1119-1129`, resolved inline at `:6179-6185`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4936-4937`, `split` `:5391@.name="split"`;
> `len` magic
> `:6221-6227`; `char_at` `Sig` `src/tychoc.c:5390@.name="char_at"`, codegen `:9706-9713`
> (`tycho_str_get`, the same call `s[i]` emits at `:11016@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:6221-6227`; `push` `:6284-6324`; `pop`
> `:6325-6345`; `reserve` `:6346-6372`.
...
> Provenance: `keys` `src/tychoc.c:6277-6282`; `m.get` sugar `:5710-5723`,
> `:5568-5583`; `map_*` removal (parse error) `:2648-2649`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6466-6479`, codegen `:10392-10394`,
> `gen_hash` `src/tychoc.c:10130@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5658-5684`; `defaultable` predicate `:8756@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6062-6069`; `channel` `:6070-6085`; `send`
> `:6086-6094`; `recv` `:6095-6100`; `close` `:6076-6120`; `ncpu` `Sig` `:5383@.name="ncpu"`;
> task/channel method sugar `:5685-5699`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11491@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4929-4930`;
> `read_file`/`write_file`/`list_dir` `:4940-4942`.
...
> Provenance: `src/tychoc.c:4948-4951`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5385@.name="die"`, codegen `:9856-9857`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4854-4952` or the
> `resolve_expr` magic block (`case E_CALL:` `:6179@case E_CALL:`, running
> through `reserve` at `:6823@"reserve"`).
