...
> Provenance: `Sig` builtins `src/tychoc.c:4684-4716`; conversion magic
> `:5902-5958`; `len` `:5960-5966`; `keys`/`push`/`pop`/`reserve` `:6016-6111`;
> `m.get` sugar `:5454-5467`,`:5312-5327`; `zero$` `:5402-5428`; concurrency
> magic `:5801-5884`; `map_*` removal `:2574-2575`; `die` codegen `:9530-9531`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4684-4716`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4688-4692`,`:4697-4698`,`:4707-4708`; `eprint` codegen `:10142@tycho_eprint`; `die` codegen
> `:9530-9531`.
...
> Provenance: conversion magic `src/tychoc.c:5902-5958`; `chr` and `to_char` `Sig`
> `:5128@.name="to_char"`, their shared codegen `:9570-9572`;
> `is_null`/`to_ptr` `Sig` `:4709-4710`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1119-1123` /
> `sized_conv_target` `:1108-1118`, resolved inline at `:5918-5924`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4700-4701`, `split` `:5135@.name="split"`;
> `len` magic
> `:5960-5966`; `char_at` `Sig` `src/tychoc.c:5134@.name="char_at"`, codegen `:9380-9387`
> (`tycho_str_get`, the same call `s[i]` emits at `:10669@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:5960-5966`; `push` `:6023-6063`; `pop`
> `:6064-6084`; `reserve` `:6085-6111`.
...
> Provenance: `keys` `src/tychoc.c:6016-6021`; `m.get` sugar `:5454-5467`,
> `:5312-5327`; `map_*` removal (parse error) `:2574-2575`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6205-6218`, codegen `:10045-10047`,
> `gen_hash` `src/tychoc.c:9783@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5402-5428`; `defaultable` predicate `:8444@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5801-5808`; `channel` `:5809-5824`; `send`
> `:5825-5833`; `recv` `:5834-5839`; `close` `:5840-5884`; `ncpu` `Sig` `:5127@.name="ncpu"`;
> task/channel method sugar `:5429-5443`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11132@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4693-4694`;
> `read_file`/`write_file`/`list_dir` `:4704-4706`.
...
> Provenance: `src/tychoc.c:4712-4715`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5129@.name="die"`, codegen `:9530-9531`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4684-4716` or the
> `resolve_expr` magic block (`case E_CALL:` `:5918@case E_CALL:`, running
> through `reserve` at `:6562@"reserve"`).
