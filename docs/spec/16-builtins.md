...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:5097-5221`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `zero$` `src/tychoc.c:6370-6396`; `defaultable` predicate `:9322@"defaultable"`.
...
> Provenance: `src/tychoc.c:5217-5220`.
...
> `assert`/`panic`/`abort` name in `register_builtins` `:5223-5330` or the
> `resolve_expr` magic block (`case E_CALL:` `:6656@case E_CALL:`, running
> through `reserve` at `:7355@"reserve"`).
