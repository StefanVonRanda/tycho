...
> Provenance: `Sig` builtins `src/tychoc.c:4654-4686`; conversion magic
> `:5835-5891`; `len` `:5893-5899`; `keys`/`push`/`pop`/`reserve` `:5949-6038`;
> `m.get` sugar `:5414-5427`,`:5282-5297`; `zero$` `:5372-5388`; concurrency
> magic `:5755-5817`; `map_*` removal `:2544-2545`; `die` codegen `:9419-9420`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4654-4686`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4658-4662`,`:4667-4668`,`:4677-4678`; `eprint` codegen `:10031@tycho_eprint`; `die` codegen
> `:9419-9420`.
...
> Provenance: conversion magic `src/tychoc.c:5835-5891`; `chr` and `to_char` `Sig`
> `:5098@.name="to_char"`, their shared codegen `:9459-9461`;
> `is_null`/`to_ptr` `Sig` `:4679-4680`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1104-1108` /
> `sized_conv_target` `:1093-1103`, resolved inline at `:5851-5857`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4670-4671`, `split` `:5105@.name="split"`;
> `len` magic
> `:5893-5899`; `char_at` `Sig` `src/tychoc.c:5104@.name="char_at"`, codegen `:9269-9276`
> (`tycho_str_get`, the same call `s[i]` emits at `:10558@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:5893-5899`; `push` `:5956-5990`; `pop`
> `:5991-6011`; `reserve` `:6012-6038`.
...
> Provenance: `keys` `src/tychoc.c:5949-5954`; `m.get` sugar `:5414-5427`,
> `:5282-5297`; `map_*` removal (parse error) `:2544-2545`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6132-6145`, codegen `:9934-9936`,
> `gen_hash` `src/tychoc.c:9672@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5372-5388`; `defaultable` predicate `:8333@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5755-5762`; `channel` `:5763-5778`; `send`
> `:5779-5787`; `recv` `:5788-5793`; `close` `:5794-5817`; `ncpu` `Sig` `:5097@.name="ncpu"`;
> task/channel method sugar `:5389-5403`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11021@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4663-4664`;
> `read_file`/`write_file`/`list_dir` `:4674-4676`.
...
> Provenance: `src/tychoc.c:4682-4685`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5099@.name="die"`, codegen `:9419-9420`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4654-4686` or the
> `resolve_expr` magic block (`case E_CALL:` `:5851@case E_CALL:`, running
> through `reserve` at `:6489@"reserve"`).
