...
> Provenance: `Sig` builtins `src/tychoc.c:4733-4840`; conversion magic
> `:6463-6519`; `len` `:6521-6527`; `keys`/`push`/`pop`/`reserve` `:6579-6674`;
> `m.get` sugar `:6010-6023`,`:5875-5890`; `zero$` `:5958-5984`; concurrency
> magic `:6337-6420`; `map_*` removal `:2730-2731`; `die` codegen `:10781-10782`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4733-4840`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4847-4854`,`:4930-4931`,`:4940-4941`; `eprint` codegen `:10371@tycho_eprint`; `die` codegen
> `:10781-10782`.
...
> Provenance: conversion magic `src/tychoc.c:6190-6246`; `chr` and `to_char` `Sig`
> `:5356@.name="to_char"`, their shared codegen `:10154-10156`;
> `is_null`/`to_ptr` `Sig` `:4942-4943`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1225-1229` /
> `sized_conv_target` `:1214-1224`, resolved inline at `:6479-6485`. The abort
...
> Provenance: `len` `src/tychoc.c:6521-6527`; `push` `:6586-6626`; `pop`
> `:6627-6647`; `reserve` `:6648-6674`.
...
> Provenance: `zero$` `src/tychoc.c:5958-5984`; `defaultable` predicate `:8791@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6362-6369`; `channel` `:6370-6385`; `send`
> `:6386-6394`; `recv` `:6395-6400`; `close` `:6376-6420`; `ncpu` `Sig` `:5355@.name="ncpu"`;
> task/channel method sugar `:5985-5999`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11342@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4817-4818`;
> `read_file`/`write_file`/`list_dir` `:4937-4939`.
...
> Provenance: `src/tychoc.c:4836-4839`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5357@.name="die"`, codegen `:10781-10782`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4842-4949` or the
> `resolve_expr` magic block (`case E_CALL:` `:6206@case E_CALL:`, running
> through `reserve` at `:6864@"reserve"`).
