...
> Provenance: `src/tychoc.c:4510-4514`,`:4519-4520`,`:4529-4530`; `eprint` codegen `:9463@tycho_eprint`; `die` codegen
> `:9150-9151`.
...
> (`tycho_str_get`, the same call `s[i]` emits at `:9980@tycho_str_get`), tychoc0
...
> Provenance: `zero$` `src/tychoc.c:5216-5232`; `defaultable` predicate `:7831@"defaultable"`.
...
> 64 is `src/tychoc.c:10403@_pk > 64`.
...
> `resolve_expr` magic block (`case E_CALL:` `:5468@case E_CALL:`, running
> through `reserve` at `:6086@"reserve"`).
