...
> Provenance: `Sig` builtins `src/tychoc.c:4555-4653`; conversion magic
> `:5861-5917`; `len` `:5919-5925`; `keys`/`push`/`pop`/`reserve` `:5975-6070`;
> `m.get` sugar `:5408-5421`,`:5273-5288`; `zero$` `:5356-5382`; concurrency
> magic `:5735-5818`; `map_*` removal `:2451-2452`; `die` codegen `:10036-10037`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4555-4653`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4560-4564`,`:4634-4635`,`:4644-4645`; `eprint` codegen `:9945@tycho_eprint`; `die` codegen
> `:10036-10037`.
...
> Provenance: conversion magic `src/tychoc.c:5861-5917`; `chr` and `to_char` `Sig`
> `:5089@.name="to_char"`, their shared codegen `:9409-9411`;
> `is_null`/`to_ptr` `Sig` `:4646-4647`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:971-975` /
> `sized_conv_target` `:960-970`, resolved inline at `:5877-5883`. The abort
...
> Provenance: `len` `src/tychoc.c:5919-5925`; `push` `:5982-6022`; `pop`
> `:6023-6043`; `reserve` `:6044-6070`.
...
> Provenance: `zero$` `src/tychoc.c:5356-5382`; `defaultable` predicate `:8397@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5760-5767`; `channel` `:5768-5783`; `send`
> `:5784-5792`; `recv` `:5793-5798`; `close` `:5774-5818`; `ncpu` `Sig` `:5088@.name="ncpu"`;
> task/channel method sugar `:5383-5397`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:10916@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4630-4631`;
> `read_file`/`write_file`/`list_dir` `:4641-4643`.
...
> Provenance: `src/tychoc.c:4649-4652`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5090@.name="die"`, codegen `:10036-10037`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4555-4653` or the
> `resolve_expr` magic block (`case E_CALL:` `:5877@case E_CALL:`, running
> through `reserve` at `:6533@"reserve"`).
