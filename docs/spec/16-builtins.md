...
> Provenance: `src/tychoc.c:4340-4344`,`:4349-4350`,`:4359-4360`; `eprint` codegen `:9015@tycho_eprint`; `die` codegen
> `:8791-8792`.
...
> Provenance: conversion magic `src/tychoc.c:5480-5536`; `chr` `Sig` `:4521@.name="chr"`,
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4352-4353`, `split` `:4528@.name="split"`;
> `len` magic
> `:5538-5544`; `char_at` `Sig` `src/tychoc.c:4527@.name="char_at"`, codegen `:8641-8648`
> (`tycho_str_get`, the same call `s[i]` emits at `:9518@tycho_str_get`), tychoc0
...
> Provenance: `zero$` `src/tychoc.c:5046-5062`; `defaultable` predicate `:7454@"defaultable"`.
...
> `:5437-5445`; `recv` `:5446-5451`; `close` `:5452-5462`; `ncpu` `Sig` `:4520@.name="ncpu"`;
...
> Provenance: `die` `Sig` `src/tychoc.c:4522@.name="die"`, codegen `:8791-8792`; `exit` `Sig`
...
> `resolve_expr` magic block (`case E_CALL:` `:5218@case E_CALL:`, running
> through `reserve` at `:5830@"reserve"`).
