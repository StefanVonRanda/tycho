...
> magic `:6141-6224`; `map_*` removal `:2690-2691`; `die` codegen `:10003-10004`.
...
> Provenance: `src/tychoc.c:4957-4961`,`:5031-5032`,`:5041-5042`; `eprint` codegen `:10636@tycho_eprint`; `die` codegen
> `:10003-10004`.
...
> `:5482@.name="to_char"`, their shared codegen `:10043-10045`;
...
> `len` magic
> `:6325-6331`; `char_at` `Sig` `src/tychoc.c:5488@.name="char_at"`, codegen `:9853-9860`
> (`tycho_str_get`, the same call `s[i]` emits at `:11163@tycho_str_get`), tychoc0
...
> form). `hash` resolve `src/tychoc.c:6577-6590`, codegen `:10539-10541`,
> `gen_hash` `src/tychoc.c:10277@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5756-5782`; `defaultable` predicate `:8895@"defaultable"`.
...
> 64 is `src/tychoc.c:11638@_pk > 64`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5483@.name="die"`, codegen `:10003-10004`; `exit` `Sig`
...
> through `reserve` at `:6934@"reserve"`).
