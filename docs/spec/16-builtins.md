...
> Provenance: `Sig` builtins `src/tychoc.c:4691-4723`; conversion magic
> `:5909-5965`; `len` `:5967-5973`; `keys`/`push`/`pop`/`reserve` `:6023-6118`;
> `m.get` sugar `:5461-5474`,`:5319-5334`; `zero$` `:5409-5435`; concurrency
> magic `:5808-5891`; `map_*` removal `:2581-2582`; `die` codegen `:9539-9540`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4691-4723`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4695-4699`,`:4704-4705`,`:4714-4715`; `eprint` codegen `:10151@tycho_eprint`; `die` codegen
> `:9539-9540`.
...
> Provenance: conversion magic `src/tychoc.c:5909-5965`; `chr` and `to_char` `Sig`
> `:5135@.name="to_char"`, their shared codegen `:9579-9581`;
> `is_null`/`to_ptr` `Sig` `:4716-4717`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1119-1123` /
> `sized_conv_target` `:1108-1118`, resolved inline at `:5925-5931`. The abort
...
> Provenance: `substr`/`find` `Sig` `src/tychoc.c:4707-4708`, `split` `:5142@.name="split"`;
> `len` magic
> `:5967-5973`; `char_at` `Sig` `src/tychoc.c:5141@.name="char_at"`, codegen `:9389-9396`
> (`tycho_str_get`, the same call `s[i]` emits at `:10678@tycho_str_get`), tychoc0
...
> Provenance: `len` `src/tychoc.c:5967-5973`; `push` `:6030-6070`; `pop`
> `:6071-6091`; `reserve` `:6092-6118`.
...
> Provenance: `keys` `src/tychoc.c:6023-6028`; `m.get` sugar `:5461-5474`,
> `:5319-5334`; `map_*` removal (parse error) `:2581-2582`, tychoc0
> `compiler/tychoc0.ty:932-935` (expression form) and `:1678-1681` (statement
> form). `hash` resolve `src/tychoc.c:6212-6225`, codegen `:10054-10056`,
> `gen_hash` `src/tychoc.c:9792@gen_hash`; its type-emission gate is `hash_keyused`
...
> Provenance: `zero$` `src/tychoc.c:5409-5435`; `defaultable` predicate `:8453@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:5808-5815`; `channel` `:5816-5831`; `send`
> `:5832-5840`; `recv` `:5841-5846`; `close` `:5847-5891`; `ncpu` `Sig` `:5134@.name="ncpu"`;
> task/channel method sugar `:5436-5450`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11141@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4700-4701`;
> `read_file`/`write_file`/`list_dir` `:4711-4713`.
...
> Provenance: `src/tychoc.c:4719-4722`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5136@.name="die"`, codegen `:9539-9540`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4691-4723` or the
> `resolve_expr` magic block (`case E_CALL:` `:5925@case E_CALL:`, running
> through `reserve` at `:6569@"reserve"`).
