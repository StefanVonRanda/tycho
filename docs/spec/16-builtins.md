...
> Provenance: `Sig` builtins `src/tychoc.c:4600-4698`; conversion magic
> `:6168-6224`; `len` `:6226-6232`; `keys`/`push`/`pop`/`reserve` `:6282-6377`;
> `m.get` sugar `:5715-5728`,`:5580-5595`; `zero$` `:5663-5689`; concurrency
> magic `:6042-6125`; `map_*` removal `:2498-2499`; `die` codegen `:10425-10426`.
...
- A name is a **builtin** (part of the language) iff `register_builtins`
  (`src/tychoc.c:4600-4698`) registers it **or** a case in `resolve_expr` /
...
> Provenance: `src/tychoc.c:4607-4611`,`:4681-4682`,`:4691-4692`; `eprint` codegen `:10122@tycho_eprint`; `die` codegen
> `:10425-10426`.
...
> Provenance: conversion magic `src/tychoc.c:6002-6058`; `chr` and `to_char` `Sig`
> `:5168@.name="to_char"`, their shared codegen `:9798-9800`;
> `is_null`/`to_ptr` `Sig` `:4693-4694`. `to_i32` (and the rest of
> `to_u8`..`to_f32`) is **not** a `Sig`: it is `is_sized_conv` `:1015-1019` /
> `sized_conv_target` `:1004-1014`, resolved inline at `:6184-6190`. The abort
...
> Provenance: `len` `src/tychoc.c:6226-6232`; `push` `:6289-6329`; `pop`
> `:6330-6350`; `reserve` `:6351-6377`.
...
> Provenance: `zero$` `src/tychoc.c:5663-5689`; `defaultable` predicate `:8543@"defaultable"`.
...
> Provenance: `wait` `src/tychoc.c:6067-6074`; `channel` `:6075-6090`; `send`
> `:6091-6099`; `recv` `:6100-6105`; `close` `:6081-6125`; `ncpu` `Sig` `:5167@.name="ncpu"`;
> task/channel method sugar `:5690-5704`. `ncpu()`'s value is
...
> 64 is `src/tychoc.c:11093@_pk > 64`.
...
> Provenance: `clock`/`now` `src/tychoc.c:4675-4676`;
> `read_file`/`write_file`/`list_dir` `:4688-4690`.
...
> Provenance: `src/tychoc.c:4694-4697`.
...
> Provenance: `die` `Sig` `src/tychoc.c:5169@.name="die"`, codegen `:10425-10426`; `exit` `Sig`
...
> `assert`/`panic`/`abort` name in `register_builtins` `:4602-4700` or the
> `resolve_expr` magic block (`case E_CALL:` `:6018@case E_CALL:`, running
> through `reserve` at `:6674@"reserve"`).
