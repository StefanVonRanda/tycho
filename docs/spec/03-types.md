...
> Provenance: scalar tags `src/tychoc.c:584-605`; C lowering `c_type`
> `:1389-1429`; equality/ordering `:6182-6215`; newtype decl `parse_typedecl`
> `:4213-4232`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2373@elem`); the
> fixed forms test both (`src/tychoc.c:2066-2067`), as does `bounded[N]T`
> (`src/tychoc.c:1981-1982`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1324-1353`; `key_hashable` `:1276-1290`.
...
> `src/tychoc.c:2251-2268@"bounded"` (capacity `:1914-1923`, element
> restriction `:1979-1980`); its twin
...
> (`arrc_sized_b` `src/tychoc.c:1049-1061@arrc_sized_b`, messages `:958@task_container_err` and `:998@chan_container_err`) and an
> explicit check at `compiler/tychoc0.ty:1890-1896@ck_affine_part`.
> Rejections: slice `src/tychoc.c:5274-5275`, `pop` `:5954-5955`, `reserve`
> `:6453@reserve does not apply to a bounded`, over-long literal `:6160-6163`. The full-push trap is emitted at
> `:11909-11912`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11319-11409`, with `inline_arrc`/
> `needs_body_first` at `:11301-11307`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4225-4227`; its twin
...
> Provenance: `src/tychoc.c:6182-6215` (equality/ordering resolver); function-
> value identity equality `:9628@identity equality`.
