...
> `:1438-1478`; equality/ordering `:6468-6501`; newtype decl `parse_typedecl`
> `:4352-4371`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2474@elem`); the
> fixed forms test both (`src/tychoc.c:2145-2146`), as does `bounded[N]T`
> (`src/tychoc.c:2034-2035`). Detailed in
...
> `src/tychoc.c:2352-2369@"bounded"` (capacity `:1967-1976`, element
> restriction `:2032-2033`); its twin
...
> Rejections: slice `src/tychoc.c:5515-5516`, `pop` `:6240-6241`, `reserve`
> `:6739@reserve does not apply to a bounded`, over-long literal `:6446-6449`. The full-push trap is emitted at
> `:12312-12315`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11714-11812`, with `inline_arrc`/
> `needs_body_first` at `:11696-11702`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4364-4366`; its twin
...
> Provenance: `src/tychoc.c:6468-6501` (equality/ordering resolver); function-
> value identity equality `:10016@identity equality`.
