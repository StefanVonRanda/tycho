...
> Provenance: scalar tags `src/tychoc.c:600-621`; C lowering `c_type`
> `:1427-1467`; equality/ordering `:6453-6486`; newtype decl `parse_typedecl`
> `:4339-4358`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2461@elem`); the
> fixed forms test both (`src/tychoc.c:2134-2135`), as does `bounded[N]T`
> (`src/tychoc.c:2023-2024`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1362-1391`; `key_hashable` `:1314-1328`.
...
> `src/tychoc.c:2339-2356@"bounded"` (capacity `:1956-1965`, element
> restriction `:2021-2022`); its twin
...
> (`arrc_sized_b` `src/tychoc.c:1065-1077@arrc_sized_b`, messages `:974@task_container_err` and `:1014@chan_container_err`) and an
> explicit check at `compiler/tychoc0.ty:1890-1896@ck_affine_part`.
> Rejections: slice `src/tychoc.c:5502-5503`, `pop` `:6225-6226`, `reserve`
> `:6724@reserve does not apply to a bounded`, over-long literal `:6431-6434`. The full-push trap is emitted at
> `:12297-12300`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11699-11797`, with `inline_arrc`/
> `needs_body_first` at `:11681-11687`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4351-4353`; its twin
...
> Provenance: `src/tychoc.c:6453-6486` (equality/ordering resolver); function-
> value identity equality `:10001@identity equality`.
