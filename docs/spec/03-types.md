...
> Provenance: scalar tags `src/tychoc.c:592-613`; C lowering `c_type`
> `:1397-1437`; equality/ordering `:6237-6270`; newtype decl `parse_typedecl`
> `:4252-4271`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2381@elem`); the
> fixed forms test both (`src/tychoc.c:2074-2075`), as does `bounded[N]T`
> (`src/tychoc.c:1989-1990`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1332-1361`; `key_hashable` `:1284-1298`.
...
> `src/tychoc.c:2259-2276@"bounded"` (capacity `:1922-1931`, element
> restriction `:1987-1988`); its twin
...
> (`arrc_sized_b` `src/tychoc.c:1057-1069@arrc_sized_b`, messages `:966@task_container_err` and `:1006@chan_container_err`) and an
> explicit check at `compiler/tychoc0.ty:1890-1896@ck_affine_part`.
> Rejections: slice `src/tychoc.c:5329-5330`, `pop` `:6009-6010`, `reserve`
> `:6508@reserve does not apply to a bounded`, over-long literal `:6215-6218`. The full-push trap is emitted at
> `:12012-12015`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11422-11512`, with `inline_arrc`/
> `needs_body_first` at `:11404-11410`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4264-4266`; its twin
...
> Provenance: `src/tychoc.c:6237-6270` (equality/ordering resolver); function-
> value identity equality `:9724@identity equality`.
