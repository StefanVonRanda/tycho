...
> Provenance: scalar tags `src/tychoc.c:657-678`; C lowering `c_type`
> `:1335-1375`; equality/ordering `:6632-6665`; newtype decl `parse_typedecl`
> `:4212-4231`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2354@elem`); the
> fixed forms test both (`src/tychoc.c:2017-2018`), as does `bounded[N]T`
> (`src/tychoc.c:1908-1909`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1270-1299`; `key_hashable` `:1222-1236`.
...
> Provenance: `src/tychoc.c:6466-6499` (equality/ordering resolver); function-
> value identity equality `:9908@identity equality`.
