...
> Provenance: scalar tags `src/tychoc.c:620-641`; C lowering `c_type`
> `:1444-1484`; equality/ordering `:6640-6673`; newtype decl `parse_typedecl`
> `:4482-4501`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2509@elem`); the
> fixed forms test both (`src/tychoc.c:2155-2156`), as does `bounded[N]T`
> (`src/tychoc.c:2043-2044`). Detailed in
...
self-embedding would be infinitely sized (§17). The same rule applies to a
*cycle* of by-value fields (`A` holding a `B` that holds an `A`), which is
rejected with the same infinite-type diagnostic.

Type declarations are **order-independent within a file**, as functions are: a
field may name a `struct`, `enum`, `type` or `handle` declared further down.
...
> Provenance: `map_of` `src/tychoc.c:1379-1408`; `key_hashable` `:1331-1345`.
...
> `src/tychoc.c:2376-2393@"bounded"` (capacity `:1975-1985`, element
> restriction `:2041-2042`); its twin
...
> (`arrc_sized_b` `src/tychoc.c:1082-1094@arrc_sized_b`, messages `:994@task_container_err` and `:1032@chan_container_err`) and an
> explicit check at `compiler/tychoc0.ty:1890-1896@ck_affine_part`.
> Rejections: slice `src/tychoc.c:5671-5672`, `pop` `:6405-6406`, `reserve`
> `:6911@reserve does not apply to a bounded`, over-long literal `:6618-6621`. The full-push trap is emitted at
> `:12595-12598`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11997-12095`, with `inline_arrc`/
> `needs_body_first` at `:11979-11985`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4494-4496`; its twin
...
> Provenance: `src/tychoc.c:6640-6673` (equality/ordering resolver); function-
> value identity equality `:10287@identity equality`.
