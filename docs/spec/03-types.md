...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2231@elem`); the
...
> `src/tychoc.c:2116-2133@"bounded"` (capacity `:1866-1875`, element
...
> (`arrc_sized_b` `src/tychoc.c:924-936@arrc_sized_b`, messages `:833@task_container_err` and `:873@chan_container_err`) and an
...
> `:6193@reserve does not apply to a bounded`, over-long literal `:6023-6026`. The full-push trap is emitted at
...
> value identity equality `:9298@identity equality`.
