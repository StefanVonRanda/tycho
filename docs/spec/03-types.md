...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2184@elem`); the
...
> `src/tychoc.c:2052-2069@"bounded"` (capacity `:1866-1875`, element
...
> (`arrc_sized_b` `src/tychoc.c:885-897@arrc_sized_b`, messages `:787@task_container_err` and `:827@chan_container_err`) and an
...
> `:6029@reserve does not apply to a bounded`, over-long literal `:6023-6026`. The full-push trap is emitted at
...
> value identity equality `:8993@identity equality`.
