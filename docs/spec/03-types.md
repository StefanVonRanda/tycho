...
> `:1343-1383`; equality/ordering `:6085-6118`; newtype decl `parse_typedecl`
> `:4141-4160`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2319@elem`); the
> fixed forms test both (`src/tychoc.c:2019-2020`), as does `bounded[N]T`
> (`src/tychoc.c:1934-1935`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1278-1307`; `key_hashable` `:1230-1244`.
...
> `src/tychoc.c:2204-2221@"bounded"` (capacity `:1867-1876`, element
> restriction `:1932-1933`); its twin
...
> Rejections: slice `src/tychoc.c:5202-5203`, `pop` `:5857-5858`, `reserve`
> `:6356@reserve does not apply to a bounded`, over-long literal `:6063-6066`. The full-push trap is emitted at
> `:11763-11766`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11173-11263`, with `inline_arrc`/
> `needs_body_first` at `:11155-11161`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4153-4155`; its twin
...
> Provenance: `src/tychoc.c:6085-6118` (equality/ordering resolver); function-
> value identity equality `:9505@identity equality`.
