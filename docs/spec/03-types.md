...
> Provenance: scalar tags `src/tychoc.c:614-635`; C lowering `c_type`
> `:1438-1478`; equality/ordering `:6519-6552`; newtype decl `parse_typedecl`
> `:4403-4422`.
...
> Provenance: dynamic `[T]` tests `void` alone (`src/tychoc.c:2475@elem`); the
> fixed forms test both (`src/tychoc.c:2146-2147`), as does `bounded[N]T`
> (`src/tychoc.c:2035-2036`). Detailed in
...
> Provenance: `map_of` `src/tychoc.c:1373-1402`; `key_hashable` `:1325-1339`.
...
> `src/tychoc.c:2353-2370@"bounded"` (capacity `:1967-1977`, element
> restriction `:2033-2034`); its twin
...
> (`arrc_sized_b` `src/tychoc.c:1076-1088@arrc_sized_b`, messages `:988@task_container_err` and `:1026@chan_container_err`) and an
> explicit check at `compiler/tychoc0.ty:1890-1896@ck_affine_part`.
> Rejections: slice `src/tychoc.c:5566-5567`, `pop` `:6291-6292`, `reserve`
> `:6790@reserve does not apply to a bounded`, over-long literal `:6497-6500`. The full-push trap is emitted at
> `:12418-12421`. Fixtures: `tests/bounded.ty`, `tests/bounded_const_cap.ty`,
...
> the pointer-shaped arrays (`src/tychoc.c:11820-11918`, with `inline_arrc`/
> `needs_body_first` at `:11802-11808`; tychoc0's `comp_dep_types`
...
> Provenance: underlying restriction `src/tychoc.c:4415-4417`; its twin
...
> Provenance: `src/tychoc.c:6519-6552` (equality/ordering resolver); function-
> value identity equality `:10110@identity equality`.
