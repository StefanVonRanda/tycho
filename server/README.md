...
- **There used to be no graceful shutdown.** Nothing installed a `SIGTERM` or
  ...
  `running = false` on an `Err` from `net.accept` (`server/main.ty:532-533`) —
  ...
  **Fixed 2026-07-31** by `core:signal` (`docs/spec/18-library.md` §32.27), a
  ...
  `server/main.ty:898@stopped` prints the count that was always unreachable. It fails
...
- **`MSG_NOSIGNAL` / `SO_NOSIGPIPE`** (`corelib/net/net_shim.c:41-53` and
  ...
  hostile disconnects and logs them as `write-failed` (`server/main.ty:431@write-failed`);
...
(`server/main.ty:682-686`) hands a runner readiness **and** the bound port on
