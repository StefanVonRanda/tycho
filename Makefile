...
# tls-verify: does core:tls actually VERIFY the certificate? The only tls test
# there was connects to a CLOSED PORT, so it cannot tell a refused CERTIFICATE
# from a refused CONNECTION -- SSL_VERIFY_NONE passed every gate in this tree.
# Runs a real TLS server on a kernel-chosen loopback port with an untrusted cert.
tls-verify:
	@sh scripts/tls_verify.sh

# http-verify: the same question for core:http, which is the HTTPS client every
# Tycho program actually reaches for. It could not be asked until 2026-08-15:
# nothing could point core:http at a private CA, so there was no leg that had to
# SUCCEED and "refused" was indistinguishable from "never connected" (FRICTION
# #57). The shim honours SSL_CERT_FILE/SSL_CERT_DIR now, so it can be.
http-verify:
...
# math-diff: core:math against Python's arithmetic. Both defects this 75-line
# package has had (gcd's sign, sign's infinities) were found by READING a claim,
# and `make corelib` compares it to a golden this repo recorded -- which agrees
# with whatever the code did the day it was written. Each arm is proved able to
# redden: reverting gcd reddens the int arm alone, sign the float arm alone.
math-diff:
	@sh scripts/math_diff.sh

# traversal-check: server/run.sh proves a traversal is REFUSED; it cannot say by
# WHICH guard. resolve() has two independent ones and either alone suffices, so
# deleting one changes no observable behaviour and every gate stays green while
# the defence halves. This defeats them one at a time in a copy and requires the
# refusal to hold, with both-defeated as the control that must LEAK.
traversal-check:
...
# intermediate (src/tychoc.c:13721). They stay because `--emit-c -o <base>` still
# writes and KEEPS `<base>.c` (src/tychoc.c:13690-13692), which is how you debug the
