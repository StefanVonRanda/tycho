...
| `corelib/csv/`, `corelib/json/`, `corelib/datetime/`, `corelib/sort/`, and the codecs (`sha256`, `md5`, `base64`, `hex`, `url`) | also `make format-diff` | ~9s. Round-trips both against Python's own modules. `make corelib` compares them to goldens this repo wrote, which cannot say whether the format was ever readable by anyone else — that is how a row of one empty field silently lost its field |
