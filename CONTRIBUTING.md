...
| `tools/tycho-rsa/` | `make rsa-check` | ~16.7s, and **the only lane that runs the RSA tool** -- the heaviest consumer of `core:bignum`, `core:crypto` and `core:rand`. The load-bearing legs reach outside the repo: modexp against python's `pow`, and an OAEP known-answer test BOTH ways against python's `cryptography`, so a re-recorded golden cannot bless a wrong ciphertext. It also proves keygen draws fresh (two 528-bit keys differ), that OAEP padding randomises -- with a fixed-seed copy as the control, since a leg that cannot fail is decoration -- and that tampered, short, over-long and non-hex inputs are all refused. SKIPS loudly with exit 0 when OpenSSL's dev package is absent, so read the last line |
