...
Thanks for trying Tycho and wanting to help. **Tycho is 0.8.5 — pre-1.0** (see the
...
| adding or deleting any `.ty`, anywhere in the tree | `make corpus-check` | ~0.09s (0.090 / 0.090 / 0.088 s, measured 2026-09-04) — the cheapest lane here, and the sub-second predictor of `parse-check`'s opening counts. Those counts are literals on purpose (a leg that scores 0 of 0 is green by accident), but nothing said one had drifted until the 13-second lane ran, and a commit that deletes a file has no reason to run it: one deletion left five numbers describing a tree that no longer existed and `parse-check` sat red on `main` for three commits. This holds no copy of any number — it extracts each literal from the file that owns it and recomputes the corpus the way that file does — so a green here means those legs will pass. `--selfcheck` perturbs each literal in turn and requires the leg to redden. |
...
| `runtime/tycho_rt.c`, or the embed rule in the `Makefile` | `make embed-check` | ~0.5s. The runtime is escaped into one C string literal, and that escape must be **awk-independent**: Debian's and Ubuntu's mawk 1.3.4 20200120 halves a backslash in a `gsub` replacement, so the rule under-escaped every backslash in the runtime and `make` died at `cc` on a fresh clone there. No other lane can see it — they all run against a header this host's awk had already written correctly. Decodes the header back and requires it to equal the runtime byte for byte, under every awk on `PATH` |
...
| a `vector[N]T`, or the element-wise arithmetic lowering in either compiler | `make vector-check` | ~13.5s (13.69 / 13.46 / 13.46 s, measured 2026-09-05; it was ~8.5s while the vector legs scored one compiler). **The only lane whose subject is a machine INSTRUCTION**, and since 2026-09-05 **every leg runs BOTH compilers**. `tests/vector_type.out` cannot see the feature at all — every line prints the same answer through a plain `[N]T`, because a vector IS a fixed array in every rule the language already had; what moved is the lowering. So the lane reads the emitted C for a GCC vector member, measures the aggregate’\s alignment against the 8 bytes the arena actually guarantees (with the pinning attribute STRIPPED as the control, which raises it to 16), and counts packed instructions in the assembly **at -O0 and -O1, where gcc’\s own auto-vectoriser is off** — at -O2 gcc vectorises the `[4]float` loop by itself and the check would pass for both spellings, proving nothing. Also asserts int `/` and `%` still route through the runtime divide guard, and all six refusals by their whole sentence. Scoring only `./tychoc` is how the SHIPPED compiler came to lower `vector[N]T` to the arena-backed dynamic array with nothing in the tree able to notice: zero `vector_size`, zero packed instructions, one arena allocation per operation. The two compilers name the aggregate differently and number their temps differently, so the patterns span both rather than pinning one spelling. |
| an `align(N) struct`, or the arena's alignment guarantee | `make align-probe` | ~2.5s (2.76 / 2.49 / 2.49 s, measured 2026-09-05). **The only lane that reads a REAL ADDRESS.** `_Alignof` is a compile-time claim about a TYPE and says nothing about where the allocator put the value, and the one unacceptable outcome for this attribute is a declared alignment the arena does not honour -- the program says 8 and gets 2 with nothing to notice. So the lane includes the emitted translation unit (runtime and all) with its `main` renamed, allocates out of a real `Arena` and checks the addresses modulo N, from BOTH compilers. Leg [2] is the load-bearing one: single values are 8-aligned whatever their type, because `arena_alloc` rounds every REQUEST to 8 -- which is exactly why N<=8 costs the allocator nothing; what the attribute buys is a SIZE that is a multiple of N, so element k of an array stays aligned. Its control STRIPS the attribute from the emitted C and 48 of 64 elements go off the boundary. Leg [3] is where the >8 REFUSAL is grounded rather than asserted: the arena delivers 16-byte alignment on 0 of 4096 allocations. Leg [6] answers the question that follows from [3] -- must every arena allocation site be audited before an alignment above 8 could ever be honoured? No: an alignment cannot exceed 8 because it cannot ENTER the type system above 8. Exactly two constructs emit an alignment attribute in either compiler -- `align(N) struct`, refused above 8 at the parser, and `vector[N]T`, pinned at `packed, aligned(8)` -- so the ceiling is checked where it is established, with a `_Static_assert(_Alignof(T) <= 8)` over every user type the emitted TU defines (21 from one compiler, 20 from the other) for a program nesting a vector in a struct, an enum payload, an array, a map value and a closure capture. Its control removes the cap the vector carries and 8 of those 21 types over-align, which is what shows the propagation is transitive rather than per-declaration. |
| the CONTENTS of a release archive, not just its bytes | `make release-content` | ~89s (measured 2026-09-05; two `scripts/release.sh` runs and its own selfcheck's three rebuilds are all of it). **`make release-check` compares two builds byte for byte and asserts nothing about what is IN them**, which is how an archive shipping the BOOTSTRAP compiler, four `.exe` files that could not start (`-pthread` with no `-static`, importing a `libwinpthread-1.dll` the archive did not carry, wine exit 53) and no `runtime/` directory was invisible to every lane here -- all three found by hand, none by a gate. Both archives are extracted and read: layout, `runtime/tycho_rt.c` cmp-identical to the repo's, the version the packaged compiler reports, every imported DLL either a Windows system one or a file the archive carries, every `.exe` actually starting under wine, the emitted C byte-identical to `./tychoc1`'s **for a program the two compilers disagree on** -- the disagreement asserted first, or the identity leg is decoration -- and a `core:strings` program emitted, linked and RUN with corelib found beside the binary and no `TYCHO_CORELIB`. 33 legs. `--selfcheck` rebuilds all three defects for real, asserts each substitution landed, and requires each to redden the leg that names it, with the unmutated archive staying clean as the revert. SKIPS loudly, exit 0, without wine or mingw. |
...
| the emitted C, a corelib shim, or `scripts/release.sh --mingw` | `make mingw-warn` | ~33s (33.1 / 33.6, measured 2026-09-05). The only lane that reads the **cross compiler's** warnings. `scripts/release.sh --mingw` cross-builds four .exe files that ship, and nothing ever read that build's warnings; `make shim-warn` cannot see them, its subject being hand-written shim C under the host cc. Cross-compiles all four programs at the release cc line (`-O2 -fwrapv -static -pthread`) and compares the warning lines to `scripts/mingw.warn`, which is **empty today** — so any warning at all reddens it. Deliberately NOT `-Wall`: the subject is EMITTED C, where `-Wall` is noise by construction, and an exemption list is how a warning lane becomes decoration. The default set is where `-Wstringop-overread` and the rest of the access diagnostics live. SKIPS loudly, exit 0, with no mingw-w64 on PATH. `RECORD=1` re-records. |
...
| `runtime/tycho_rt.c`, the driver's `cc` line, or any C a program links | `make glibc-check` | ~2.1s (2.26 / 1.95 / 1.97 s, measured 2026-09-06). The libc FLOOR of a program this tree builds. No other lane can see it: the binary compiles, runs here and matches its golden while requiring a symbol version the target distro does not have. On a C23-default host glibc redirects `strtol` to `__isoc23_strtol@GLIBC_2.38`, which is above Rocky 9 (2.34), Ubuntu 22.04 (2.35) and Debian 12 (2.36). Four legs — the program runs and prints the right answer, no `__isoc23_*` symbol, nothing above `GLIBC_2.34`, and the `TYCHO_BLOCK` knob still moves the block size. `--selfcheck` is a mirror pair: a `-std=gnu23` `strtol` program must be flagged, a clean one must not |
...
| `corelib/crypto/`, `corelib/tls/`, `corelib/http/`, `corelib/image/` — any handle a caller holds | also `make handle-guard` | ~21.9s, and **the only lane that can see a double free or a use-after-free of one of these handles**. They are a bare `ptr`, which is not affine, so the compiler cannot refuse either: before the guard a second free was a SIGSEGV and a use after one returned freed memory with exit 0. `make corelib` runs only CORRECT programs and a golden recorded from a build with the guard removed is byte-identical, so nothing else here can redden. Three legs per package from one probe — the legitimate open/use/free must still work, a second free must die naming the handle, a use after it must too — plus two builds against copies of corelib, one with the guard's condition removed (the diagnosis must vanish while the probe still reaches the free) and one pristine (it must come back) |
...
| `tools/tycho-chess/` | `make chess-check` | ~26.8s, the slowest lane in this block and **the only lane that runs the chess engine**. Its subject is a move generator, which a self-recorded golden cannot judge -- so 24 of the perft totals are PUBLISHED answers from outside this repo, and a dropped en-passant, promotion or castling case moves a number nobody here chose. It also pins the search as deterministic and unchanged with the transposition table off, and `parsearch` as agreeing with the serial search, so a `parallel for` regression reddens here too |
| `tools/tycho-kvsrv/` | `make kvsrv-check` | ~2.3s, and **the only lane that runs the KV server** -- the second consumer of `core:httpd` and `core:net` after `server-check`. It binds a kernel-chosen port, waits on the stderr banner rather than a sleep, and drives one raw socket: round trips, the 404 and 405 paths, keep-alive over a single connection, and four concurrent PUT/GETs that must all come back intact, which is the leg a request-handling race shows up in |
| `tools/tycho-sat/` | `make sat-check` | ~3.9s, and **the only lane that runs the SAT solver**. Both verdicts are checked independently of the program under test: PHP(2..9) are pigeonhole instances that are UNSAT by construction, and every planted SAT answer is a model the RUNNER evaluates itself -- so "solved" cannot degrade into "printed something". Determinism and the clause-learning comparison are pinned alongside |
| `tools/tycho-build/` | `make build-check` | ~6.7s, and **the only lane that runs the build tool**. Seven legs: the dispatch order, a second build being a no-op, a touched source rebuilding only its dependents, a failing recipe skipping them, two clean fixtures building byte-identically, exit 2 on usage errors, and end-to-end vendoring through `tycho-fetch`. That last leg makes this a `core:http` consumer, and it used to DEPEND on the shim accepting `file://`; since commit dbeff7de it vendors with `--local` and asserts the refusal instead -- a `file://` URL must be rejected, must name `--local`, and must leave no `vendor/` directory behind |
| `tools/tycho-debug/` | `make debug-check` | ~11.0s, and **the only lane that runs the debugger** -- the only thing in the tree that drives a real `gdb` MI session. Seven legs: a breakpoint set and hit on the right source line with locals readable, `-b` pre-set breakpoints doing the same, a run to completion reporting the exit and the program's output, Ctrl-C reaching the debuggee as a SIGINT, fail-closed refusals (missing file, a build with no line info), the `tycho debug` dispatcher, and which `tychoc` the session picks up. SKIPS loudly with exit 0 when `gdb` is not on PATH |
| `tools/tycho-fold/` | `make fold-check` | ~2.0s, and **the only lane that runs the text folder**. Its subject is UTF-8 WIDTH, which a transcript cannot see -- a fold that counts bytes renders plausibly and diffs clean against a golden recorded from it. So 200 generated lines mixing ASCII, Latin-1, CJK and emoji are folded at widths 3..30 and asserted as properties: nothing lost or reordered, no line over the width in CODEPOINTS, every output line still valid UTF-8. `--bytes` must AGREE on pure ASCII and DISAGREE on non-ASCII, which is what stops those three passing on a byte-counting fold; invalid UTF-8 in is reported rather than passed through |
