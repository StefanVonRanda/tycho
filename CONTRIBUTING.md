...
| `tools/tycho-db/` — `sql/`, `store/` or `exec/` | `make db-check` | ~5.4s, and **the only lane that runs the database**. Two fresh runs of `demo.sql` must give a cmp-identical transcript *and* a cmp-identical store file; four processes prove a row survives a process exit and that a reopened store still takes writes; all eleven `store.StoreErr` / `exec.ExecErr` variants must exit non-zero with their own whole message. `RECORD=1 sh tools/tycho-db/run.sh` re-records the golden — it cannot bless a lost row, because the persistence rows are literals in the runner |
