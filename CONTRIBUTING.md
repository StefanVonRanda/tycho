...
| a `vector[N]T`, or the element-wise arithmetic lowering in either compiler | `make vector-check` | ~8.5s (8.70 / 8.42 / 8.41 s, measured 2026-09-04). **The only lane whose subject is a machine INSTRUCTION.** `tests/vector_type.out` cannot see the feature at all — every line prints the same answer through a plain `[N]T`, because a vector IS a fixed array in every rule the language already had; what moved is the lowering. So the lane reads the emitted C for a GCC vector member, measures the aggregate’\s alignment against the 8 bytes the arena actually guarantees (with the pinning attribute STRIPPED as the control, which raises it to 16), and counts packed instructions in the assembly **at -O0 and -O1, where gcc’\s own auto-vectoriser is off** — at -O2 gcc vectorises the `[4]float` loop by itself and the check would pass for both spellings, proving nothing. Also asserts int `/` and `%` still route through the runtime divide guard, and all six refusals by their whole sentence in both compilers. |
