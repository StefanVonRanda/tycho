...
for d in tests/pkg/*/; do
    ...
    if ./tychoc "$entry" --bundle 2>/dev/null | "$T/B" > "$T/pg.c" 2>/dev/null && $CC -O2 -o "$T/pg" "$T/pg.c" -lm 2>/dev/null; then
...
if ./tychoc "$T/split/main.ty" -o "$T/Apkg" >/dev/null 2>&1; then
    ...
    if "$T/Apkg" < "$T/sb.ty" > "$T/eA.c" 2>/dev/null && $CC -O2 -o "$T/Epkg" "$T/eA.c" -lm 2>/dev/null \
