#!/usr/bin/env python3
"""Per-file verdict differential: ./tychoc vs ./tychoc1 --parse, over the WHOLE tree.
...
def tychoc_verdict(path):
    ...
    return ("SYNTAX" if (best[0] < C.PARSE_END and not needs) else "SEMANTIC"), msg
...
def main():
    ...
    n = {"ACCEPT": 0, "SEMANTIC": 0, "SYNTAX": 0, "UNMATCHED": 0}
    bad = 0
    for f in files:
    ...
    print("leg5  whole-tree verdicts: files=%d tychoc(accept=%d semantic=%d syntax=%d) disagreements=%d"
          % (len(files), n["ACCEPT"], n["SEMANTIC"], n["SYNTAX"], bad))
    if len(files) != EXPECT:
