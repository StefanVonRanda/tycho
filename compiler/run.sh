...
# leg1 289 -> 290: tests/inline_arr_reassign_loop.ty; leg1c 179 -> 180: tools/tycho-grade/main.ty (2026-09-24)
leg_accept "leg1  tests/*.ty" "$(ls tests/*.ty)" 290
...
# SEMANTIC 271 -> 273: L4 -- lane_past_end and lane_too_many, the two lane-name
# refusals. Both need the element type, so a parser alone must ACCEPT them.
...
# SYNTAX 104 -> 108: the four `align(N) struct` fixtures (L1, 2026-09-05), all
...
[ "$nsyn" = 120 ] && [ "$nname" = 35 ] && [ "$ntype" = 175 ] && [ "$nsem" = 296 ] || { echo "parse-check: the split moved -- expected SYNTAX=120 NAME=35 TYPE=175 SEMANTIC=296"; rc=1; }   # SYNTAX 119 -> 120: tests/reject/value_if_nested.ty (2026-09-24); SEMANTIC 292 -> 295: the three handle use-after-close fixtures (2026-09-24); -> 296: handle_close_param (2026-09-24)
...
[ "$tr" = 626 ] || { echo "parse-check: --typecheck refused $tr of 626, expected 626"; rc=1; }   # +2 each: the unbound-opener and handle-destructor fixtures (2026-09-21); +1: value_if_nested (2026-09-24); +3: handle use-after-close (2026-09-24); +1: handle_close_param (2026-09-24)
...
# THROUGH a subscript, which is the `g.at(0) = 5` place form (src/tychoc.c:4462).
pr ok_subscript <<'EOF'
package main
struct G:
    xs: [int]
...
# is decoration until Phase 9 pins message text. src/tychoc.c:6728 answers
# `pkg.Name` written with NO call; src/tychoc.c:7087 answers `pkg.name(...)`.
