...
| **The whole gate** | **78 lanes** (`grep -c '^step "' scripts/ci.sh`), green under **gcc and clang** on x86-64 Linux and under **clang on darwin-arm64**. Wall clock is deliberately not quoted as one number — repeated runs here span 263-390s, which is FRICTION 91 | `make ci` |
