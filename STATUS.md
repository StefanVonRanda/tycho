...
| **Real programs** | **32 programs in Tycho, 18,779 lines** — every `.ty` under `tools/`, which is 29 `tools/tycho-*/` directories plus `tycho.ty`, `tychofmt.ty` and `lsp.ty`. **6 over 1000 lines** (`db` 2063, `scheme` 1609, `sheet` 1483, `lsp` 1403, `q` 1359, `chess` 1166), 10 between 300 and 999, 16 under 300. One of the 32 was not written by the author: `tycho-du` came in from the 2026-09-21 `handle` probe ([record](docs/internals/probe-handle-2026-09-21.md)), which is ROADMAP §1's instrument rather than its answer. Not 32 substantial programs; `tycho-db`'s gate does assert crash recovery from a real `kill -9`, torn-record discard and a TCP server | `find tools -name '*.ty' \| xargs wc -l` |
| **Normative spec** | 28 files, and the implementation is gated against them | `make spec-check` |
| **The whole gate** | **79 lanes** (`grep -c '^step "' scripts/ci.sh`), green under **gcc and clang** on x86-64 Linux and under **clang on darwin-arm64**. Wall clock is deliberately not quoted as one number — repeated runs here span 263-390s, which is FRICTION 91 | `make ci` |
...
| **Docs** | 90 reachable pages, no dead links, every `path:line` citation resolves to the line it names, and **every public corelib function is named in the catalogue** (423/423, matched as a word — it was 431/431 by substring until FRICTION 118) | `make check-links` |
