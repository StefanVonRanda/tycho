...
> One place to see where the project stands: architecture, the verification
> surface (what each gate proves), what's shipped, what's a decided non-goal, and what's
> actually open. If you're trying to get your bearings, start here, not in `docs/internals/`
> (those are reference + historical design records; see the index at the bottom).
>
> Everything here is checked against the transpilers/gates, not asserted from memory.
...
`C = B·tychoc0.ty`; assert `B == C` byte-identical (tychoc0 reproduces its own emitted C) and
that `B`'s program output matches the reference. `make fixpoint`. tychoc0 is a *subset*
transpiler — the breadth gap vs `tychoc` is tracked by the parity gates below, and as of this
sync it's broad and clean.
The old newtype-erasure limitation is now **closed**: tychoc0 tracks newtype identity
