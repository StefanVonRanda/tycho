...
- Verified 2026-08-13 by reading the index: 8 entries carry no resolution marker;
  2 more are stale DUPLICATES of entries already closed above them.
...
- [ ] **Phase 1 — delete the two stale duplicates**
  - `docs/internals/FRICTION.md:1362` repeats `### 3. compress.decompress` which
    is CLOSED at `:1331`; `:1420` repeats `### 4. strings.parse_int` which is
    marked ALREADY FIXED at `:1396`. Confirm each pair is the same entry before
    deleting the later copy.
  - Verify: `python3 scripts/check_citations.py`, `sh scripts/check_links.sh`.
    No other gate can redden — this is Markdown only.

- [ ] **Phase 2 — `bytes` slices clamp, so a slice is not a bounds check** (`:1435`)
...
- [ ] **Phase 3 — `core:json` accepts input it cannot represent** (`:1738`)
...
- [ ] **Phase 4 — `core:decimal` has no `div`** (`:1849`)
...
- [ ] **Phase 5 — `iter.try_map` has no `Result(void, E)` shape** (`:2306`)
...
- [ ] **Phase 6 — a `string` across the FFI truncates at its first NUL** (`:2376`)
...
- [ ] **Phase 7 — a two-key comparator cannot be written inline** (`:2486`)
...
- [ ] **Phase 8 — a first `--shim` C file must hand-declare `tycho_int`** (`:2540`)
...
- [ ] **Phase 9 — a `for` binding does not destructure a tuple** (`:2603`)
