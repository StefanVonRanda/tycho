...
`runtime/tycho_rt.c:1083@NUL` records that a Tycho string is NUL-terminated and its
...
1. **`(ptr, len)`, not a NULL-terminated argv.** It would follow the
   `[int]`/`[float]` convention already at `src/tychoc.c:10080@arrp`. A callee wanting
...
3. **The return direction stays refused.** `[string]` *out* of C has no length
   header to reconstruct — the same reason `src/tychoc.c:4457-4458` bans a
