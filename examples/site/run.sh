...
# core:datetime's tz-offset shim (dtx_*, pure libc, no external deps); the tychoc0
# paths below emit C that WE link, so we gather that shim (mirroring corelib/run.sh)
# -- tychoc auto-discovers it. Built by all three compilers (the C tychoc, tychoc0 via
# --bundle, and standalone tychoc0), each run against the local fixture site so the
...
# (2) self-hosted tychoc0 (bundle)
if ! { "$TYCHOC" "$SRC" --bundle 2>/dev/null | "$T/h0" > "$T/h0.c" 2>/dev/null && \
       $CC -O2 "$T/h0.c" $shim -o "$T/h0bin" -lm 2>"$T/h0.log"; }; then
    echo "FAIL: tychoc0 compile"; sed 's/^/      /' "$T/h0.log"; fail=1
else
    "$T/h0bin" "$SITE" "$T/out" > "$T/h0.out" 2>&1
fi

# (3) standalone tychoc0 (resolves core: itself)
if ! { "$T/h0" "$SRC" > "$T/sd.c" 2>/dev/null && $CC -O2 "$T/sd.c" $shim -o "$T/sdbin" -lm 2>"$T/sd.log"; }; then
    echo "FAIL: standalone tychoc0 compile"; sed 's/^/      /' "$T/sd.log"; fail=1
else
    "$T/sdbin" "$SITE" "$T/out" > "$T/sd.out" 2>&1
fi

# (4) ASan/UBSan over the emitted C
if ! $CC -fsanitize=address,undefined -fno-sanitize-recover=all -g -O1 "$T/h0.c" $shim -o "$T/san" -lm 2>"$T/san.log"; then
...
if [ "$fail" -eq 0 ] && ! cmp -s "$T/c.out" "$T/h0.out"; then
    echo "FAIL: tychoc vs tychoc0 differ"; diff "$T/c.out" "$T/h0.out" | sed 's/^/      /'; fail=1
fi
if [ "$fail" -eq 0 ] && ! cmp -s "$T/c.out" "$T/sd.out"; then
    echo "FAIL: standalone tychoc0 differs"; diff "$T/c.out" "$T/sd.out" | sed 's/^/      /'; fail=1
fi
...
[ "$fail" -eq 0 ] && echo "site: green (io+path+json+csv+strings+sort+datetime+sha256 compose; tychoc+tychoc0+standalone+ASan)" || { echo "site: FAIL"; exit 1; }
