...
# built by the C compiler AND the self-hosted hierc0, and run against a local
...
if ! "$HIERC" examples/fetch/main.hi -o "$T/c" >"$T/c.log" 2>&1; then
    echo "FAIL: hierc compile"; sed 's/^/      /' "$T/c.log"; fail=1
else
    "$T/c" "$URL" > "$T/c.out" 2>&1
fi

# (2) self-hosted hierc0: emit C, link the shim + libcurl ourselves
if ! { "$HIERC" examples/fetch/main.hi --bundle 2>/dev/null | "$T/h0" > "$T/h0.c" 2>/dev/null && \
...
[ "$fail" -eq 0 ] && echo "fetch: green (http+json+sha256+io+path compose; hierc+hierc0+ASan; real libcurl via file://)" || { echo "fetch: FAIL"; exit 1; }
