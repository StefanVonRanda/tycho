...
# tychoc auto-links a package's <pkg>_shim.c; the tychoc0 transpile path needs it
# named explicitly. weblog's only shim'd import is core:io (the streaming reader).
$CC -O2 "$T/wl0.c" corelib/io/io_shim.c -o "$T/wl_0" -lm 2>/dev/null || { echo "weblog: tychoc0-emitted C did not compile"; exit 1; }
"$T/wl_0" > "$T/out_0"
