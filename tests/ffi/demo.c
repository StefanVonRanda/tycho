...
int64_t ffi_sum3(int64_t a, int64_t b, int64_t c) {       /* ints -> int */
    return a + b + c;
}

double ffi_scale(double x, int64_t k) {          /* float + int -> float */
    return x * (double)k;
}
...
static int slot;
void *ffi_open(int64_t id) {                     /* int -> ptr; NULL handle for id <= 0 */
    if (id <= 0) return 0;
    slot = (int)id;
    return &slot;
}
int64_t ffi_read(void *h) {                       /* ptr -> int; -1 for a NULL handle */
    return h ? *(int *)h : -1;
}
...
int64_t ffi_bsum(const unsigned char *p, int64_t n) {
    int64_t s = 0;
    for (int64_t i = 0; i < n; i++) s += p[i];
    return s;
}
void ffi_brev(const unsigned char *p, int64_t n, unsigned char **out, int64_t *outlen) {
    unsigned char *r = (unsigned char *)malloc(n > 0 ? (size_t)n : 1);
    for (int64_t i = 0; i < n; i++) r[i] = p[n - 1 - i];
    *out = r;
    *outlen = n;
}
...
static int64_t g_res_live = 0;
void *ffi_res_open(int64_t id) { (void)id; g_res_live++; return (void *)1; }
int64_t ffi_res_close(void *h) { (void)h; g_res_live--; return 0; }
int64_t ffi_res_use(void *h) { return h ? 7 : -1; }
int64_t ffi_res_live(void) { return g_res_live; }
...
const char *ffi_get(int64_t key) { return key > 0 ? "hit" : 0; }
...
int64_t ffi_mk(int64_t id, void **out) { if (id <= 0) { *out = 0; return -1; } slot = (int)id; *out = &slot; return 0; }
void ffi_dbl(int64_t a, int64_t *out) { *out = a * 2; }
...
int64_t ffi_ptrval(void *p)  { return (int64_t)p; }

/* arrays cross as (const T*, int64_t): ffi_isum sums an [int], ffi_favg averages a
 * [float]. Proves [int]/[float] params reach C as a flat (pointer, length) pair. */
int64_t   ffi_isum(const int64_t *xs, int64_t n)   { int64_t s = 0; for (int64_t i = 0; i < n; i++) s += xs[i]; return s; }
double ffi_favg(const double *xs, int64_t n) { if (n == 0) return 0.0; double s = 0; for (int64_t i = 0; i < n; i++) s += xs[i]; return s / (double)n; }
...
void ffi_iota(int64_t n, int64_t **out, int64_t *outlen) {
    if (n < 0) n = 0;
    int64_t *p = (int64_t *)malloc(n ? (size_t)n * sizeof(int64_t) : 1);
    for (int64_t i = 0; i < n; i++) p[i] = i;
    *out = p; *outlen = n;
}
void ffi_dscale(const double *xs, int64_t n, double k, double **out, int64_t *outlen) {
    if (n < 0) n = 0;
