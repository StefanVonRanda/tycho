...
corpus_census() {
    ...
    # still prints `ok`. So the count is pinned to a literal here. 182 = the 82 at
    ...
    # 25 R21f added for the builtin-call block at src/tychoc.c:6774-6997, plus all
    # 21 R21f-2 added for src/tychoc.c:7004-7598, plus all 11 R21f-3 added for
    # the parallel-for block and the 9k/12k tail.
    ...
    [ "$cc_exp" -eq 182 ] || cc_bad="$cc_bad reject-expect:$cc_exp!=182"
