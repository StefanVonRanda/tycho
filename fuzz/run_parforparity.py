...
# is kept only as a fail-closed assertion (src/tychoc.c:7887@r_step). The constraint that
...
# (src/tychoc.c:7776-7779), so this fixture's original body `a, b = b, a` was never
...
fn main():
    ...
    print(str(acc) + "\\n")
''',
# `0..<N` starts at a literal 0 (src/tychoc.c:4085@i_dotlt), so the old `range(1, 8)` is
