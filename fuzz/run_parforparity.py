...
# is kept only as a fail-closed assertion (src/tychoc.c:7657@r_step). The constraint that
# took its place is the LOWER bound: `0..<N` demands a literal `0`
# (src/tychoc.c:3959@i_dotlt), so this is the range-shape rejection that still has a source
...
# (src/tychoc.c:7556-7559), so this fixture's original body `a, b = b, a` was never
...
fn main():
    ...
    print(str(acc) + "\\n")
''',
# `0..<N` starts at a literal 0 (src/tychoc.c:3959@i_dotlt), so the old `range(1, 8)` is
