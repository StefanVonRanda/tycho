...
TIMEOUT = 20        # compile steps (hierc / hierc0 / cc)
...
def run_seed(seed, h0, tmp):
    ...
    if hc_ok == "timeout" or h0_ok == "timeout":
        ...
        return "FAIL", "compile-timeout divergence: hierc=%s hierc0=%s" % (hc_ok, h0_ok)
    ...
    if is_to(e1) or is_to(e2):
        if is_to(e1) and is_to(e2):
            return "timeout", "both sides timed out (%ds): hierc=%s hierc0=%s" % (RUN_TIMEOUT, e1, e2)
        return "FAIL", "one-sided timeout (hang): hierc=%s hierc0=%s" % (e1, e2)
    if e1: return "FAIL", "hierc " + e1            # reference itself faulted -> real bug
    if e2: return "FAIL", "hierc0 " + e2
    # ASan/UBSan runs of BOTH sides: catches a UAF/UB in hierc-only codegen
    ...
    if is_to(e1a): return "timeout", "hierc-ASan timed out: " + e1a
    if e1a: return "FAIL", "hierc-ASan " + e1a
    out_as, e3 = build_run(os.path.join(tmp, "h0.c"), os.path.join(tmp, "run_h0a"), tmp, asan=True)
    if is_to(e3): return "timeout", "hierc0-ASan timed out: " + e3
    if e3: return "FAIL", "hierc0-ASan " + e3
    if out_hc != out_h0:
        return "FAIL", "output diverge: hierc=%r hierc0=%r" % (out_hc, out_h0)
    if out_hc != out_hca:
        return "FAIL", "hierc native vs ASan diverge (UB): %r vs %r" % (out_hc, out_hca)
    if out_h0 != out_as:
        return "FAIL", "hierc0 native vs ASan diverge (UB): %r vs %r" % (out_h0, out_as)
    return "ok", None
