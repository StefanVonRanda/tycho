...
def adapt(lt, ll, rt, rl):
    ...
    (`src/tychoc.c:6171-6180`): an int literal takes the other side's u32/u64;
...
def expect(lt, lform, op, rt, rform):
    ...
    if op in ("and", "or"):                       # `src/tychoc.c:6155` -- bool operands, no adaptation
        return "accept" if lt == "bool" and rt == "bool" else "reject"
    lt, rt = adapt(lt, ll, rt, rl)
    if op in ("==", "!="):                        # `src/tychoc.c:6186` -- structural, but the types must be EQUAL.
        ...
        # type error while `2.5 < 7` is not (`src/tychoc.c:6197-6204` explains why).
        return "accept" if lt == rt else "reject"
    if op in ("<", ">", "<=", ">="):              # `src/tychoc.c:6201-6208`
    ...
    if op == "+" and lt == "string":              # `src/tychoc.c:6215` -- string + string|char, ONE-directional
        return "accept" if rt in ("string", "char") else "reject"
    if op in ("<<", ">>"):                        # `src/tychoc.c:6237-6243`
        ...
        # require matching operands and is enforced at `src/tychoc.c:6350`.
    ...
    if op in ("%", "&", "|", "^"):                # `src/tychoc.c:6347` -- two MATCHING integers
        return "accept" if lt in INTEGER and lt == rt else "reject"
    # arithmetic `+ - * /`
    if lt == rt and lt in ({"int", "float", "f32"} | SIZED):   # `src/tychoc.c:6357-6369`
        return "accept"
    if op in ("+", "-") and (lt == "char" or rt == "char") \
       and lt in ("char", "int") and rt in ("char", "int"):    # `src/tychoc.c:6365-6368` -- char±int, int±char, char±char
        return "accept"
    if lt == "float" and rt == "int" and rl: return "accept"   # `src/tychoc.c:6374`
    if rt == "float" and lt == "int" and ll: return "accept"   # `src/tychoc.c:6378`
    return "reject"
