...
def adapt(lt, ll, rt, rl):
    ...
    (`src/tychoc.c:6765-6774`): an int literal takes the other side's u32/u64;
...
def expect(lt, lform, op, rt, rform):
    ...
    if op in ("and", "or"):                       # `src/tychoc.c:6749` -- bool operands, no adaptation
        return "accept" if lt == "bool" and rt == "bool" else "reject"
    lt, rt = adapt(lt, ll, rt, rl)
    if op in ("==", "!="):                        # `src/tychoc.c:6780` -- structural, but the types must be EQUAL.
        ...
        # type error while `2.5 < 7` is not (`src/tychoc.c:6791-6798` explains why).
        return "accept" if lt == rt else "reject"
    if op in ("<", ">", "<=", ">="):              # `src/tychoc.c:6795-6802`
    ...
    if op == "+" and lt == "string":              # `src/tychoc.c:6809` -- string + string|char, ONE-directional
        return "accept" if rt in ("string", "char") else "reject"
    if op in ("<<", ">>"):                        # `src/tychoc.c:6831-6837`
        return "accept" if lt in INTEGER and rt in INTEGER else "reject"
    if op in ("%", "&", "|", "^"):                # `src/tychoc.c:6941` -- two MATCHING integers
        return "accept" if lt in INTEGER and lt == rt else "reject"
    # arithmetic `+ - * /`
    if lt == rt and lt in ({"int", "float", "f32"} | SIZED):   # `src/tychoc.c:6951-6963`
        return "accept"
    if op in ("+", "-") and (lt == "char" or rt == "char") \
       and lt in ("char", "int") and rt in ("char", "int"):    # `src/tychoc.c:6959-6962` -- char±int, int±char, char±char
        return "accept"
    if lt == "float" and rt == "int" and rl: return "accept"   # `src/tychoc.c:6968`
    if rt == "float" and lt == "int" and ll: return "accept"   # `src/tychoc.c:6972`
    return "reject"
