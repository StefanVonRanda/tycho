...
def adapt(lt, ll, rt, rl):
    ...
    (`src/tychoc.c:7130-7139`): an int literal takes the other side's u32/u64;
...
def expect(lt, lform, op, rt, rform):
    ...
    if op in ("and", "or"):                       # `src/tychoc.c:7114` -- bool operands, no adaptation
        return "accept" if lt == "bool" and rt == "bool" else "reject"
    lt, rt = adapt(lt, ll, rt, rl)
    if op in ("==", "!="):                        # `src/tychoc.c:7145` -- structural, but the types must be EQUAL.
        ...
        # type error while `2.5 < 7` is not (`src/tychoc.c:7156-7163` explains why).
        return "accept" if lt == rt else "reject"
    if op in ("<", ">", "<=", ">="):              # `src/tychoc.c:7160-7167`
    ...
    if op == "+" and lt == "string":              # `src/tychoc.c:7174` -- string + string|char, ONE-directional
        return "accept" if rt in ("string", "char") else "reject"
    if op in ("<<", ">>"):                        # `src/tychoc.c:7196-7202`
        return "accept" if lt in INTEGER and rt in INTEGER else "reject"
    if op in ("%", "&", "|", "^"):                # `src/tychoc.c:7307` -- two MATCHING integers
        return "accept" if lt in INTEGER and lt == rt else "reject"
    # arithmetic `+ - * /`
    if lt == rt and lt in ({"int", "float", "f32"} | SIZED):   # `src/tychoc.c:7317-7329`
        return "accept"
    if op in ("+", "-") and (lt == "char" or rt == "char") \
       and lt in ("char", "int") and rt in ("char", "int"):    # `src/tychoc.c:7325-7328` -- char±int, int±char, char±char
        return "accept"
    if lt == "float" and rt == "int" and rl: return "accept"   # `src/tychoc.c:7334`
    if rt == "float" and lt == "int" and ll: return "accept"   # `src/tychoc.c:7338`
    return "reject"
