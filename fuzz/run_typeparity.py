...
def adapt(lt, ll, rt, rl):
    ...
    (`src/tychoc.c:6199-6208`): an int literal takes the other side's u32/u64;
...
def expect(lt, lform, op, rt, rform):
    ...
    if op in ("and", "or"):                       # `src/tychoc.c:6183` -- bool operands, no adaptation
        return "accept" if lt == "bool" and rt == "bool" else "reject"
    lt, rt = adapt(lt, ll, rt, rl)
    if op in ("==", "!="):                        # `src/tychoc.c:6214` -- structural, but the types must be EQUAL.
        ...
        # type error while `2.5 < 7` is not (`src/tychoc.c:6225-6232` explains why).
        return "accept" if lt == rt else "reject"
    if op in ("<", ">", "<=", ">="):              # `src/tychoc.c:6229-6236`
    ...
    if op == "+" and lt == "string":              # `src/tychoc.c:6243` -- string + string|char, ONE-directional
        return "accept" if rt in ("string", "char") else "reject"
    if op in ("<<", ">>"):                        # `src/tychoc.c:6265-6271`
        ...
        # require matching operands and is enforced at `src/tychoc.c:6378`.
    ...
    if op in ("%", "&", "|", "^"):                # `src/tychoc.c:6375` -- two MATCHING integers
        return "accept" if lt in INTEGER and lt == rt else "reject"
    # arithmetic `+ - * /`
    if lt == rt and lt in ({"int", "float", "f32"} | SIZED):   # `src/tychoc.c:6385-6397`
        return "accept"
    if op in ("+", "-") and (lt == "char" or rt == "char") \
       and lt in ("char", "int") and rt in ("char", "int"):    # `src/tychoc.c:6393-6396` -- char±int, int±char, char±char
        return "accept"
    if lt == "float" and rt == "int" and rl: return "accept"   # `src/tychoc.c:6402`
    if rt == "float" and lt == "int" and ll: return "accept"   # `src/tychoc.c:6406`
    return "reject"
