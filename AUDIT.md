...
> Multi-agent audit of language semantics, scoped to **semantic corner-cases** and
> **value-semantics / determinism**. Each finding was probed by an auditor that read
> both compilers (`src/tychoc.c`, `compiler/tychoc0.ty`) + `docs/spec/`, then ran a
> minimal repro through **both** the C reference compiler (`tychoc`) and the self-hosted
> compiler (`tychoc0`), and was then **adversarially re-verified** on both. Per the
> review directive, every resolution defers to **Odin, Swift, or Go**. Report only —
> no code was changed.

## Summary

**19 confirmed inconsistencies** across all 11 audited dimensions (12 high · 5 medium · 2 low). 4 raised-but-dismissed (below).

**The dominant theme: the self-hosted `tychoc0` is missing semantic/safety checks that
the C reference `tychoc` enforces.** Most high-severity items are `tychoc0` accepting a
program `tychoc` rejects — several are outright **miscompiles / UB** (SIGSEGV, silent
lossy coercion, use-after-free), not mere message differences. A handful are the reverse
(`tychoc` too strict) or genuine two-sided gaps. The current 4 parity lanes
(type/eq/unary/parfor, accept-reject only) do **not** cover these areas.

### Confirmed, by dimension

| Dimension | Findings | Top severity |
|---|---|---|
| `closures` | 2 | high |
| `concurrency-determinism` | 3 | high |
| `generics-newtype` | 1 | high |
| `integer-overflow` | 2 | high |
| `maps` | 1 | high |
| `match` | 2 | high |
| `numeric-coercion` | 2 | high |
| `option-result-none` | 2 | high |
| `strings-char` | 2 | high |
| `value-semantics` | 2 | high |

Every one of the 11 dimensions was audited and verified (eval-order produced only a
spec-sanctioned dismissal). No dimension is un-covered.
...
- **[HIGH] Plain indexed assignment `a[i] = rhs` evaluates LHS index vs RHS in opposite order across the two compilers, silently producing different program state** — Eval order of `a[i] = rhs` differs, but the spec (§13.4, docs/spec/09-expressions.md:78-92) declares subexpression evaluation order **unspecified by design** (C/Go stance). Not a bug — a candidate only if cross-compiler determinism is later promised.
- **[HIGH] -0.0 vs +0.0 in a composite (float-leaf) map key: hash-by-bits vs eq-by-value breaks the equal-implies-hash-equal invariant, giving seed-visible nondeterministic (tychoc) and cross-compiler-divergent map contents** — Verifier did **not** confirm a cross-compiler divergence (both compilers behaved the same on the ±0.0 composite-key probe). Possible internal wart worth a second look, not a cc/c0 inconsistency.
- **[LOW] tychoc0 emits a generic "type: unknown type ''" where tychoc gives a specific actionable message, for every bare-None-in-aggregate rejection** — Both compilers reject; only the **diagnostic text** differs (tychoc specific, tychoc0 generic). Message-quality gap, not a semantic divergence.
- **[LOW] has_str(T) predicate rejects char even though str(char) is a valid builtin call** — `has_str(T)` rejects `char` although `str(char)` is a valid builtin; verifier did not confirm this as a cc/c0 divergence (both agree). Flag as a possible internal predicate wart.
