  ...
  rules: {
    ...
    builtin: ($) =>
      choice(
        ...
        "floor", "fabs", "reserve",
        // `map_get`/`map_set` were here and are NOT builtins: src/tychoc.c:3107-3110
    ...
    // (src/tychoc.c:711). It is listed even though `.` `.` `<` would already
    ...
    // (src/tychoc.c:626). Without it 204 corpus files failed to lex.
    punctuation: ($) => choice("(", ")", "[", "]", "{", "}", ",", ":", ";"),
  },
});
