...
| `tycho-rsa` | RSA keygen and RSAES-OAEP on `core:bignum` | the textbook vector plus python `pow()` as the oracle, and an OAEP known-answer test against python `cryptography` in both directions; `make rsa-check` |
