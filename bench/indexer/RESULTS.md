# Parallel text indexer: tycho vs C vs Go
...
Corpus: 400 files × 4000 words, 5000-word vocabulary (`gencorpus.ty`, a fixed
...
|------|---------:|-----:|----------|-------------------|
| tycho | 6.9 MB | 36 ms | files=400 tokens=1600000 distinct=5000 csum=4800000 | value semantics: each worker map deep-copied back across `wait()`, zero GC, zero free |
...
**tycho beats Go on peak memory (6.9 vs 8.6 MB) with no garbage collector and
not one line of memory management in the source.** C is tightest (3.1 MB) and
fastest (7 ms); tycho trails on wall (36 ms) — the honest cost of the value
...
`match`-arm accumulator and a `inout` merge are both exactly that.
