# dbquery — tycho vs C vs Go over real SQLite

The first head-to-head on a **real C library**, not a pure-language microbench:
tycho driving in-memory SQLite through its FFI, against the same program in C
...
(tycho's arena vs C's stack/manual vs Go's GC):
...
Re-measured at `-O3` (2026-06-07): tycho 4.3 MB/354 ms, C 4.3 MB/352 ms, Go 7.4 MB/387 ms
— unchanged within noise. This workload is SQLite-dominated, so host-code `-O3` barely
moves it (`run.sh` now builds the C port at `-O3`; the tycho port already builds via the
`-O3` driver default).

### macOS (Apple Silicon, Darwin 25.5, clang 21 — 2026-06-09)

`sh bench/dbquery/run.sh` (checksum `495113400` agrees): tycho **3.2 MB / 410 ms**,
C 3.1 MB / 553 ms. Fair `-O3` best-of-3 (`bench/fair_rest.sh`): tycho 3.2 MB / 274 ms,
C 3.1 MB / 273 ms. tycho is **~par C on a real SQLite FFI workload** on a second OS,
...
no `pkg-config` `.pc` file, so the runner now detects sqlite3 and passes tycho
`--link sqlite3` (instead of `--pkg sqlite3`, which needs pkg-config) — without that
fix the tycho port failed to build on macOS.

## What it shows

- **tycho ≈ C on a real database workload** — within ~0.1 MB of peak RSS and ~3% of
...
- **tycho < Go** — ~1.8× lighter peak RSS than the GC port doing the identical
...
- This workload is **SQLite-dominated**: the absolute footprints are small and
  ...
  *not* where the arena's dramatic wins live — those are the pure-language churn
  ...
  adoption: **on real work behind a real C library, tycho costs what C costs and
  beats the GC, for free.**
- tycho pays a real cost C doesn't: every `sx_col_text` return is **arena-copied**
