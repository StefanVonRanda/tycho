set -u
cd "$(dirname "$0")/../.." || exit 2
TYCHOC="${TYCHOC:-./tychoc1}"; [ -x "$TYCHOC" ] || { echo "no $TYCHOC -- run 'make' first"; exit 2; }
...
$TYCHOC "$D/gcscan.ty" --emit-c -o "$T/gh" >/dev/null 2>&1 && $CC -O3 -o "$T/gh" "$T/gh.c" -lm; row "tycho" "$T/gh" ""
