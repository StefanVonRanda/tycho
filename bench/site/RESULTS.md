# Static-site generation — head-to-head (hier vs C vs Go)
...
times — `site.hi`, `site.c`, `site.go` — and each is fed an identical generated
corpus (`gensite.hi` writes `<i>.md` for `i` in `0..N-1`). Every rendered byte is
...
```
  hier      1.5 MB     123 ms   pages=5000 bytes=8645000 fnv=4062399490
...
**hier holds a flat ~1.5 MB while the corpus grows 20×.** Each page is read and
...
this workload hier's resident set is ~5× smaller than Go's and dead flat.
...
hier is ~2× C and ~1.4× Go on wall-clock here. The string-building rebuilds each
...
memory, not raw speed** — hier delivers C-flat memory automatically, and pays a
...
- Peak RSS is `ru_maxrss` via `bench/peakrss`; Go's figure includes its runtime
  ...
  hier's *flatness across scale* is the load-bearing claim, and it is unambiguous.
