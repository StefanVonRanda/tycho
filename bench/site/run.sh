...
# build the renderers (tycho + C required; Go optional/skipped if absent)
$TYCHOC "$D/gensite.ty" -o "$T/gensite" >"$T/log" 2>&1 || { echo "gensite build failed"; cat "$T/log"; exit 2; }
$TYCHOC "$D/site.ty"    -o "$T/tycho"    >"$T/log" 2>&1 || { echo "tycho build failed"; cat "$T/log"; exit 2; }
...
# generate the corpus (dogfoods tycho's write_file)
...
run_one tycho "$T/tycho"
run_one c    "$T/c"
[ "$have_go" = 1 ] && run_one go "$T/go" || printf '  %-5s (go not installed)\n' go

[ "$FAIL" = 0 ] && echo "site-bench: OK (identical checksum -- tycho holds C-flat memory, no manual free, no GC)" || { echo "site-bench: CHECKSUM MISMATCH"; exit 1; }
