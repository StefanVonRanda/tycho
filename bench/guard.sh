...
# workloads hier's bump-allocate + bulk-free must comfortably BEAT hand-written
# malloc/free C. The check is RELATIVE (hier_ms vs C_ms on the same machine), so
# it's portable: healthy hier is ~0.23x C, the regression was ~0.93x C, and the
# 0.60x gate cleanly separates them with margin for noise.
set -eu
cd "$(dirname "$0")/.."
T=$(mktemp -d); trap 'rm -rf "$T"' EXIT
cc -O2 -o "$T/pk" bench/peakrss.c
GATE_NUM=60   # hier must be < GATE_NUM/100 of C's wall (i.e. < 0.60x C)
...
for w in binary_trees maptree; do
  ./hierc "bench/prongB/$w.hi" --emit-c -o "$T/h" >/dev/null 2>&1
  ...
  if [ $((100 * h)) -lt $((GATE_NUM * c)) ]; then
    printf '  ok    %-14s hier=%sms  C=%sms  (%d%% of C, gate <%d%%)\n' "$w" "$h" "$c" $((100 * h / c)) "$GATE_NUM"
  else
    printf '  FAIL  %-14s hier=%sms  C=%sms  (%d%% of C, gate <%d%%) -- tree-alloc perf regressed\n' "$w" "$h" "$c" $((100 * h / c)) "$GATE_NUM"
    rc=1
  fi
done
[ "$rc" -eq 0 ] && echo "bench-guard: ok (hier beats C on tree workloads)" || echo "bench-guard: FAILED"
exit "$rc"
